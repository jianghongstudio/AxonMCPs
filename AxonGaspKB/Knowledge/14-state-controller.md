...
|---|-----|------|------|-----------|----------|---------------|
| 1 | `Idle -> Locomotion` | `Transition to Locomotion` | state→state | 0.20s | 角色正在移动（`IsMoving()`：当前/未来速度或加速度非零） | `Is Moving` |
| 2 | `Locomotion -> Idle` | `Transition to Idle` | state→state | 0.20s | 角色正在移动（`IsMoving()`：当前/未来速度或加速度非零）；取反 | `Is Moving`, `NOT Boolean` |
...
| 19 | `Conduit` | `Transition to Locomotion` | conduit→state | 0.20s | 角色正在移动（`IsMoving()`：当前/未来速度或加速度非零） | `Is Moving` |
| 20 | `Conduit` | `Transition to Idle` | conduit→state | 0.20s | 角色正在移动（`IsMoving()`：当前/未来速度或加速度非零）；取反 | `Is Moving`, `NOT Boolean` |
