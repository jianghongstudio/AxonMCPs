...
// delegate to FAxonReflectionWalker (AxonCore). Writes reuse the
// FAxonDryRunReport / FAxonBulkFillFieldWrite reporting shape so callers get the same
...
namespace AxonDataTableInternal
{
	...
	// values object, so FAxonReflectionWalker (which matches on internal names)
	...
	static TSharedPtr<FJsonObject> ToCanonicalTree(const UScriptStruct* RowStruct, const TSharedPtr<FJsonObject>& FriendlyValues)
	{
		...
		{
			if (FProperty* Prop = ResolveRowProperty(RowStruct, AxonKeyToString(Pair.Key)))
			{
				Tree->SetField(Prop->GetName(), Pair.Value);
			}
			else
			{
				Tree->SetField(AxonKeyToString(Pair.Key), Pair.Value);
			}
		}
		return Tree;
	}
	...
	static TArray<TSharedPtr<FJsonValue>> BuildSchemaArray(UScriptStruct* RowStruct)
	{
		...
		const FAxonSchemaDescriptor Root = FAxonReflectionWalker::DescribeStruct(RowStruct);
		for (const FAxonSchemaDescriptor& Child : Root.Children)
		{
...
void FAxonBlueprintDataTableActions::RegisterActions(FAxonToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("read_data_table"),
		TEXT("Read a DataTable's full contents plus its inline row schema. Returns the row struct, total_rows, an FAxonSchemaDescriptor-shaped 'schema' array (field type_name, import_text_form, enum_values, range, nested children), and a 'rows' array of {row_name, values}. Supersedes get_data_table_rows by inlining schema with data."),
		FAxonActionHandler::CreateStatic(&HandleReadDataTable),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("describe_data_table_schema"),
		TEXT("Return ONLY a DataTable's row schema (no row data). Useful for planning edits to a large table. Returns row_struct, row_struct_path, and an FAxonSchemaDescriptor-shaped 'schema' array."),
		FAxonActionHandler::CreateStatic(&HandleDescribeDataTableSchema),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("DataTable asset path, e.g. /Game/Data/DT_Weapons"))
			.Build());

	Registry.RegisterAction(TEXT("blueprint"), TEXT("set_data_table_rows"),
		TEXT("Bulk add/update DataTable rows in one call. Each row is {row_name, values:{field:value}, mode?}. Mode is upsert (default), add, or update. Supports dry_run (validate only) and strict (promote coercion/unknown-field/enum-miss to hard errors). Returns an FAxonDryRunReport-shaped per-field result {path,current,proposed,ok,reason}. Fires one editor-refresh broadcast at the end."),
		FAxonActionHandler::CreateStatic(&HandleSetDataTableRows),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("DataTable asset path, e.g. /Game/Data/DT_Weapons"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("remove_data_table_row"),
		...
		FAxonActionHandler::CreateStatic(&HandleRemoveDataTableRow),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("rename_data_table_row"),
		...
		FAxonActionHandler::CreateStatic(&HandleRenameDataTableRow),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("DataTable asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("duplicate_data_table_row"),
		...
		FAxonActionHandler::CreateStatic(&HandleDuplicateDataTableRow),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("DataTable asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("export_data_table"),
		...
		FAxonActionHandler::CreateStatic(&HandleExportDataTable),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"),       TEXT("DataTable asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("import_data_table"),
		...
		FAxonActionHandler::CreateStatic(&HandleImportDataTable),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("DataTable asset path"))
...
FAxonActionResult FAxonBlueprintDataTableActions::HandleReadDataTable(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonDataTableInternal;
...
FAxonActionResult FAxonBlueprintDataTableActions::HandleDescribeDataTableSchema(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonDataTableInternal;
...
FAxonActionResult FAxonBlueprintDataTableActions::HandleSetDataTableRows(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonDataTableInternal;
	...
	{
		return FAxonActionResult::Error(TEXT("Missing or empty required parameter: rows (array of {row_name, values, mode?})"));
	}
	...
	if (!bDryRun)
	{
		...
		Transaction = MakeUnique<FScopedTransaction>(NSLOCTEXT(
			"AxonBlueprintDataTableActions", "SetDataTableRows", "Axon Set DataTable Rows"));
		DataTable->Modify();
	}
	...
	for (const TSharedPtr<FJsonValue>& RowVal : *RowsArray)
	{
		...
		FAxonDryRunReport RowReport = bDryRun
			? FAxonReflectionWalker::InspectTree(CanonicalTree, RowStruct, RowData, Spec)
			: FAxonReflectionWalker::WriteTree(CanonicalTree, RowStruct, RowData, nullptr, Spec);
		...
		for (const FAxonBulkFillFieldWrite& W : RowReport.FieldWrites)
		{
...
FAxonActionResult FAxonBlueprintDataTableActions::HandleRemoveDataTableRow(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonDataTableInternal;
	...
	if (RowName.IsEmpty())   { return FAxonActionResult::Error(TEXT("Missing required parameter: row_name")); }

	const UScriptStruct* RowStruct = nullptr;
	FString Error;
	UDataTable* DataTable = ResolveDataTable(AssetPath, RowStruct, Error);
	if (!DataTable) { return FAxonActionResult::Error(Error); }
...
FAxonActionResult FAxonBlueprintDataTableActions::HandleRenameDataTableRow(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonDataTableInternal;
	...
	if (AssetPath.IsEmpty()) { return FAxonActionResult::Error(TEXT("Missing required parameter: asset_path")); }
	if (OldName.IsEmpty())   { return FAxonActionResult::Error(TEXT("Missing required parameter: old_name")); }
	if (NewName.IsEmpty())   { return FAxonActionResult::Error(TEXT("Missing required parameter: new_name")); }
...
FAxonActionResult FAxonBlueprintDataTableActions::HandleDuplicateDataTableRow(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonDataTableInternal;
	...
	const FString NewName = Params->GetStringField(TEXT("new_name"));
	if (AssetPath.IsEmpty())  { return FAxonActionResult::Error(TEXT("Missing required parameter: asset_path")); }
	if (SourceRow.IsEmpty())  { return FAxonActionResult::Error(TEXT("Missing required parameter: source_row")); }
	if (NewName.IsEmpty())    { return FAxonActionResult::Error(TEXT("Missing required parameter: new_name")); }
...
FAxonActionResult FAxonBlueprintDataTableActions::HandleExportDataTable(const TSharedPtr<FJsonObject>& Params)
{
#if WITH_EDITOR
	using namespace AxonDataTableInternal;
	...
	return FAxonActionResult::Success(Root);
#else
	return FAxonActionResult::Error(TEXT("export_data_table requires an editor build (GetTableAsJSON/CSV are WITH_EDITOR-only)."));
#endif
}
...
FAxonActionResult FAxonBlueprintDataTableActions::HandleImportDataTable(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonDataTableInternal;
	...
	if (AssetPath.IsEmpty()) { return FAxonActionResult::Error(TEXT("Missing required parameter: asset_path")); }
	if (Text.IsEmpty())      { return FAxonActionResult::Error(TEXT("Missing required parameter: text")); }
	...
	FScopedTransaction Transaction(NSLOCTEXT(
		"AxonBlueprintDataTableActions", "ImportDataTable", "Axon Import DataTable"));
	DataTable->Modify();
