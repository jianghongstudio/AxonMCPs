...
#include "Reflection/AxonReflectionReader.h"
...
void FAxonBlueprintStructActions::RegisterActions(FAxonToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("create_user_defined_struct"),
		...
		FAxonActionHandler::CreateStatic(&HandleCreateUserDefinedStruct),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("create_user_defined_enum"),
		...
		FAxonActionHandler::CreateStatic(&HandleCreateUserDefinedEnum),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("create_data_table"),
		...
		FAxonActionHandler::CreateStatic(&HandleCreateDataTable),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("add_data_table_row"),
		...
		FAxonActionHandler::CreateStatic(&HandleAddDataTableRow),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("get_data_table_rows"),
		...
		FAxonActionHandler::CreateStatic(&HandleGetDataTableRows),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("DataTable asset path, e.g. /Game/Data/DT_Weapons"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("create_data_asset"),
		...
		FAxonActionHandler::CreateStatic(&HandleCreateDataAsset),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("seed_data_asset"),
		TEXT("Create AND populate a UObject DataAsset in one call: create_data_asset's body plus a reflection-walker fill of the supplied property 'tree'. Supports dry_run (validate before creating) and strict. Returns asset_path, actual_class, and FAxonDryRunReport-shaped field_writes."),
		FAxonActionHandler::CreateStatic(&HandleSeedDataAsset),
		FParamSchemaBuilder()
...
FAxonActionResult FAxonBlueprintStructActions::HandleCreateUserDefinedStruct(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath = Params->GetStringField(TEXT("save_path"));
	if (SavePath.IsEmpty())
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: save_path"));
	}
	...
	{
		return FAxonActionResult::Error(TEXT("Missing or empty required parameter: fields (array of {name, type, default_value?})"));
	}
	...
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Invalid save_path — must contain at least one '/': %s"), *SavePath));
	}
	FString AssetName = SavePath.Mid(LastSlash + 1);
	if (AssetName.IsEmpty())
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("save_path must not end with '/': %s"), *SavePath));
	}
	...
	if (!Package)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Failed to create package at path: %s"), *SavePath));
	}
	Package->FullyLoad();
	...
	if (!Struct)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("FStructureEditorUtils::CreateUserDefinedStruct failed for: %s"), *AssetName));
	}
...
FAxonActionResult FAxonBlueprintStructActions::HandleCreateUserDefinedEnum(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath = Params->GetStringField(TEXT("save_path"));
	if (SavePath.IsEmpty())
	{
	...
	{
		return FAxonActionResult::Error(TEXT("Missing or empty required parameter: values (array of strings)"));
	}
	...
	if (!Enum)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("FEnumEditorUtils::CreateUserDefinedEnum failed for: %s"), *AssetName));
	}
...
FAxonActionResult FAxonBlueprintStructActions::HandleCreateDataTable(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath = Params->GetStringField(TEXT("save_path"));
	if (SavePath.IsEmpty())
	{
	...
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: row_struct"));
	}
	...
	if (!DataTable)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Failed to create UDataTable: %s"), *AssetName));
	}
...
FAxonActionResult FAxonBlueprintStructActions::HandleAddDataTableRow(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (RowName.IsEmpty())
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: row_name"));
	}
	...
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: values (JSON object of column->value)"));
	}

	// Load the DataTable
	UDataTable* DataTable = FAxonAssetUtils::LoadAssetByPath<UDataTable>(AssetPath);
	if (!DataTable)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("DataTable not found: %s"), *AssetPath));
	}

	const UScriptStruct* RowStruct = DataTable->GetRowStruct();
	if (!RowStruct)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("DataTable '%s' has no RowStruct set"), *AssetPath));
	}
	...
	{
		const FString FieldName = AxonKeyToString(Pair.Key);
		...
		if (FAxonReflectionWalker::ResolveUserDefinedEnumToken(Prop, ValueStr, ResolvedEnumValue))
		{
...
FAxonActionResult FAxonBlueprintStructActions::HandleGetDataTableRows(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	{
...
FAxonActionResult FAxonBlueprintStructActions::HandleCreateDataAsset(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath = Params->GetStringField(TEXT("save_path"));
	if (SavePath.IsEmpty())
	{
	...
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: class_name"));
	}
	...
	FScopedTransaction Transaction(NSLOCTEXT("AxonBlueprintStructActions",
		"CreateDataAsset", "Axon Create Data Asset"));
	NewAsset->Modify();
	...
	for (TFieldIterator<FProperty> It(ResolvedClass); It; ++It)
	{
		...
		if (Prop->HasAnyPropertyFlags(CPF_Transient | CPF_Deprecated))
			continue;
		AxonEditCradle::FireFullCradle(NewAsset, Prop);
	}
...
namespace AxonSeedDataAssetInternal
{
	// Serialise an FAxonDryRunReport's field writes into the response (same shape as
	// FAxonDryRunGuard::ReportToJson, inlined here to avoid extra includes).
	static void AppendFieldWrites(const TSharedPtr<FJsonObject>& Root, const FAxonDryRunReport& Report)
	{
...
FAxonActionResult FAxonBlueprintStructActions::HandleSeedDataAsset(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonSeedDataAssetInternal;
	...
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: tree (JSON object of property->value)"));
	}
	...
	if (bDryRun)
	{
		...
		FAxonDryRunReport Report = FAxonReflectionWalker::InspectTree(Tree, ResolvedClass, CDO, Spec);
	...
	// Engine edit cradle + walker fill (mirror AxonBlueprintBulkFillAdapter).
	NewAsset->SetFlags(RF_Transactional);
	FScopedTransaction Transaction(NSLOCTEXT("AxonBlueprintStructActions",
		"SeedDataAsset", "Axon Seed Data Asset"));
	NewAsset->Modify();
	NewAsset->PreEditChange(nullptr);

	FAxonDryRunReport Report = FAxonReflectionWalker::WriteTree(Tree, ResolvedClass, NewAsset, NewAsset, Spec);
	...
	{
		FProperty* TopProp = FAxonReflectionWalker::FindPropertyForwarding(ResolvedClass, AxonKeyToString(KV.Key));
		if (TopProp)
		{
			AxonEditCradle::ReparentTransientInstancedSubobjects(NewAsset, TopProp);
			AxonEditCradle::FireFullCradle(NewAsset, TopProp);
		}
	}
		...
		for (const auto& KV : Tree->Values)
		{
			...
			if (TopProp)
			{
				...
				Values->SetField(TopProp->GetName(),
					FAxonReflectionReader::PropertyToJsonValue(TopProp, ValuePtr, NewAsset));
			}
		}
