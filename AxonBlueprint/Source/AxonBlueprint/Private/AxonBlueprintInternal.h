...
namespace AxonBlueprintInternal
{
	/**
	...
	inline UBlueprint* LoadBlueprintFromParams(const TSharedPtr<FJsonObject>& Params, FString& OutAssetPath)
	{
		...
		UBlueprint* BP = FAxonAssetUtils::LoadAssetByPath<UBlueprint>(OutAssetPath);
		if (BP) return BP;
	...
	inline TSharedPtr<FJsonObject> SerializeNode(UEdGraphNode* Node)
	{
		...
		AxonPropertyAccessReader::SerializePropertyAccessBlock(Node, NObj);
	 ...
	 */
	inline FAxonActionResult ResolveDelegateOwnerAndProperty(
