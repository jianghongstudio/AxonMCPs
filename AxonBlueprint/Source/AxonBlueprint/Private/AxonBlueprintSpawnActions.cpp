	...
	const FString& MobilityStr)
{
	...
	if (!MobilityStr.IsEmpty())
	{
		...
		if (AxonParamUtils::ParseMobility(MobilityStr, Mobility))
		{
...
void FAxonBlueprintSpawnActions::RegisterActions(FAxonToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("spawn_blueprint_actor"),
		...
		FAxonActionHandler::CreateStatic(&HandleSpawnBlueprintActor),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("batch_spawn_blueprint_actors"),
		...
		FAxonActionHandler::CreateStatic(&HandleBatchSpawnBlueprintActors),
		FParamSchemaBuilder()
...
FAxonActionResult FAxonBlueprintSpawnActions::HandleSpawnBlueprintActor(const TSharedPtr<FJsonObject>& Params)
{
	// PIE guard
	if (GEditor && GEditor->IsPlayingSessionInEditor())
	{
		return FAxonActionResult::Error(TEXT("Cannot spawn Blueprint actors during Play-In-Editor session"));
	}
	...
	{
		return FAxonActionResult::Error(TEXT("Missing required param: blueprint"));
	}

	// Normalize and load class
	FString ClassPath = AxonParamUtils::NormalizeBlueprintClassPath(BlueprintPath);
	...
	// Editor world
	UWorld* World = AxonParamUtils::GetEditorWorld();
	if (!World)
	{
		return FAxonActionResult::Error(TEXT("No editor world available. Is a level open?"));
	}

	// Parse optional transform
	FVector Location(0, 0, 0);
	AxonParamUtils::ParseVector(Params, TEXT("location"), Location);

	FRotator Rotation(0, 0, 0);
	AxonParamUtils::ParseRotator(Params, TEXT("rotation"), Rotation);

	FVector Scale(1, 1, 1);
	AxonParamUtils::ParseVector(Params, TEXT("scale"), Scale);
	...
	if (!TargetLevel)
	{
		return FAxonActionResult::Error(SublevelError);
	}

	// Begin transaction
	GEditor->BeginTransaction(NSLOCTEXT("Axon", "SpawnBPActor", "Axon: Spawn Blueprint Actor"));
	...
	Result->SetArrayField(TEXT("location"), AxonParamUtils::VectorToJsonArray(NewActor->GetActorLocation()));
	{
	...
	Result->SetArrayField(TEXT("scale"), AxonParamUtils::VectorToJsonArray(NewActor->GetActorScale3D()));
...
FAxonActionResult FAxonBlueprintSpawnActions::HandleBatchSpawnBlueprintActors(const TSharedPtr<FJsonObject>& Params)
{
	// PIE guard
	if (GEditor && GEditor->IsPlayingSessionInEditor())
	{
	...
	{
		return FAxonActionResult::Error(TEXT("Missing or invalid required param: count (must be >= 1)"));
	}
	int32 Count = FMath::RoundToInt32(CountD);
	if (Count > 1000)
	{
		return FAxonActionResult::Error(TEXT("count exceeds maximum of 1000"));
	}
	...
	AxonParamUtils::ParseVector(Params, TEXT("origin"), Origin);

	double Spacing = 200.0;
	Params->TryGetNumberField(TEXT("spacing"), Spacing);
	if (Spacing <= 0)
	{
		return FAxonActionResult::Error(TEXT("spacing must be positive"));
	}
	...
	AxonParamUtils::ParseVector(Params, TEXT("direction"), Direction);
	...
	GEditor->BeginTransaction(NSLOCTEXT("Axon", "BatchSpawnBP", "Axon: Batch Spawn Blueprint Actors"));
	...
	for (int32 i = 0; i < Count; i++)
	{
		...
		EntryObj->SetArrayField(TEXT("location"), AxonParamUtils::VectorToJsonArray(NewActor->GetActorLocation()));
