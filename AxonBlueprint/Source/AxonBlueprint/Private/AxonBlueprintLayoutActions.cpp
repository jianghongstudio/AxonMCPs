...
void FAxonBlueprintLayoutActions::RegisterActions(FAxonToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("auto_layout"),
		...
		FParamSchemaBuilder()
			...
			.Optional(TEXT("formatter"), TEXT("string"), TEXT("Formatter: 'auto' (default, prefers Blueprint Assist if available), 'blueprint_assist' (BA only, fails if unavailable), or 'axon' (built-in Sugiyama)"), TEXT("auto"))
			.Build());
}
...
FAxonActionResult FAxonBlueprintLayoutActions::HandleAutoLayout(const TSharedPtr<FJsonObject>& Params)
{
	// --- Parse params ---
	FString AssetPath;
	...
	// Validate formatter
	if (Formatter != TEXT("auto") && Formatter != TEXT("blueprint_assist") && Formatter != TEXT("axon"))
	{
		return FAxonActionResult::Error(FString::Printf(
			TEXT("Unknown formatter '%s'. Valid: 'auto', 'blueprint_assist', 'axon'"), *Formatter));
	}
	...
	if (Formatter == TEXT("auto") || Formatter == TEXT("blueprint_assist"))
	{
		...
		if (bBAAvailable)
		{
			...
			if (IAxonGraphFormatter::Get().FormatGraph(Graph, NodesFormatted, ErrorMessage))
			{
				...
				FAxonFormatterInfo Info = IAxonGraphFormatter::Get().GetFormatterInfo(Graph);
	...
	if (LayoutNodes.Num() == 0)
	{
		...
		R->SetStringField(TEXT("formatter_used"), TEXT("axon"));
		return FAxonActionResult::Success(R);
	}
	...
	ResultJson->SetStringField(TEXT("formatter_used"), TEXT("axon"));
