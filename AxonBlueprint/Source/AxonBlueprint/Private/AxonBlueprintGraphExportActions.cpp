...
void FAxonBlueprintGraphExportActions::RegisterActions(FAxonToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("export_graph"),
		...
		FAxonActionHandler::CreateStatic(&HandleExportGraph),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("copy_nodes"),
		...
		FAxonActionHandler::CreateStatic(&HandleCopyNodes),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("duplicate_graph"),
		...
		FAxonActionHandler::CreateStatic(&HandleDuplicateGraph),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blueprint asset path"))
...
namespace
{
	...
	TSharedPtr<FJsonObject> SerializeNodeExtended(UEdGraphNode* Node)
	{
		...
		TSharedPtr<FJsonObject> NObj = AxonBlueprintInternal::SerializeNode(Node);
...
FAxonActionResult FAxonBlueprintGraphExportActions::HandleExportGraph(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
...
FAxonActionResult FAxonBlueprintGraphExportActions::HandleCopyNodes(const TSharedPtr<FJsonObject>& Params)
{
	// Load source Blueprint
	FString SourceAssetPath = Params->GetStringField(TEXT("source_asset"));
	if (SourceAssetPath.IsEmpty())
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: source_asset"));
	}
	UBlueprint* SourceBP = FAxonAssetUtils::LoadAssetByPath<UBlueprint>(SourceAssetPath);
	if (!SourceBP)
	{
		// Try level blueprint fallback
		SourceBP = AxonBlueprintInternal::TryLoadLevelBlueprint(SourceAssetPath);
	}
	if (!SourceBP)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Source Blueprint not found: %s"), *SourceAssetPath));
	}
	...
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: target_asset"));
	}
	UBlueprint* TargetBP = FAxonAssetUtils::LoadAssetByPath<UBlueprint>(TargetAssetPath);
	if (!TargetBP)
	{
		TargetBP = AxonBlueprintInternal::TryLoadLevelBlueprint(TargetAssetPath);
	}
	if (!TargetBP)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Target Blueprint not found: %s"), *TargetAssetPath));
	}

	// Find source graph
	FString SourceGraphName = Params->GetStringField(TEXT("source_graph"));
	UEdGraph* SourceGraph = AxonBlueprintInternal::FindGraphByName(SourceBP, SourceGraphName);
	if (!SourceGraph)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Source graph not found: %s"), *SourceGraphName));
	}

	// Find target graph
	FString TargetGraphName = Params->GetStringField(TEXT("target_graph"));
	UEdGraph* TargetGraph = AxonBlueprintInternal::FindGraphByName(TargetBP, TargetGraphName);
	if (!TargetGraph)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Target graph not found: %s"), *TargetGraphName));
	}
	...
	{
		return FAxonActionResult::Error(TEXT("Missing or empty required parameter: node_ids"));
	}
...
FAxonActionResult FAxonBlueprintGraphExportActions::HandleDuplicateGraph(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = AxonBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: graph_name"));
	}
	...
	UEdGraph* SourceGraph = AxonBlueprintInternal::FindGraphByName(BP, GraphName);
	if (!SourceGraph)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Graph not found: %s"), *GraphName));
	}
	...
	if (AxonBlueprintInternal::FindGraphByName(BP, NewName))
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("A graph named '%s' already exists in this Blueprint"), *NewName));
	}
	...
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Schema does not allow duplication of graph '%s'"), *GraphName));
	}
	...
	if (!DuplicatedGraph)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Failed to duplicate graph '%s'"), *GraphName));
	}
