...
DEFINE_LOG_CATEGORY_STATIC(LogAxonBpContract, Log, All);
...
// ============================================================
namespace AxonBpContractInternal
{
	...
	static UStruct* ContractResolveSide(const FString& Side, UClass*& OutClass, UBlueprint*& OutBP, FString& OutKind, FString& OutError)
	{
		...
		if (Side.Contains(TEXT("/")))
		{
			...
			FString Resolved;
			if (UBlueprint* BP = AxonBlueprintInternal::LoadBlueprintFromParams(Synthetic, Resolved))
			{
		...
		// Native class name resolution (mirrors AxonBlueprintStructActions resolver).
...
void FAxonBlueprintContractActions::RegisterActions(FAxonToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("compare_class_variable_contract"),
		...
		FAxonActionHandler::CreateStatic(&HandleCompareClassVariableContract),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("promote_variables_to_parent"),
		...
		FAxonActionHandler::CreateStatic(&HandlePromoteVariablesToParent),
		FParamSchemaBuilder()
	...
	UE_LOG(LogAxonBpContract, Log, TEXT("AxonBlueprint Contract: registered 2 actions"));
}
...
FAxonActionResult FAxonBlueprintContractActions::HandleCompareClassVariableContract(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonBpContractInternal;

	const FString LeftSide  = Params->GetStringField(TEXT("left"));
	const FString RightSide = Params->GetStringField(TEXT("right"));
	if (LeftSide.IsEmpty())  return FAxonActionResult::Error(TEXT("Missing required parameter: left"));
	if (RightSide.IsEmpty()) return FAxonActionResult::Error(TEXT("Missing required parameter: right"));
	...
	if (!LeftStruct) return FAxonActionResult::Error(LeftErr);
	...
	if (!RightStruct) return FAxonActionResult::Error(RightErr);
...
FAxonActionResult FAxonBlueprintContractActions::HandlePromoteVariablesToParent(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonBpContractInternal;
	...
	if (!BP->GeneratedClass)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Blueprint '%s' has no GeneratedClass (needs compile)."), *AssetPath));
	}
	...
	if (!Params->TryGetArrayField(TEXT("variables"), RequestedVars) || !RequestedVars || RequestedVars->Num() == 0)
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: variables (non-empty array of names)"));
	}
