...
void FAxonBlueprintBuildActions::RegisterActions(FAxonToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("build_blueprint_from_spec"),
		...
		FAxonActionHandler::CreateStatic(&HandleBuildBlueprintFromSpec),
		FParamSchemaBuilder()
...
FAxonActionResult FAxonBlueprintBuildActions::HandleBuildBlueprintFromSpec(const TSharedPtr<FJsonObject>& Params)
{
	// ---- Load Blueprint ----
	FString AssetPath;
	...
	// ---- Begin transaction ----
	GEditor->BeginTransaction(NSLOCTEXT("Axon", "BuildBPFromSpec", "Build Blueprint From Spec"));
	...
	// ============================================================
	{
		...
		if (ParseJsonArray(Params, TEXT("variables"), VarSpecs))
		{
			for (int32 i = 0; i < VarSpecs.Num(); ++i)
			{
				...
				FAxonActionResult Result = FAxonBlueprintVariableActions::HandleAddVariable(SubParams);
				if (Result.bSuccess)
				{
					VariablesCreated++;
				}
	...
	// ============================================================
	{
		...
		if (ParseJsonArray(Params, TEXT("components"), CompSpecs))
		{
			for (int32 i = 0; i < CompSpecs.Num(); ++i)
			{
				...
				FAxonActionResult Result = FAxonBlueprintComponentActions::HandleAddComponent(SubParams);
				if (Result.bSuccess)
				{
					ComponentsCreated++;
				}
	...
	// ============================================================
	{
		...
		if (ParseJsonArray(Params, TEXT("nodes"), NodeSpecs))
		{
			for (int32 i = 0; i < NodeSpecs.Num(); ++i)
			{
				...
				FAxonActionResult Result = FAxonBlueprintNodeActions::HandleAddNode(SubParams);
				if (Result.bSuccess)
				{
					NodesCreated++;
	...
	// ============================================================
	{
		...
		if (ParseJsonArray(Params, TEXT("connections"), ConnSpecs))
		{
			for (int32 i = 0; i < ConnSpecs.Num(); ++i)
			{
				...
				FAxonActionResult Result = FAxonBlueprintNodeActions::HandleConnectPins(SubParams);
				if (Result.bSuccess)
				{
					ConnectionsMade++;
				}
	...
	// ============================================================
	{
		...
		if (ParseJsonArray(Params, TEXT("pin_defaults"), PinSpecs))
		{
			for (int32 i = 0; i < PinSpecs.Num(); ++i)
			{
				...
				FAxonActionResult Result = FAxonBlueprintNodeActions::HandleSetPinDefault(SubParams);
				if (Result.bSuccess)
				{
					PinDefaultsSet++;
				}
	...
	if (bAutoCompile)
	{
		...
		FAxonActionResult CResult = FAxonBlueprintCompileActions::HandleCompileBlueprint(CompileParams);
		bCompiled = true;
