...
void FAxonBlueprintTemplateActions::RegisterActions(FAxonToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("apply_template"),
		...
		FAxonActionHandler::CreateStatic(&HandleApplyTemplate),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("list_templates"),
		...
		FAxonActionHandler::CreateStatic(&HandleListTemplates),
		FParamSchemaBuilder()
			.Build());
}
		...
		const TMap<FString, FString>& Defaults)
	{
		...
		if (UserParams.IsValid())
		{
			for (const auto& Pair : UserParams->Values)
			{
				...
				{
					Result.Add(AxonKeyToString(Pair.Key), Pair.Value->AsString());
				}
				else if (Pair.Value->Type == EJson::Number)
				{
					Result.Add(AxonKeyToString(Pair.Key), FString::SanitizeFloat(Pair.Value->AsNumber()));
				}
				else if (Pair.Value->Type == EJson::Boolean)
				{
					Result.Add(AxonKeyToString(Pair.Key), Pair.Value->AsBool() ? TEXT("true") : TEXT("false"));
				}
			}
		}
...
FAxonActionResult FAxonBlueprintTemplateActions::HandleListTemplates(const TSharedPtr<FJsonObject>& Params)
{
...
FAxonActionResult FAxonBlueprintTemplateActions::HandleApplyTemplate(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: template_name"));
	}
	...
	UBlueprint* BP = FAxonAssetUtils::LoadAssetByPath<UBlueprint>(AssetPath);
	if (!BP)
	{
	...
	FAxonActionResult BatchResult = FAxonBlueprintNodeActions::HandleBatchExecute(BatchParams);
