...
void FAxonBlueprintDiffActions::RegisterActions(FAxonToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("compare_blueprints"),
		...
		FAxonActionHandler::CreateStatic(&HandleCompareBlueprints),
		FParamSchemaBuilder()
...
namespace
{
	...
	TArray<FVarInfo> CollectVariables(UBlueprint* BP)
	{
		...
		for (const FBPVariableDescription& Var : BP->NewVariables)
		{
			...
			Info.Type = AxonBlueprintInternal::ContainerPrefix(Var.VarType) +
...
FAxonActionResult FAxonBlueprintDiffActions::HandleCompareBlueprints(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (PathA.IsEmpty())
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: asset_path_a"));
	}
	if (PathB.IsEmpty())
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: asset_path_b"));
	}

	UBlueprint* BPA = FAxonAssetUtils::LoadAssetByPath<UBlueprint>(PathA);
	if (!BPA)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Blueprint A not found: %s"), *PathA));
	}

	UBlueprint* BPB = FAxonAssetUtils::LoadAssetByPath<UBlueprint>(PathB);
	if (!BPB)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Blueprint B not found: %s"), *PathB));
	}
