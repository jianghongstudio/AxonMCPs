...
void FAxonMotionMatchingScaffoldActions::RegisterActions(FAxonToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("set_anim_class"),
		...
		FAxonActionHandler::CreateStatic(&HandleSetAnimClass),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("apply_movement_preset"),
		...
		FAxonActionHandler::CreateStatic(&HandleApplyMovementPreset),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("add_engine_component_typed"),
		...
		FAxonActionHandler::CreateStatic(&HandleAddEngineComponentTyped),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("scaffold_locomotion_input"),
		...
		FAxonActionHandler::CreateStatic(&HandleScaffoldLocomotionInput),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("bp_path"), TEXT("Character Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("validate_animbp_variable_contract"),
		...
		FAxonActionHandler::CreateStatic(&HandleValidateAnimBpVariableContract),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("scaffold_motion_matching_character"),
		...
		FAxonActionHandler::CreateStatic(&HandleScaffoldMotionMatchingCharacter),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("get_inherited_component_override"),
		...
		FAxonActionHandler::CreateStatic(&HandleGetInheritedComponentOverride),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("scaffold_threadsafe_update"),
		...
		FAxonActionHandler::CreateStatic(&HandleScaffoldThreadSafeUpdate),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("abp_path"), TEXT("Animation Blueprint asset path"))
			.Build());
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("add_pawn_owner_access"),
		...
		FAxonActionHandler::CreateStatic(&HandleAddPawnOwnerAccess),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("abp_path"), TEXT("Animation Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("scaffold_locomotion_anim_values"),
		...
		FAxonActionHandler::CreateStatic(&HandleScaffoldLocomotionAnimValues),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("abp_path"), TEXT("Animation Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("apply_locomotion_speed_band"),
		...
		FAxonActionHandler::CreateStatic(&HandleApplyLocomotionSpeedBand),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("bp_path"), TEXT("Character Blueprint asset path"))
...
namespace
{
	 ...
	 * Done by string match on the node class so AxonBlueprint need not depend on
...
FAxonActionResult FAxonMotionMatchingScaffoldActions::HandleSetAnimClass(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (BpPath.IsEmpty())     return FAxonActionResult::Error(TEXT("Missing required parameter: bp_path"));
	if (CompName.IsEmpty())   return FAxonActionResult::Error(TEXT("Missing required parameter: component"));
	if (AnimBpPath.IsEmpty()) return FAxonActionResult::Error(TEXT("Missing required parameter: anim_bp_path"));

	UBlueprint* BP = FAxonAssetUtils::LoadAssetByPath<UBlueprint>(BpPath);
	if (!BP)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Blueprint not found: %s"), *BpPath));
	}

	UAnimBlueprint* ABP = FAxonAssetUtils::LoadAssetByPath<UAnimBlueprint>(AnimBpPath);
	if (!ABP)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Animation Blueprint not found: %s"), *AnimBpPath));
	}
...
FAxonActionResult FAxonMotionMatchingScaffoldActions::HandleApplyMovementPreset(const TSharedPtr<FJsonObject>& Params)
{
	const FString BpPath = Params->GetStringField(TEXT("bp_path"));
	const FString Preset = Params->GetStringField(TEXT("preset"));

	if (BpPath.IsEmpty()) return FAxonActionResult::Error(TEXT("Missing required parameter: bp_path"));
	if (Preset.IsEmpty()) return FAxonActionResult::Error(TEXT("Missing required parameter: preset"));
	...
	if (!CmcClass)
	{
		return FAxonActionResult::Error(TEXT("apply_movement_preset: UCharacterMovementComponent class not found"));
	}
	...
	for (const TPair<FString, FString>& Write : Writes)
	{
		...
		FAxonActionResult R = FAxonBlueprintComponentActions::HandleSetComponentProperty(Sub);
		if (!R.bSuccess)
		{
			return FAxonActionResult::Error(FString::Printf(
...
FAxonActionResult FAxonMotionMatchingScaffoldActions::HandleAddEngineComponentTyped(const TSharedPtr<FJsonObject>& Params)
{
	const FString BpPath = Params->GetStringField(TEXT("bp_path"));
	...
	if (BpPath.IsEmpty())   return FAxonActionResult::Error(TEXT("Missing required parameter: bp_path"));
	if (CompType.IsEmpty()) return FAxonActionResult::Error(TEXT("Missing required parameter: component_type"));
	if (CompName.IsEmpty()) return FAxonActionResult::Error(TEXT("Missing required parameter: component_name"));
	...
	if (!CompClass)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Component class not found: %s"), *CompType));
	}
	...
	return FAxonBlueprintComponentActions::HandleAddComponent(Sub);
}
...
namespace
{
	...
	UObject* CreateInputAsset(UClass* AssetClass, const FString& PackagePath, FString& OutError)
	{
		...
		if (FAxonAssetUtils::AssetExists(PackagePath))
		{
...
FAxonActionResult FAxonMotionMatchingScaffoldActions::HandleScaffoldLocomotionInput(const TSharedPtr<FJsonObject>& Params)
{
	const FString BpPath = Params->GetStringField(TEXT("bp_path"));
	const FString ImcPath = Params->GetStringField(TEXT("imc_path"));

	if (BpPath.IsEmpty())  return FAxonActionResult::Error(TEXT("Missing required parameter: bp_path"));
	if (ImcPath.IsEmpty()) return FAxonActionResult::Error(TEXT("Missing required parameter: imc_path"));
	...
	{
		return FAxonActionResult::Error(TEXT("'actions' is required and must be a non-empty array of {name, value_type}"));
	}
	...
	if (!ImcObj)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("scaffold_locomotion_input: %s"), *CreateError));
	}
	...
	for (const TSharedPtr<FJsonValue>& Entry : *ActionsArr)
	{
		...
		if (!Obj.IsValid())
		{
			return FAxonActionResult::Error(TEXT("Each 'actions' entry must be an object {name, value_type}"));
		}
		FString ActName;
		Obj->TryGetStringField(TEXT("name"), ActName);
		if (ActName.IsEmpty())
		{
			return FAxonActionResult::Error(TEXT("Each 'actions' entry requires a non-empty 'name'"));
		}
		FString ValueType;
	...
	for (const FString& ActName : ActionNames)
	{
		...
		FAxonActionResult NR = FAxonBlueprintNodeActions::HandleAddNode(NodeSub);
		if (NR.bSuccess)
		{
			NodesAdded++;
		}
...
FAxonActionResult FAxonMotionMatchingScaffoldActions::HandleValidateAnimBpVariableContract(const TSharedPtr<FJsonObject>& Params)
{
...
FAxonActionResult FAxonMotionMatchingScaffoldActions::HandleScaffoldMotionMatchingCharacter(const TSharedPtr<FJsonObject>& Params)
{
	const FString BpPath = Params->GetStringField(TEXT("bp_path"));
	const FString AnimBpPath = Params->GetStringField(TEXT("anim_bp_path"));
	...
	if (!BP)
	{
		...
		FAxonActionResult CR = FAxonBlueprintCompileActions::HandleCreateBlueprint(CreateSub);
		if (!CR.bSuccess)
		{
			return FAxonActionResult::Error(FString::Printf(
		...
		BP = FAxonAssetUtils::LoadAssetByPath<UBlueprint>(BpPath);
		if (!BP)
		{
			return FAxonActionResult::Error(TEXT("scaffold_motion_matching_character: BP created but failed to reload"));
		}
	}
	else
	{
			...
			!CurrentParent.Equals(FString(TEXT("A")) + ParentClass, ESearchCase::IgnoreCase))
		{
			...
			FAxonActionResult RR = FAxonBlueprintGraphActions::HandleReparentBlueprint(ReparentSub);
	...
	if (!Mesh.IsEmpty())
	{
		...
		USkeletalMesh* MeshAsset = FAxonAssetUtils::LoadAssetByPath<USkeletalMesh>(Mesh);
		if (!MeshSMC)
		{
	...
	// --- Compose 5.1: set_anim_class on the 'Mesh' component ---
	{
		...
		FAxonActionResult AR = HandleSetAnimClass(AnimSub);
		if (!AR.bSuccess)
		{
			return FAxonActionResult::Error(FString::Printf(
	...
	// --- Compose 5.2: apply_movement_preset ---
	{
		...
		FAxonActionResult PR = HandleApplyMovementPreset(PresetSub);
		if (!PR.bSuccess)
		{
			return FAxonActionResult::Error(FString::Printf(
	...
	// --- Compile ---
	{
		...
		FAxonActionResult CR = FAxonBlueprintCompileActions::HandleCompileBlueprint(CompileSub);
...
FAxonActionResult FAxonMotionMatchingScaffoldActions::HandleGetInheritedComponentOverride(const TSharedPtr<FJsonObject>& Params)
{
	const FString BpPath = Params->GetStringField(TEXT("bp_path"));
	const FString CompName = Params->GetStringField(TEXT("component"));
	FString SingleProp;
...
namespace
{
	...
	bool EnsureThreadSafeUpdate(const FString& AbpPath, UAnimBlueprint* ABP, bool& bOutCreated, FString& OutError)
	{
		...
		FAxonActionResult R = FAxonBlueprintGraphActions::HandleOverrideParentFunction(Sub);
		if (!R.bSuccess)
		{
	 ...
	 */
	void ExtractBulkNodeIds(const FAxonActionResult& BulkResult, TMap<FString, FString>& OutMap)
	{
...
FAxonActionResult FAxonMotionMatchingScaffoldActions::HandleScaffoldThreadSafeUpdate(const TSharedPtr<FJsonObject>& Params)
{
	const FString AbpPath = Params->GetStringField(TEXT("abp_path"));
	if (AbpPath.IsEmpty()) return FAxonActionResult::Error(TEXT("Missing required parameter: abp_path"));
...
FAxonActionResult FAxonMotionMatchingScaffoldActions::HandleAddPawnOwnerAccess(const TSharedPtr<FJsonObject>& Params)
{
	const FString AbpPath = Params->GetStringField(TEXT("abp_path"));
	if (AbpPath.IsEmpty()) return FAxonActionResult::Error(TEXT("Missing required parameter: abp_path"));
	...
	FAxonActionResult NR = FAxonBlueprintNodeActions::HandleAddNodesBulk(NodesSub);
	if (!NR.bSuccess)
	{
		return FAxonActionResult::Error(FString::Printf(
	...
	if (!GetPawnNodeId.IsEmpty() && !CastNodeId.IsEmpty())
	{
		...
		FAxonBlueprintNodeActions::HandleConnectPinsBulk(ConnSub); // best-effort wire
	}
...
FAxonActionResult FAxonMotionMatchingScaffoldActions::HandleScaffoldLocomotionAnimValues(const TSharedPtr<FJsonObject>& Params)
{
	const FString AbpPath = Params->GetStringField(TEXT("abp_path"));
	if (AbpPath.IsEmpty()) return FAxonActionResult::Error(TEXT("Missing required parameter: abp_path"));
	...
	{
		if (!AxonBlueprintInternal::FindGraphByName(ABP, TargetGraphName))
		{
			return FAxonActionResult::Error(FString::Printf(
	...
	for (const FVarSpec& V : Vars)
	{
		...
		FAxonActionResult VR = FAxonBlueprintVariableActions::HandleAddVariable(VarSub);
	...
	auto AddPropAccessNode = [&](const TArray<FString>& APath) -> FString
	{
		...
		FAxonActionResult R = FAxonBlueprintNodeActions::HandleAddPropertyAccessNode(Sub);
	...
	FString EntryNodeId;
	if (UEdGraph* TsGraph = AxonBlueprintInternal::FindGraphByName(ABP, TargetGraphName))
	{
	...
	int32 Connected = 0;
	{
		...
		ConnSub->SetArrayField(TEXT("connections"), Conns);
		FAxonActionResult CR = FAxonBlueprintNodeActions::HandleConnectPinsBulk(ConnSub);
	...
	// Set the GroundSpeed threshold default on the compare's B pin (3.0 cm/s).
	{
		...
		FAxonBlueprintNodeActions::HandleSetPinDefault(DefSub); // best-effort
	}
...
FAxonActionResult FAxonMotionMatchingScaffoldActions::HandleApplyLocomotionSpeedBand(const TSharedPtr<FJsonObject>& Params)
{
	const FString BpPath = Params->GetStringField(TEXT("bp_path"));
	if (BpPath.IsEmpty()) return FAxonActionResult::Error(TEXT("Missing required parameter: bp_path"));
	...
	if (!Params->TryGetNumberField(TEXT("walk_speed"), WalkSpeed))
		return FAxonActionResult::Error(TEXT("Missing required parameter: walk_speed"));
	if (!Params->TryGetNumberField(TEXT("run_speed"), RunSpeed))
		return FAxonActionResult::Error(TEXT("Missing required parameter: run_speed"));
	if (!Params->TryGetNumberField(TEXT("crouch_speed"), CrouchSpeed))
		return FAxonActionResult::Error(TEXT("Missing required parameter: crouch_speed"));
	...
	if (!CmcClass)
	{
		return FAxonActionResult::Error(TEXT("apply_locomotion_speed_band: UCharacterMovementComponent class not found"));
	}
	UActorComponent* CmcComp = ResolveComponentOnBP(BP, FString(), CmcClass, {});
	if (!CmcComp)
	{
