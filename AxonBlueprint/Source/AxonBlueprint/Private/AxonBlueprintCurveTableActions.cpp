...
namespace AxonCurveTableInternal
{
	...
	{
		UCurveTable* CurveTable = FAxonAssetUtils::LoadAssetByPath<UCurveTable>(AssetPath);
		if (!CurveTable)
		{
...
void FAxonBlueprintCurveTableActions::RegisterActions(FAxonToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("read_curve_table"),
		...
		FAxonActionHandler::CreateStatic(&HandleReadCurveTable),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("set_curve_table_keys"),
		...
		FAxonActionHandler::CreateStatic(&HandleSetCurveTableKeys),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("add_curve_table_row"),
		...
		FAxonActionHandler::CreateStatic(&HandleAddCurveTableRow),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"),  TEXT("CurveTable asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("remove_curve_table_row"),
		...
		FAxonActionHandler::CreateStatic(&HandleRemoveCurveTableRow),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"),  TEXT("CurveTable asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("rename_curve_table_row"),
		...
		FAxonActionHandler::CreateStatic(&HandleRenameCurveTableRow),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"),  TEXT("CurveTable asset path"))
...
FAxonActionResult FAxonBlueprintCurveTableActions::HandleReadCurveTable(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonCurveTableInternal;
...
FAxonActionResult FAxonBlueprintCurveTableActions::HandleSetCurveTableKeys(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonCurveTableInternal;
	...
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: keys (array of {time, value})"));
	}

	FString Error;
	UCurveTable* CurveTable = ResolveCurveTable(AssetPath, Error);
	if (!CurveTable) { return FAxonActionResult::Error(Error); }
	...
	FScopedTransaction Transaction(NSLOCTEXT(
		"AxonBlueprintCurveTableActions", "SetCurveTableKeys", "Axon Set CurveTable Keys"));
	CurveTable->Modify();
...
FAxonActionResult FAxonBlueprintCurveTableActions::HandleAddCurveTableRow(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonCurveTableInternal;
	...
	CurveTable->SetFlags(RF_Transactional);
	FScopedTransaction Transaction(NSLOCTEXT(
		"AxonBlueprintCurveTableActions", "AddCurveTableRow", "Axon Add CurveTable Row"));
	CurveTable->Modify();
...
FAxonActionResult FAxonBlueprintCurveTableActions::HandleRemoveCurveTableRow(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonCurveTableInternal;
	...
	CurveTable->SetFlags(RF_Transactional);
	FScopedTransaction Transaction(NSLOCTEXT(
		"AxonBlueprintCurveTableActions", "RemoveCurveTableRow", "Axon Remove CurveTable Row"));
	CurveTable->Modify();
	FCurveTableEditorUtils::BroadcastPreChange(CurveTable, FCurveTableEditorUtils::ECurveTableChangeInfo::RowList);
...
FAxonActionResult FAxonBlueprintCurveTableActions::HandleRenameCurveTableRow(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonCurveTableInternal;
	...
	CurveTable->SetFlags(RF_Transactional);
	FScopedTransaction Transaction(NSLOCTEXT(
		"AxonBlueprintCurveTableActions", "RenameCurveTableRow", "Axon Rename CurveTable Row"));
	CurveTable->Modify();
	FCurveTableEditorUtils::BroadcastPreChange(CurveTable, FCurveTableEditorUtils::ECurveTableChangeInfo::RowList);
