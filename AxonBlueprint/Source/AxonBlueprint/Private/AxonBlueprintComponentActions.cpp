...
void FAxonBlueprintComponentActions::RegisterActions(FAxonToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("add_component"),
		...
		FAxonActionHandler::CreateStatic(&HandleAddComponent),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("remove_component"),
		...
		FAxonActionHandler::CreateStatic(&HandleRemoveComponent),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("rename_component"),
		...
		FAxonActionHandler::CreateStatic(&HandleRenameComponent),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("reparent_component"),
		...
		FAxonActionHandler::CreateStatic(&HandleReparentComponent),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("set_component_property"),
		...
		FAxonActionHandler::CreateStatic(&HandleSetComponentProperty),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("duplicate_component"),
		...
		FAxonActionHandler::CreateStatic(&HandleDuplicateComponent),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blueprint asset path"))
...
FAxonActionResult FAxonBlueprintComponentActions::HandleAddComponent(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	...
	{
		return FAxonActionResult::Error(TEXT("component_class is required"));
	}
	...
	if (!CompClass)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Component class not found: %s"), *ClassName));
	}
	if (!CompClass->IsChildOf(UActorComponent::StaticClass()))
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Class '%s' is not a UActorComponent"), *ClassName));
	}
	...
	if (!NewNode)
	{
		return FAxonActionResult::Error(TEXT("Failed to create SCS node"));
	}
	...
	if (!ParentName.IsEmpty())
	{
		...
		if (!ParentNode)
		{
			return FAxonActionResult::Error(FString::Printf(TEXT("Parent component not found: %s"), *ParentName));
		}
...
FAxonActionResult FAxonBlueprintComponentActions::HandleRemoveComponent(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = AxonBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	if (!BP->SimpleConstructionScript)
	{
		return FAxonActionResult::Error(TEXT("Blueprint has no SimpleConstructionScript"));
	}

	FString CompName = Params->GetStringField(TEXT("component_name"));
	if (CompName.IsEmpty())
	{
		return FAxonActionResult::Error(TEXT("component_name is required"));
	}

	USCS_Node* Node = FindSCSNodeByName(BP->SimpleConstructionScript, FName(*CompName));
	if (!Node)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Component not found: %s"), *CompName));
	}
...
FAxonActionResult FAxonBlueprintComponentActions::HandleRenameComponent(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = AxonBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	if (CompName.IsEmpty()) return FAxonActionResult::Error(TEXT("component_name is required"));
	if (NewName.IsEmpty())  return FAxonActionResult::Error(TEXT("new_name is required"));
	if (FName(*NewName).IsNone()) return FAxonActionResult::Error(TEXT("'None' is a reserved FName and cannot be used as a component name"));
	...
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("A component named '%s' already exists"), *NewName));
	}
	...
	for (const FBPVariableDescription& Var : BP->NewVariables)
	{
		...
		{
			return FAxonActionResult::Error(FString::Printf(TEXT("Cannot rename component to '%s': a variable with that name already exists"), *NewName));
		}
	}
	...
	for (UEdGraph* Graph : BP->FunctionGraphs)
	{
		...
		{
			return FAxonActionResult::Error(FString::Printf(TEXT("Cannot rename component to '%s': a function with that name already exists"), *NewName));
		}
	}
...
FAxonActionResult FAxonBlueprintComponentActions::HandleReparentComponent(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = AxonBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	if (!NewParent.IsEmpty())
	{
		...
		if (!NewParentNode)
		{
			return FAxonActionResult::Error(FString::Printf(TEXT("New parent component not found: %s"), *NewParent));
		}
		if (NewParentNode == Node)
		{
			return FAxonActionResult::Error(TEXT("Cannot reparent a component to itself"));
		}
		...
		{
			return FAxonActionResult::Error(TEXT("Cannot reparent component to one of its own descendants"));
		}
	}
...
FAxonActionResult FAxonBlueprintComponentActions::HandleSetComponentProperty(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = AxonBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	if (CompName.IsEmpty()) return FAxonActionResult::Error(TEXT("component_name is required"));
	if (PropName.IsEmpty()) return FAxonActionResult::Error(TEXT("property_name is required"));
	...
	if (ObjProp)
	{
		...
		if (!bIsNone)
		{
			...
			NewObject = FAxonAssetUtils::LoadAssetByPath(UObject::StaticClass(), Path);
			if (!NewObject)
			{
				return FAxonActionResult::Error(FString::Printf(
...
FAxonActionResult FAxonBlueprintComponentActions::HandleDuplicateComponent(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = AxonBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Component '%s' has no ComponentClass"), *CompName));
	}
	...
	if (!NewNode)
	{
		return FAxonActionResult::Error(TEXT("Failed to create duplicate SCS node"));
	}
