 ...
 */
class FAxonBlueprintContractActions
{
public:
	static void RegisterActions(FAxonToolRegistry& Registry);

private:
	static FAxonActionResult HandleCompareClassVariableContract(const TSharedPtr<FJsonObject>& Params);
	static FAxonActionResult HandlePromoteVariablesToParent(const TSharedPtr<FJsonObject>& Params);
};
