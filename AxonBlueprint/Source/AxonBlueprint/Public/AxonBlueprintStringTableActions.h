 ...
 */
class FAxonBlueprintStringTableActions
{
public:
	static void RegisterActions(FAxonToolRegistry& Registry);

	static FAxonActionResult HandleReadStringTable(const TSharedPtr<FJsonObject>& Params);
	static FAxonActionResult HandleSetStringTableEntries(const TSharedPtr<FJsonObject>& Params);
	static FAxonActionResult HandleRemoveStringTableEntry(const TSharedPtr<FJsonObject>& Params);
};
