...
#include "AxonCppParser.h"
#include "AxonShaderParser.h"
#include "AxonReferenceBuilder.h"
#include "HAL/RunnableThread.h"
#include "HAL/FileManager.h"
#include "Misc/Paths.h"
#include "Internationalization/Regex.h"

namespace AxonSourceIndexerDetail
{
...
FAxonSourceIndexer::FAxonSourceIndexer()
{
}
...
void FAxonSourceIndexer::SetSourcePath(const FString& InPath)   { SourcePath = InPath; }
void FAxonSourceIndexer::SetShaderPath(const FString& InPath)   { ShaderPath = InPath; }
void FAxonSourceIndexer::SetProjectPath(const FString& InPath)  { ProjectPath = InPath; }
void FAxonSourceIndexer::SetDatabasePath(const FString& InPath) { DbPath = InPath; }
void FAxonSourceIndexer::SetCleanBuild(bool bClean)             { bCleanBuild = bClean; }
void FAxonSourceIndexer::SetIndexProjectSource(bool bIndex)     { bIndexProjectSource = bIndex; }

// ============================================================
// Thread control
// ============================================================

bool FAxonSourceIndexer::StartAsync()
{
	if (bIsRunning) return false;
	Thread = FRunnableThread::Create(this, TEXT("AxonSourceIndexer"), 0, TPri_BelowNormal);
	return Thread != nullptr;
}

bool FAxonSourceIndexer::RunSynchronous()
{
	if (bIsRunning) return false;
	Init();
	Run();
...
void FAxonSourceIndexer::RequestStop()
{
	bShouldStop = true;
}

bool FAxonSourceIndexer::IsRunning() const
{
	return bIsRunning;
}

FSourceIndexDiagnostics FAxonSourceIndexer::GetDiagnostics() const
{
...
bool FAxonSourceIndexer::Init()
{
	return true;
}

void FAxonSourceIndexer::Stop()
{
	bShouldStop = true;
}

uint32 FAxonSourceIndexer::Run()
{
	bIsRunning = true;
	...
	{
		UE_LOG(LogAxonSource, Error, TEXT("Indexer: Failed to open DB for writing: %s"), *DbPath);
		bIsRunning = false;
		return 1;
	}
	...
	if (!SourcePath.IsEmpty())
	{
		...
		UE_LOG(LogAxonSource, Log, TEXT("Indexer: Found %d engine modules"), EngineModules.Num());
	...
	if (bIndexProjectSource && !ProjectPath.IsEmpty() && !bShouldStop)
	{
		...
		if (!bCleanBuild)
		{
			UE_LOG(LogAxonSource, Log, TEXT("Indexer: Loading existing symbols for incremental indexing..."));
		...
		UE_LOG(LogAxonSource, Log, TEXT("Indexer: Found %d project modules"), ProjectModules.Num());
	...
	UE_LOG(LogAxonSource, Log, TEXT("Indexer complete: %d files, %d symbols, %d errors"), Files, Symbols, Errors);
...
void FAxonSourceIndexer::DiscoverEngineModules(TArray<FModuleEntry>& OutModules)
{
		...
		IFileManager::Get().IterateDirectory(*CategoryDir, [&](const TCHAR* Path, bool bIsDir) -> bool
		{
			if (bIsDir)
			{
				...
				OutModules.Add({ FString(Path), DirName, Category,
					AxonSourceIndexerDetail::DeriveBuildCsPath(FString(Path), DirName) });
			}
		...
		if (bIsDir)
		{
			...
			if (DirName == TEXT("Source"))
			{
				...
				OutModules.Add({ FString(Path), ModuleName, TEXT("Plugin"),
					AxonSourceIndexerDetail::DeriveBuildCsPath(FString(Path), ModuleName) });
			}
		}
		return true;
	});
...
void FAxonSourceIndexer::DiscoverProjectModules(TArray<FModuleEntry>& OutModules)
{
		...
		if (bIsDir)
		{
			...
			if (DirName == TEXT("Source"))
			{
				...
				OutModules.Add({ FullPath, ModuleName, ModuleType,
					AxonSourceIndexerDetail::DeriveBuildCsPath(FullPath, ModuleName) });
			}
		}
		return true;
	});
...
void FAxonSourceIndexer::IndexModule(const FModuleEntry& Module, FAxonSourceDatabase& DB)
{
...
int32 FAxonSourceIndexer::IndexCppFile(const FString& FilePath, int64 ModuleId, FAxonSourceDatabase& DB)
{
	FAxonCppParser Parser;
...
int32 FAxonSourceIndexer::IndexShaderFile(const FString& FilePath, int64 ModuleId, FAxonSourceDatabase& DB)
{
	FAxonShaderParser Parser;
	FParsedFileResult ParseResult = Parser.ParseFile(FilePath);
...
void FAxonSourceIndexer::UpdateSymbolMap(const FString& Name, int64 SymId, int32 LineStart, int32 LineEnd)
{
...
void FAxonSourceIndexer::UpdateClassMap(const FString& Name, int64 SymId, int32 LineStart, int32 LineEnd)
{
	if (Name.StartsWith(TEXT("_bases_"))) return;
...
void FAxonSourceIndexer::Finalize(FAxonSourceDatabase& DB)
{
	UE_LOG(LogAxonSource, Log, TEXT("Indexer: Finalizing — resolving inheritance..."));
	...
	UE_LOG(LogAxonSource, Log, TEXT("Indexer: Extracting references from %d new files..."), NewFileIds.Num());

	FAxonReferenceBuilder RefBuilder(DB, SymbolNameToId);
	...
	for (int64 FileId : NewFileIds)
	{
		...
		if (FilesProcessed % 500 == 0)
		{
			DB.CommitTransaction();
			DB.BeginTransaction();
			UE_LOG(LogAxonSource, Log, TEXT("  References: %d files processed, %d refs found"), FilesProcessed, RefCount);
		}
	}
	DB.CommitTransaction();

	UE_LOG(LogAxonSource, Log, TEXT("Indexer: Reference extraction complete — %d refs from %d files"), RefCount, FilesProcessed);
