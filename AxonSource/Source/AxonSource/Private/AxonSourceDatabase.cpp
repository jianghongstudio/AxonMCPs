...
#include "AxonSourceSchema.h"
#include "SQLiteDatabase.h"
#include "Misc/Paths.h"
#include "HAL/PlatformFileManager.h"

DEFINE_LOG_CATEGORY(LogAxonSource);
...
FAxonSourceDatabase::FAxonSourceDatabase()
{
}
...
bool FAxonSourceDatabase::Open(const FString& DbPath)
{
	...
	{
		UE_LOG(LogAxonSource, Warning, TEXT("Engine source DB not found: %s"), *DbPath);
		return false;
	}
	...
	{
		UE_LOG(LogAxonSource, Error, TEXT("Failed to open engine source DB: %s"), *DbPath);
		delete Database;
		Database = nullptr;
		return false;
	}
	...
	// same file by AxonReflectionIntel: UE 5.7's custom `unreal-fs` SQLite VFS
	...
	{
		UE_LOG(LogAxonSource, Warning,
			TEXT("Open: PRAGMA journal_mode=DELETE failed on '%s' (continuing — hygiene only)"),
			*DbPath);
	}

	UE_LOG(LogAxonSource, Log, TEXT("Engine source DB opened: %s"), *DbPath);
	return true;
}

void FAxonSourceDatabase::Close()
{
	FScopeLock Lock(&DbLock);
	if (Database)
	{
...
bool FAxonSourceDatabase::IsOpen() const
{
	FScopeLock Lock(&DbLock);
	return Database != nullptr && Database->IsValid();
}

FSQLiteDatabase* FAxonSourceDatabase::GetRawHandle() const
{
	// Deliberately NOT locked here: the borrower (AxonReflectionIntel) holds
...
FString FAxonSourceDatabase::EscapeFTS(const FString& Query)
{
	// Replace :: with space
...
FAxonSourceSymbol FAxonSourceDatabase::ReadSymbolFromStatement(FSQLitePreparedStatement& Stmt)
{
	FAxonSourceSymbol Sym;
...
FAxonSourceReference FAxonSourceDatabase::ReadReferenceFromStatement(FSQLitePreparedStatement& Stmt, bool bIsRefTo)
{
	FAxonSourceReference Ref;
...
TArray<FAxonSourceSymbol> FAxonSourceDatabase::GetSymbolsByName(const FString& Name, const FString& Kind)
{
	FScopeLock Lock(&DbLock);
	TArray<FAxonSourceSymbol> Result;
...
TArray<FAxonSourceSymbol> FAxonSourceDatabase::SearchSymbolsFTS(const FString& Query, int32 Limit)
{
	FScopeLock Lock(&DbLock);
	TArray<FAxonSourceSymbol> Result;
	if (!Database || !Database->IsValid()) return Result;
...
TOptional<FAxonSourceSymbol> FAxonSourceDatabase::GetSymbolById(int64 Id)
{
	FScopeLock Lock(&DbLock);
...
FString FAxonSourceDatabase::GetFilePath(int64 FileId)
{
	FScopeLock Lock(&DbLock);
...
TOptional<FAxonSourceFile> FAxonSourceDatabase::FindFileBySuffix(const FString& Suffix)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return {};
...
bool FAxonSourceDatabase::GetFileModuleInfo(int64 FileId, FString& OutModuleName, FString& OutBuildCsPath)
{
	FScopeLock Lock(&DbLock);
	OutModuleName.Empty();
...
TOptional<FAxonSourceFile> FAxonSourceDatabase::FindFileByPath(const FString& Path)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return {};
...
TArray<FAxonSourceReference> FAxonSourceDatabase::GetReferencesTo(int64 SymbolId, const FString& RefKind, int32 Limit)
{
	FScopeLock Lock(&DbLock);
	TArray<FAxonSourceReference> Result;
	if (!Database || !Database->IsValid()) return Result;
...
TArray<FAxonSourceReference> FAxonSourceDatabase::GetReferencesFrom(int64 SymbolId, const FString& RefKind, int32 Limit)
{
	FScopeLock Lock(&DbLock);
	TArray<FAxonSourceReference> Result;
	if (!Database || !Database->IsValid()) return Result;
...
TArray<FAxonSourceInheritance> FAxonSourceDatabase::GetParents(int64 SymbolId)
{
	FScopeLock Lock(&DbLock);
	TArray<FAxonSourceInheritance> Result;
	if (!Database || !Database->IsValid()) return Result;
...
TArray<FAxonSourceInheritance> FAxonSourceDatabase::GetChildren(int64 SymbolId)
{
	FScopeLock Lock(&DbLock);
	TArray<FAxonSourceInheritance> Result;
	if (!Database || !Database->IsValid()) return Result;
...
TOptional<FAxonSourceModuleStats> FAxonSourceDatabase::GetModuleStats(const FString& ModuleName)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return {};
...
TArray<FAxonSourceSymbol> FAxonSourceDatabase::GetSymbolsInModule(const FString& ModuleName, const FString& Kind, int32 Limit)
{
	FScopeLock Lock(&DbLock);
	TArray<FAxonSourceSymbol> Result;
	if (!Database || !Database->IsValid()) return Result;
...
TArray<FAxonSourceChunk> FAxonSourceDatabase::SearchSourceFTS(const FString& Query, const FString& Scope, int32 Limit)
{
	FScopeLock Lock(&DbLock);
	TArray<FAxonSourceChunk> Result;
	if (!Database || !Database->IsValid()) return Result;
...
TArray<FAxonSourceChunk> FAxonSourceDatabase::SearchSourceFTSFiltered(const FString& Query, const FString& Scope, const FString& Module, const FString& PathFilter, int32 Limit)
{
	FScopeLock Lock(&DbLock);
	TArray<FAxonSourceChunk> Result;
	if (!Database || !Database->IsValid()) return Result;
...
TArray<FAxonSourceSymbol> FAxonSourceDatabase::SearchSymbolsFTSFiltered(const FString& Query, const FString& Kind, const FString& Module, const FString& PathFilter, int32 Limit)
{
	FScopeLock Lock(&DbLock);
	TArray<FAxonSourceSymbol> Result;
	if (!Database || !Database->IsValid()) return Result;
...
int32 FAxonSourceDatabase::CountSymbolsFTSFiltered(const FString& Query, const FString& Kind, const FString& Module, const FString& PathFilter)
{
	FScopeLock Lock(&DbLock);
...
int32 FAxonSourceDatabase::CountSourceFTSFiltered(const FString& Query, const FString& Scope, const FString& Module, const FString& PathFilter)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return 0;
...
bool FAxonSourceDatabase::OpenForWriting(const FString& DbPath)
{
	FScopeLock Lock(&DbLock);
	...
	{
		UE_LOG(LogAxonSource, Error, TEXT("OpenForWriting: failed to open/create DB: %s"), *DbPath);
		delete Database;
		Database = nullptr;
		return false;
	}
	...
	// (FAxonSourceDatabase::GetRawHandle) rather than opening its own. We keep
	...
	UE_LOG(LogAxonSource, Log, TEXT("Engine source DB opened for writing: %s"), *DbPath);
	return true;
}
...
bool FAxonSourceDatabase::CreateTablesIfNeeded()
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid())
	{
		UE_LOG(LogAxonSource, Error, TEXT("CreateTablesIfNeeded: DB not open"));
		return false;
	}

	if (!ExecuteMulti(*Database, AxonSourceSchema::DDL_Tables))
	{
		UE_LOG(LogAxonSource, Error, TEXT("CreateTablesIfNeeded: DDL_Tables failed — %s"), *Database->GetLastError());
		return false;
	}
	if (!ExecuteMulti(*Database, AxonSourceSchema::DDL_FTS))
	{
		UE_LOG(LogAxonSource, Error, TEXT("CreateTablesIfNeeded: DDL_FTS failed — %s"), *Database->GetLastError());
		return false;
	}
	if (!ExecuteMulti(*Database, AxonSourceSchema::DDL_Triggers))
	{
		UE_LOG(LogAxonSource, Error, TEXT("CreateTablesIfNeeded: DDL_Triggers failed — %s"), *Database->GetLastError());
		return false;
	}
	...
	MetaStmt.SetBindingValueByIndex(2, FString::FromInt(AxonSourceSchema::SchemaVersion));
	MetaStmt.Step();

	UE_LOG(LogAxonSource, Log, TEXT("Schema created/verified (version %d)"), AxonSourceSchema::SchemaVersion);
	return true;
}

bool FAxonSourceDatabase::ResetDatabase()
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid())
	{
		UE_LOG(LogAxonSource, Error, TEXT("ResetDatabase: DB not open"));
		return false;
	}

	if (!ExecuteMulti(*Database, AxonSourceSchema::DDL_Drop))
	{
		UE_LOG(LogAxonSource, Error, TEXT("ResetDatabase: drop failed — %s"), *Database->GetLastError());
		return false;
	}

	UE_LOG(LogAxonSource, Log, TEXT("ResetDatabase: all tables dropped, recreating schema"));

	// Execute DDL inline (we're already holding DbLock, can't call CreateTablesIfNeeded)
	if (!ExecuteMulti(*Database, AxonSourceSchema::DDL_Tables))
	{
		UE_LOG(LogAxonSource, Error, TEXT("ResetDatabase: DDL_Tables failed — %s"), *Database->GetLastError());
		return false;
	}
	if (!ExecuteMulti(*Database, AxonSourceSchema::DDL_FTS))
	{
		UE_LOG(LogAxonSource, Error, TEXT("ResetDatabase: DDL_FTS failed — %s"), *Database->GetLastError());
		return false;
	}
	if (!ExecuteMulti(*Database, AxonSourceSchema::DDL_Triggers))
	{
		UE_LOG(LogAxonSource, Error, TEXT("ResetDatabase: DDL_Triggers failed — %s"), *Database->GetLastError());
		return false;
	}
	...
	UE_LOG(LogAxonSource, Log, TEXT("ResetDatabase: schema recreated successfully"));
	return true;
}
...
bool FAxonSourceDatabase::BeginTransaction()
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return false;
...
bool FAxonSourceDatabase::CommitTransaction()
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return false;
...
bool FAxonSourceDatabase::RollbackTransaction()
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return false;
...
int64 FAxonSourceDatabase::InsertModule(const FString& Name, const FString& Path, const FString& ModuleType, const FString& BuildCsPath)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return 0;
	...
	UE_LOG(LogAxonSource, Warning, TEXT("InsertModule: could not retrieve id for '%s'"), *Name);
	return 0;
}

int64 FAxonSourceDatabase::InsertFile(const FString& FilePath, int64 ModuleId, const FString& FileType, int32 LineCount, double LastModified)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return 0;
	...
	UE_LOG(LogAxonSource, Warning, TEXT("InsertFile: could not retrieve id for '%s'"), *FilePath);
	return 0;
}

int64 FAxonSourceDatabase::InsertSymbol(
...
void FAxonSourceDatabase::InsertInheritance(int64 ChildId, int64 ParentId)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return;
...
void FAxonSourceDatabase::InsertReference(int64 FromSymbolId, int64 ToSymbolId, const FString& RefKind, int64 FileId, int32 Line)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return;
...
void FAxonSourceDatabase::InsertInclude(int64 FileId, const FString& IncludedPath, int32 Line)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return;
...
void FAxonSourceDatabase::InsertSourceChunks(int64 FileId, const TArray<FString>& Lines)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return;
...
void FAxonSourceDatabase::SetMeta(const FString& Key, const FString& Value)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return;
...
FString FAxonSourceDatabase::GetMeta(const FString& Key)
{
	FScopeLock Lock(&DbLock);
...
void FAxonSourceDatabase::InsertDeprecation(int64 SymbolId, const FString& SymbolName, const FString& Version, const FString& Message, const FString& Kind)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return;
...
TOptional<FAxonDeprecationRow> FAxonSourceDatabase::GetDeprecation(const FString& SymbolName)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return {};
...
TMap<FString, FAxonDeprecationRow> FAxonSourceDatabase::GetDeprecationsBatch(const TArray<FString>& SymbolNames)
{
	FScopeLock Lock(&DbLock);
	TMap<FString, FAxonDeprecationRow> Result;
	if (!Database || !Database->IsValid()) return Result;
...
int32 FAxonSourceDatabase::GetDeprecationCount()
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return 0;
...
int32 FAxonSourceDatabase::LoadExistingSymbols(
	...
	TMap<FString, TPair<int32,int32>>& OutClassSpans)
{
	...
	UE_LOG(LogAxonSource, Log, TEXT("LoadExistingSymbols: loaded %d symbols (%d classes/structs)"),
