...
FAxonReferenceBuilder::FAxonReferenceBuilder(FAxonSourceDatabase& InDB, const TMap<FString, int64>& InSymbolMap)
	: DB(InDB)
...
int32 FAxonReferenceBuilder::ExtractReferences(const FString& FilePath, int64 FileId)
{
	FString Source;
	if (!FFileHelper::LoadFileToString(Source, *FilePath))
	{
		UE_LOG(LogAxonSource, Warning, TEXT("ReferenceBuilder: failed to read %s"), *FilePath);
		return 0;
	}

	// Strip UE macros first, then comments/strings — same pipeline as the first-pass parser
	FString Clean = AxonUEPreprocessor::PreprocessSource(Source);
...
// as AxonUEPreprocessor::PreprocessSource().
//
// Handles:
...
FString FAxonReferenceBuilder::StripCommentsAndStrings(const FString& Source)
{
...
int32 FAxonReferenceBuilder::ExtractCallReferences(const TArray<FString>& Lines, int64 FileId)
{
...
int32 FAxonReferenceBuilder::ExtractTypeReferences(const TArray<FString>& Lines, int64 FileId)
{
...
int64 FAxonReferenceBuilder::ResolveSymbol(const FString& Name) const
{
	if (Name.IsEmpty())
	{
		return 0;
