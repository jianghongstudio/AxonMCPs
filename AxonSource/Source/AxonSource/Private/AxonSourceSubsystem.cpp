...
UAxonSourceSubsystem::~UAxonSourceSubsystem()
{
	delete Indexer;
	Indexer = nullptr;
}

void UAxonSourceSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	Database = MakeUnique<FAxonSourceDatabase>();
	...
	if (PlatformFile.FileExists(*DbPath))
	{
		...
		{
			UE_LOG(LogAxonSource, Log, TEXT("Engine source DB loaded from %s"), *DbPath);
		}
	}
	else
	{
		UE_LOG(LogAxonSource, Log, TEXT("Engine source DB not found at %s — run source.trigger_reindex to create it"), *DbPath);
	}
	...
	ReloadCompleteHandle = FCoreUObjectDelegates::ReloadCompleteDelegate.AddUObject(
		this, &UAxonSourceSubsystem::OnReloadComplete);
}

void UAxonSourceSubsystem::Deinitialize()
{
...
void UAxonSourceSubsystem::OnReloadComplete(EReloadCompleteReason Reason)
{
...
void UAxonSourceSubsystem::TriggerReindex()
{
	if (bIsIndexing)
	{
		UE_LOG(LogAxonSource, Warning, TEXT("Indexing already in progress"));
		return;
	}
	...
	bIsIndexing = true;
		...
		delete Indexer;
	Indexer = new FAxonSourceIndexer();
	...
	Indexer->OnComplete.AddLambda([this, DbPath](int32 Files, int32 Symbols, int32 Errors)
	{
		...
		{
			bIsIndexing = false;
			UE_LOG(LogAxonSource, Log, TEXT("C++ source indexing complete: %d files, %d symbols, %d errors"), Files, Symbols, Errors);
			ReopenDatabase(DbPath);
		});
	});

	UE_LOG(LogAxonSource, Log, TEXT("Starting full source indexing (engine + project) via C++ indexer"));
	Indexer->StartAsync();
}
...
void UAxonSourceSubsystem::TriggerProjectReindex()
{
	if (bIsIndexing)
	{
		UE_LOG(LogAxonSource, Warning, TEXT("Indexing already in progress"));
	...
	if (!PlatformFile.FileExists(*DbPath))
	{
		UE_LOG(LogAxonSource, Error, TEXT("EngineSource.db not found at %s — run full TriggerReindex() first"), *DbPath);
		return;
	}
	...
	Indexer->OnComplete.AddLambda([this, DbPath](int32 Files, int32 Symbols, int32 Errors)
	{
		...
		{
			bIsIndexing = false;
			UE_LOG(LogAxonSource, Log, TEXT("Project source indexing complete: %d files, %d symbols, %d errors"), Files, Symbols, Errors);
			ReopenDatabase(DbPath);
		});
	});

	UE_LOG(LogAxonSource, Log, TEXT("Starting project source indexing (incremental) via C++ indexer"));
	Indexer->StartAsync();
}
...
void UAxonSourceSubsystem::ReopenDatabase(const FString& DbPath)
{
	if (Database.IsValid())
	{
		IPlatformFile& PlatformFile = FPlatformFileManager::Get().GetPlatformFile();
		if (PlatformFile.FileExists(*DbPath))
		{
			if (Database->Open(DbPath))
			{
				UE_LOG(LogAxonSource, Log, TEXT("Engine source DB reopened: %s"), *DbPath);
			}
		}
	}
}

FString UAxonSourceSubsystem::GetDatabasePath() const
{
	...
	return FPaths::ProjectPluginsDir() / TEXT("AxonMCPs") / TEXT("AxonSource") / TEXT("Saved") / TEXT("EngineSource.db");
}

FString UAxonSourceSubsystem::GetEngineSourcePath() const
{
	const UAxonSettings* Settings = UAxonSettings::Get();
...
FString UAxonSourceSubsystem::GetEngineShaderPath() const
{
	return FPaths::ConvertRelativePathToFull(FPaths::EngineDir() / TEXT("Shaders"));
}

FString UAxonSourceSubsystem::GetProjectPath() const
{
