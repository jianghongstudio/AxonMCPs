...
class FAxonSourceIndexer : public FRunnable
{
public:
	FAxonSourceIndexer();
	~FAxonSourceIndexer();
...
private:
	...
	void IndexModule(const FModuleEntry& Module, FAxonSourceDatabase& DB);
	int32 IndexCppFile(const FString& FilePath, int64 ModuleId, FAxonSourceDatabase& DB);
	int32 IndexShaderFile(const FString& FilePath, int64 ModuleId, FAxonSourceDatabase& DB);
	void Finalize(FAxonSourceDatabase& DB);
