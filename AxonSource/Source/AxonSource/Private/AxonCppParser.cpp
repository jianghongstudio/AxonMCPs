...
FParsedFileResult FAxonCppParser::ParseFile(const FString& FilePath)
{
	...
	{
		UE_LOG(LogAxonSource, Warning, TEXT("CppParser: Failed to read file: %s"), *FilePath);
		return Result;
	}
	...
	FString CleanSource = AxonUEPreprocessor::PreprocessSource(Source);
...
void FAxonCppParser::ExtractIncludes(const FString& Source, FParsedFileResult& Result)
{
...
void FAxonCppParser::ExtractClassesAndStructs(const TArray<FString>& OrigLines, const TArray<FString>& CleanLines, FParsedFileResult& Result)
{
...
void FAxonCppParser::ExtractEnums(const TArray<FString>& OrigLines, const TArray<FString>& CleanLines, FParsedFileResult& Result)
{
	const int32 NumLines = CleanLines.Num();
	TSet<FString> FoundNames;
...
void FAxonCppParser::ExtractFreeFunctions(const TArray<FString>& OrigLines, const TArray<FString>& CleanLines, FParsedFileResult& Result)
{
	const int32 NumLines = CleanLines.Num();
...
void FAxonCppParser::ExtractMacrosAndTypedefs(const TArray<FString>& OrigLines, FParsedFileResult& Result)
{
...
void FAxonCppParser::ExtractMembers(const TArray<FString>& OrigLines, int32 BodyStartLine, int32 BodyEndLine,
...
FString FAxonCppParser::ExtractDocstring(const TArray<FString>& Lines, int32 TargetLine, bool bSkipUEMacro)
{
...
int32 FAxonCppParser::FindClosingBrace(const TArray<FString>& Lines, int32 OpenBraceLine)
{
...
bool FAxonCppParser::IsUEMacroLine(const FString& Line)
{
	FString Trimmed = Line.TrimStartAndEnd();
...
void FAxonCppParser::ParseBaseClasses(const FString& InheritanceClause, TArray<FString>& OutBases)
{
