...
#include "AxonSourceDatabase.h"
#include "AxonSourceSubsystem.h"
#include "AxonToolRegistry.h"
#include "AxonParamSchema.h"
#include "AxonJsonUtils.h"
#include "AxonCursorCodec.h"
...
void FAxonSourceActions::RegisterAll()
{
	...
	Registry.RegisterAction(TEXT("source"), TEXT("read_source"),
		...
		FAxonActionHandler::CreateStatic(&FAxonSourceActions::HandleReadSource),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("find_references"),
		...
		FAxonActionHandler::CreateStatic(&FAxonSourceActions::HandleFindReferences),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("find_callers"),
		...
		FAxonActionHandler::CreateStatic(&FAxonSourceActions::HandleFindCallers),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("find_callees"),
		...
		FAxonActionHandler::CreateStatic(&FAxonSourceActions::HandleFindCallees),
		FParamSchemaBuilder()
			.Required(TEXT("symbol"), TEXT("string"), TEXT("Function name"))
			.Optional(TEXT("limit"), TEXT("integer"), TEXT("Max results"), TEXT("50"))
			.Build());
	...
	Registry.RegisterAction(TEXT("source"), TEXT("search_source"),
		...
		FAxonActionHandler::CreateStatic(&FAxonSourceActions::HandleSearchSource),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("get_class_hierarchy"),
		...
		FAxonActionHandler::CreateStatic(&FAxonSourceActions::HandleGetClassHierarchy),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("get_module_info"),
		...
		FAxonActionHandler::CreateStatic(&FAxonSourceActions::HandleGetModuleInfo),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("get_symbol_context"),
		...
		FAxonActionHandler::CreateStatic(&FAxonSourceActions::HandleGetSymbolContext),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("read_file"),
		...
		FAxonActionHandler::CreateStatic(&FAxonSourceActions::HandleReadFile),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("trigger_reindex"),
		...
		FAxonActionHandler::CreateStatic(&FAxonSourceActions::HandleTriggerReindex),
	...
	Registry.RegisterAction(TEXT("source"), TEXT("trigger_project_reindex"),
		...
		FAxonActionHandler::CreateStatic(&FAxonSourceActions::HandleTriggerProjectReindex),
		MakeShared<FJsonObject>());
	...
	Registry.RegisterAction(TEXT("source"), TEXT("get_include_path"),
		...
		FAxonActionHandler::CreateStatic(&FAxonSourceActions::HandleGetIncludePath),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("get_signature"),
		...
		FAxonActionHandler::CreateStatic(&FAxonSourceActions::HandleGetSignature),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("check_deprecations"),
		...
		FAxonActionHandler::CreateStatic(&FAxonSourceActions::HandleCheckDeprecations),
		FParamSchemaBuilder()
	...
	FAxonDispatcherAnnotations SourceAnnotations;
	...
	Registry.RegisterAction(TEXT("source"), TEXT("verify_symbols"),
		...
		FAxonActionHandler::CreateStatic(&FAxonSourceActions::HandleVerifySymbols),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("find_example_usage"),
		...
		FAxonActionHandler::CreateStatic(&FAxonSourceActions::HandleFindExampleUsage),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("lint_header"),
		...
		FAxonActionHandler::CreateStatic(&FAxonSourceActions::HandleLintHeader),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("generate_class_stub"),
		...
		FAxonActionHandler::CreateStatic(&FAxonSourceActions::HandleGenerateClassStub),
		FParamSchemaBuilder()
...
FAxonSourceDatabase* FAxonSourceActions::GetDB()
{
	if (!GEditor) return nullptr;
	UAxonSourceSubsystem* Subsystem = Cast<UAxonSourceSubsystem>(GEditor->GetEditorSubsystemBase(UAxonSourceSubsystem::StaticClass()));
	if (!Subsystem) return nullptr;
	return Subsystem->GetDatabase();
}

FString FAxonSourceActions::ShortPath(const FString& FullPath)
{
...
FString FAxonSourceActions::DeriveIncludePath(const FString& IndexedFilePath, bool& bOutIncludable, FString& OutWarning)
{
	bOutIncludable = true;
	OutWarning.Empty();
...
bool FAxonSourceActions::ResolveOwningModule(FAxonSourceDatabase* DB, const FString& Symbol, FString& OutModule, FString& OutBuildCsNote)
{
	OutModule.Empty();
	...
	TArray<FAxonSourceSymbol> Symbols = DB->GetSymbolsByName(LookupName);
...
bool FAxonSourceActions::ResolveIncludeForSymbol(FAxonSourceDatabase* DB, const FString& Symbol,
	FString& OutInclude, bool& OutIncludable, FString& OutModule, FString& OutWarning)
{
	...
	const FAxonSourceSymbol* Chosen = &Symbols[0];
	for (const FAxonSourceSymbol& S : Symbols)
	{
...
bool FAxonSourceActions::ResolveFirstSignature(FAxonSourceDatabase* DB, const FString& Symbol,
	FString& OutSignature, FString& OutSource)
{
	...
	TArray<FAxonSourceSymbol> Symbols = DB->GetSymbolsByName(MethodName, TEXT("function"));
	for (const FAxonSourceSymbol& S : Symbols)
	{
	...
	TArray<FAxonSourceChunk> Chunks = DB->SearchSourceFTS(Symbol, TEXT("all"), 50);
	for (const FAxonSourceChunk& Chunk : Chunks)
	{
...
bool FAxonSourceActions::SymbolExists(FAxonSourceDatabase* DB, const FString& Symbol)
{
	if (!DB) return false;
	...
	const FString NeedlePattern = MethodName + TEXT("(");
	TArray<FAxonSourceChunk> Chunks = DB->SearchSourceFTS(Symbol, TEXT("all"), 25);
	for (const FAxonSourceChunk& Chunk : Chunks)
	{
		const FString FilePath = DB->GetFilePath(Chunk.FileId);
		TArray<FString> FileLines;
...
FString FAxonSourceActions::ReadFileLines(const FString& FilePath, int32 StartLine, int32 EndLine)
{
...
bool FAxonSourceActions::IsForwardDeclaration(const FString& FilePath, int32 LineStart, int32 LineEnd)
{
...
FString FAxonSourceActions::ExtractMembers(const FString& FilePath, int32 StartLine, int32 EndLine)
{
	TArray<FString> Lines;
	if (!FFileHelper::LoadFileToStringArray(Lines, *FilePath))
	{
...
FString FAxonSourceActions::MakeTextResult(const FString& Text)
{
	// Return text as a JSON result with a "text" field
	// (But the registry expects FAxonActionResult with a JSON object)
...
FAxonActionResult FAxonSourceActions::HandleReadSource(const TSharedPtr<FJsonObject>& Params)
{
	FAxonSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
		return FAxonActionResult::Error(TEXT("Engine source DB not available. Run source.trigger_reindex first."));
	}
	...
	TArray<FAxonSourceSymbol> Symbols = DB->GetSymbolsByName(Symbol);
	if (Symbols.Num() == 0)
	{
	...
	if (Symbols.Num() == 0)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("No symbol found matching '%s'."), *Symbol));
	}
	...
	if (bHasDefinition)
	{
		TArray<FAxonSourceSymbol> Filtered;
		for (const auto& Sym : Symbols)
		{
	...
	ContentArr.Add(MakeShared<FJsonValueObject>(ContentItem));
	ResultObj->SetArrayField(TEXT("content"), ContentArr);
	return FAxonActionResult::Success(ResultObj);
}
...
FAxonActionResult FAxonSourceActions::HandleFindReferences(const TSharedPtr<FJsonObject>& Params)
{
	FAxonSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
		return FAxonActionResult::Error(TEXT("Engine source DB not available."));
	}
	...
	for (const auto& Sym : Symbols)
	{
		TArray<FAxonSourceReference> Refs = DB->GetReferencesTo(Sym.Id, RefKind, Limit);
...
FAxonActionResult FAxonSourceActions::HandleFindCallers(const TSharedPtr<FJsonObject>& Params)
{
	FAxonSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
	...
	TArray<FAxonSourceSymbol> Symbols = DB->GetSymbolsByName(Function, TEXT("function"));
	if (Symbols.Num() == 0)
	{
		TArray<FAxonSourceSymbol> AllSyms = DB->SearchSymbolsFTS(Function, 5);
	...
	if (Symbols.Num() == 0)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("No function found matching '%s'."), *Function));
	}

	TArray<FString> Lines;
	for (const auto& Sym : Symbols)
	{
		TArray<FAxonSourceReference> Refs = DB->GetReferencesTo(Sym.Id, TEXT("call"), Limit);
		for (const auto& Ref : Refs)
		{
...
FAxonActionResult FAxonSourceActions::HandleFindCallees(const TSharedPtr<FJsonObject>& Params)
{
	FAxonSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
	...
	for (const auto& Sym : Symbols)
	{
		TArray<FAxonSourceReference> Refs = DB->GetReferencesFrom(Sym.Id, TEXT("call"), Limit);
		for (const auto& Ref : Refs)
		{
...
FAxonActionResult FAxonSourceActions::HandleSearchSource(const TSharedPtr<FJsonObject>& Params)
{
	...
	const uint32 CurrentHash = AxonCursorCodec::ComputeQueryHash(
	...
	if (!CursorIn.IsEmpty())
	{
		AxonCursorCodec::FCursorState State;
		if (!AxonCursorCodec::Decode(CursorIn, State))
		{
	...
	TArray<FAxonSourceSymbol> SymResultsAll;
	if (!bPastCap)
	{
	...
	if (SymRowsThisPage > 0)
	{
		...
		{
			const FAxonSourceSymbol& Sym = SymResultsAll[i];
			FString FilePath = DB->GetFilePath(Sym.FileId);
	...
	TArray<FAxonSourceChunk> SourceMergedDeduped;
	if (!bPastCap)
	{
		TSet<FString> Seen;
		for (const FString& S : Scopes)
		{
			TArray<FAxonSourceChunk> ScopeBatch = DB->SearchSourceFTSFiltered(Query, S, Module, PathFilter, N);
			for (const FAxonSourceChunk& Match : ScopeBatch)
			{
	...
	if (SrcRowsThisPage > 0)
	{
		...
		{
			const FAxonSourceChunk& Match = SourceMergedDeduped[i];
	...
	{
		AxonCursorCodec::FCursorState OutState;
		...
		ResultObj->SetStringField(TEXT("next_cursor"), AxonCursorCodec::Encode(OutState));
	}
...
void FAxonSourceActions::WalkAncestors(FAxonSourceDatabase* DB, int64 SymId, TArray<FString>& Lines, int32 Indent, int32 MaxDepth, FHierarchyCounter& Counter, TSet<int64>& Visited)
{
	if (Indent > MaxDepth || Visited.Contains(SymId)) return;
	Visited.Add(SymId);

	TArray<FAxonSourceInheritance> Parents = DB->GetParents(SymId);
...
void FAxonSourceActions::WalkDescendants(FAxonSourceDatabase* DB, int64 SymId, TArray<FString>& Lines, int32 Indent, int32 MaxDepth, FHierarchyCounter& Counter, TSet<int64>& Visited)
{
	if (Indent > MaxDepth || Visited.Contains(SymId)) return;
	Visited.Add(SymId);

	TArray<FAxonSourceInheritance> Children = DB->GetChildren(SymId);
...
FAxonActionResult FAxonSourceActions::HandleGetClassHierarchy(const TSharedPtr<FJsonObject>& Params)
{
	FAxonSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
	...
	TArray<FAxonSourceSymbol> Symbols = DB->GetSymbolsByName(ClassName, TEXT("class"));
	if (Symbols.Num() == 0) Symbols = DB->GetSymbolsByName(ClassName, TEXT("struct"));
	if (Symbols.Num() == 0)
	{
		TArray<FAxonSourceSymbol> AllSyms = DB->SearchSymbolsFTS(ClassName, 5);
		for (const auto& S : AllSyms)
		{
	...
	if (Symbols.Num() == 0)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("No class or struct found matching '%s'."), *ClassName));
	}
	...
	const FAxonSourceSymbol& Sym = Symbols[0];
	FString FilePath = DB->GetFilePath(Sym.FileId);
	TArray<FString> Lines;
...
FAxonActionResult FAxonSourceActions::HandleGetModuleInfo(const TSharedPtr<FJsonObject>& Params)
{
	FAxonSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
	...
	TOptional<FAxonSourceModuleStats> Stats = DB->GetModuleStats(ModuleName);
	if (!Stats.IsSet())
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("No module found matching '%s'."), *ModuleName));
	}

	const FAxonSourceModuleStats& S = Stats.GetValue();
	TArray<FString> Lines;
	...
	// Show key classes
	TArray<FAxonSourceSymbol> KeyClasses = DB->GetSymbolsInModule(ModuleName, TEXT("class"), 20);
	if (KeyClasses.Num() > 0)
	{
		Lines.Add(TEXT(""));
...
FAxonActionResult FAxonSourceActions::HandleGetSymbolContext(const TSharedPtr<FJsonObject>& Params)
{
	FAxonSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
...
FAxonActionResult FAxonSourceActions::HandleReadFile(const TSharedPtr<FJsonObject>& Params)
{
	FAxonSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
	...
	else
	{
		...
		TOptional<FAxonSourceFile> F = DB->FindFileByPath(NormalizedPath);
		if (F.IsSet())
		{
			ResolvedPath = F->Path;
		}
	...
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("No file found matching '%s'."), *Path));
	}
...
FAxonActionResult FAxonSourceActions::HandleTriggerReindex(const TSharedPtr<FJsonObject>& Params)
{
	if (!GEditor)
	{
		return FAxonActionResult::Error(TEXT("Editor not available."));
	}

	UAxonSourceSubsystem* Subsystem = Cast<UAxonSourceSubsystem>(GEditor->GetEditorSubsystemBase(UAxonSourceSubsystem::StaticClass()));
	if (!Subsystem)
	{
		return FAxonActionResult::Error(TEXT("AxonSourceSubsystem not available."));
	}

	if (Subsystem->IsIndexing())
	{
		return FAxonActionResult::Error(TEXT("Indexing already in progress."));
	}
...
FAxonActionResult FAxonSourceActions::HandleTriggerProjectReindex(const TSharedPtr<FJsonObject>& Params)
{
	if (!GEditor)
	{
		return FAxonActionResult::Error(TEXT("Editor not available."));
...
FAxonActionResult FAxonSourceActions::HandleGetIncludePath(const TSharedPtr<FJsonObject>& Params)
{
	FAxonSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
	...
	if (Symbol.IsEmpty())
	{
		return FAxonActionResult::Error(TEXT("'symbol' is required."));
	}
...
FString FAxonSourceActions::CompactDeclaration(const TArray<FString>& Lines, int32 StartIdx)
{
...
FAxonActionResult FAxonSourceActions::HandleGetSignature(const TSharedPtr<FJsonObject>& Params)
{
	FAxonSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
	...
	if (Overloads.Num() == 0)
	{
		...
		TArray<FAxonSourceChunk> Chunks = DB->SearchSourceFTS(FtsQuery, TEXT("all"), 50);
	...
	if (Overloads.Num() == 0)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("No signature found for '%s'."), *Symbol));
	}
...
FAxonActionResult FAxonSourceActions::HandleCheckDeprecations(const TSharedPtr<FJsonObject>& Params)
{
	FAxonSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
	...
	{
		return FAxonActionResult::Error(TEXT("'symbols' must be a non-empty array of symbol names."));
	}
	...
	TMap<FString, FAxonDeprecationRow> Deprecated = DB->GetDeprecationsBatch(SymbolNames);
	...
	for (const FString& Name : SymbolNames)
	{
		...
		const FAxonDeprecationRow* Found = Deprecated.Find(Name);
		if (Found)
		{
...
FAxonActionResult FAxonSourceActions::HandleVerifySymbols(const TSharedPtr<FJsonObject>& Params)
{
	FAxonSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
	...
	TMap<FString, FAxonDeprecationRow> Deprecated;
	...
	for (const FString& Symbol : SymbolNames)
	{
		...
		else if (const FAxonDeprecationRow* Dep = Deprecated.Find(MethodName))
		{
...
// cursor-paginate via AxonCursorCodec + the rerun-slice scheme (the same
// moving-FTS-index rationale as search_source).
// ============================================================================

FAxonActionResult FAxonSourceActions::HandleFindExampleUsage(const TSharedPtr<FJsonObject>& Params)
{
	FAxonSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
	...
	{
		AxonCursorCodec::FCursorState State;
		if (!AxonCursorCodec::Decode(CursorIn, State) || State.QueryHash != CurrentHash)
		{
			TSharedPtr<FJsonObject> ErrData = MakeShared<FJsonObject>();
			ErrData->SetStringField(TEXT("error_code"), TEXT("INVALID_CURSOR"));
			return FAxonActionResult::Error(
	...
	TArray<FAxonSourceChunk> Chunks = DB->SearchSourceFTS(Symbol, TEXT("all"), FTS_FETCH);
	for (const FAxonSourceChunk& Chunk : Chunks)
	{
		const FString FilePath = DB->GetFilePath(Chunk.FileId);
		TArray<FString> FileLines;
...
TArray<FAxonSourceActions::FLintFinding> FAxonSourceActions::LintHeaderLines(
	const FString& FilePath, const TArray<FString>& Lines, const TSet<FString>& ValidSpecifiers)
{
	using namespace AxonLintDetail;
...
FAxonActionResult FAxonSourceActions::HandleLintHeader(const TSharedPtr<FJsonObject>& Params)
{
	const FString FilePath = Params->GetStringField(TEXT("file_path"));
	if (FilePath.IsEmpty())
	{
		return FAxonActionResult::Error(TEXT("'file_path' is required."));
	}

	TArray<FString> Lines;
	if (!FFileHelper::LoadFileToStringArray(Lines, *FilePath))
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Could not read header file: %s"), *FilePath));
	}
	...
	TSet<FString> ValidSpecifiers;
	{
		...
		if (Registry.HasAction(TEXT("source"), TEXT("list_class_specifiers")))
		{
			...
			FAxonActionResult SpecRes = Registry.ExecuteAction(TEXT("source"), TEXT("list_class_specifiers"), SpecParams);
...
void FAxonSourceActions::GenerateClassStubText(
...
FAxonActionResult FAxonSourceActions::HandleGenerateClassStub(const TSharedPtr<FJsonObject>& Params)
{
	FAxonSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
	...
	{
		return FAxonActionResult::Error(TEXT("'parent', 'class_name', and 'module' are all required."));
	}

	// Resolve the parent's class row. UCLASS-derived parents ONLY (v1).
	TArray<FAxonSourceSymbol> ParentRows = DB->GetSymbolsByName(ParentClass);
	...
	const FAxonSourceSymbol* ParentSym = nullptr;
	for (const FAxonSourceSymbol& S : ParentRows)
	{
	...
	const FAxonSourceSymbol* HeaderSym = ParentSym;
	for (const FAxonSourceSymbol& S : ParentRows)
	{
	...
	{
		TArray<FAxonSourceSymbol> CtorRows = DB->GetSymbolsByName(ParentClass, TEXT("function"));
		bool bSawAnyCtor = false;
		bool bSawPlainCtor = false;
		bool bSawObjInitCtor = false;
		for (const FAxonSourceSymbol& S : CtorRows)
		{
			if (S.Signature.IsEmpty()) continue;
