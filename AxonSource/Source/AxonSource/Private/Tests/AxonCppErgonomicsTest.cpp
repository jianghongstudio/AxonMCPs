...
// Plan: Plugins/Axon/Docs/plans/2026-06-10-llm-cpp-ergonomics-actions.md (§12 Phase 1).
//
// Tests use disposable SQLite DBs at FPaths::AutomationTransientDir(), never the
// real EngineSource.db. Fixtures live under
// Source/AxonSource/Private/Tests/Fixtures/CppErgoCorpus/.
//
...
namespace AxonCppErgoTestDetail
{
	/** Resolve the fixture corpus dir relative to the Axon plugin install. */
	static FString GetFixtureCorpusDir()
	{
		TSharedPtr<IPlugin> Plugin = IPluginManager::Get().FindPlugin(TEXT("AxonSource"));
		if (Plugin.IsValid())
		{
			return Plugin->GetBaseDir()
				/ TEXT("Source") / TEXT("AxonSource")
				/ TEXT("Private") / TEXT("Tests") / TEXT("Fixtures") / TEXT("CppErgoCorpus");
		}
		return FPaths::ProjectPluginsDir()
			/ TEXT("Axon") / TEXT("Source") / TEXT("AxonSource")
			/ TEXT("Private") / TEXT("Tests") / TEXT("Fixtures") / TEXT("CppErgoCorpus");
	}
...
bool FCppErgoDeprecationSchemaBootstrapTest::RunTest(const FString& /*Parameters*/)
{
	...
	const FString DbPath = MakeTempDbPath();
	{
		...
		TOptional<FAxonDeprecationRow> Got = DB.GetDeprecation(TEXT("Foo"));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"Axon.Source.CppErgonomics.DeprecationIndexExtraction",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCppErgoDeprecationIndexExtractionTest::RunTest(const FString& /*Parameters*/)
{
	...
	TOptional<FAxonDeprecationRow> Foo = DB.GetDeprecation(TEXT("Foo"));
	...
	TOptional<FAxonDeprecationRow> Baz = DB.GetDeprecation(TEXT("Baz"));
	...
	// AxonCppParser now indexes such plain classes and ExtractMembers extracts the
	// class-body methods Foo/Baz as `function` symbol rows. The deprecation extractor
	// resolves symbol_id by EXACT NAME (AxonSourceIndexer SymbolNameToId.Find), so
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"Axon.Source.CppErgonomics.IncludePathDerivation",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCppErgoIncludePathDerivationTest::RunTest(const FString& /*Parameters*/)
{
	...
	// Public/ -> strip, includable.
	{
		...
		const FString Out = FAxonSourceActions::DeriveIncludePath(In, bIncludable, Warning);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"Axon.Source.CppErgonomics.SignatureCompaction",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCppErgoSignatureCompactionTest::RunTest(const FString& /*Parameters*/)
{
	...
	{
		const FString Sig = FAxonSourceActions::CompactDeclaration(Lines, MultiIdx);
	...
	{
		const FString Sig = FAxonSourceActions::CompactDeclaration(Lines, InlineIdx);
	...
	{
		const FString Sig = FAxonSourceActions::CompactDeclaration(Lines, MacroIdx);
...
// extension so UBT's `*.cpp` glob never compiles them into AxonSource (a
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"Axon.Source.CppErgonomics.VerifySymbolsComposition",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCppErgoVerifySymbolsCompositionTest::RunTest(const FString& /*Parameters*/)
{
	...
	TestTrue(TEXT("class-body method exists:true"),
		FAxonSourceActions::SymbolExists(&DB, TEXT("UCppErgoUsage::CallMe")));

	// Owning class itself exists.
	TestTrue(TEXT("class exists:true"),
		FAxonSourceActions::SymbolExists(&DB, TEXT("UCppErgoUsage")));

	// A symbol absent from the corpus reports exists:false (no error).
	TestFalse(TEXT("missing symbol exists:false"),
		FAxonSourceActions::SymbolExists(&DB, TEXT("UThisDoesNotExistAnywhere::Nope")));

	// Signature resolution for the class-body method comes from declaration_read.
	{
		FString Sig, Source;
		const bool bOk = FAxonSourceActions::ResolveFirstSignature(&DB, TEXT("UCppErgoUsage::CallMe"), Sig, Source);
	...
	{
		TOptional<FAxonDeprecationRow> Dep = DB.GetDeprecation(TEXT("Foo"));
...
// distinct call-site lines for `CallMe(`; the rerun-slice + AxonCursorCodec
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"Axon.Source.CppErgonomics.FindExampleUsagePagination",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCppErgoFindExampleUsagePaginationTest::RunTest(const FString& /*Parameters*/)
{
	...
	TArray<FAxonSourceChunk> Chunks = DB.SearchSourceFTS(Symbol, TEXT("all"), 400);
	...
	const int32 Limit = 5;
	const uint32 QHash = AxonCursorCodec::ComputeQueryHash(
	...
	if (P0Rows >= Limit && (P0Start + Limit) < Total)
	{
		...
		NextCursor = AxonCursorCodec::Encode(OutState);
	}
	TestFalse(TEXT("page 0 emits next_cursor"), NextCursor.IsEmpty());

	// Decode the cursor and fetch page 1.
	AxonCursorCodec::FCursorState Decoded;
	const bool bDecoded = AxonCursorCodec::Decode(NextCursor, Decoded);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"Axon.Source.CppErgonomics.LintHeaderRuleTable",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCppErgoLintHeaderRuleTableTest::RunTest(const FString& /*Parameters*/)
{
	...
	auto LintFixture = [&](const FString& FixtureName, const FString& RealHeaderName,
		const TSet<FString>& Specs) -> TArray<FAxonSourceActions::FLintFinding>
	{
		...
		return FAxonSourceActions::LintHeaderLines(Path, Lines, Specs);
	};

	auto HasRule = [](const TArray<FAxonSourceActions::FLintFinding>& F, const TCHAR* RuleId) -> bool
	{
		for (const FAxonSourceActions::FLintFinding& Fi : F) { if (Fi.RuleId == RuleId) return true; }
		return false;
	};
	...
	// missing ones.
	auto TestExactRules = [&](const TCHAR* Label, const TArray<FAxonSourceActions::FLintFinding>& F,
		const TArray<FString>& Expected)
	{
		TSet<FString> Got;
		for (const FAxonSourceActions::FLintFinding& Fi : F) { Got.Add(Fi.RuleId); }
		TSet<FString> Want;
	...
	{
		TArray<FAxonSourceActions::FLintFinding> F = LintFixture(TEXT("LintClean.h.fixture"), TEXT("LintClean.h"), NoSpecs);
		TestEqual(TEXT("clean header zero findings"), F.Num(), 0);
	}

	// (a) missing GENERATED_BODY — EXACTLY {missing_generated_body}.
	{
		TArray<FAxonSourceActions::FLintFinding> F = LintFixture(TEXT("LintMissingGeneratedBody.h.fixture"), TEXT("LintMissingGeneratedBody.h"), NoSpecs);
		TestExactRules(TEXT("(a) LintMissingGeneratedBody exact rules"), F, { TEXT("missing_generated_body") });
		// Each finding carries a message.
		for (const FAxonSourceActions::FLintFinding& Fi : F)
		{
	...
	{
		TArray<FAxonSourceActions::FLintFinding> F = LintFixture(TEXT("LintGeneratedNotLast.h.fixture"), TEXT("LintGeneratedNotLast.h"), NoSpecs);
	...
	{
		TArray<FAxonSourceActions::FLintFinding> F = LintFixture(TEXT("LintNameMismatch.h.fixture"), TEXT("LintNameMismatch.h"), NoSpecs);
	...
	{
		TArray<FAxonSourceActions::FLintFinding> F = LintFixture(TEXT("LintMissingApiMacro.h.fixture"), TEXT("LintMissingApiMacro.h"), NoSpecs);
	...
	{
		TArray<FAxonSourceActions::FLintFinding> F = LintFixture(TEXT("LintOrphanUproperty.h.fixture"), TEXT("LintOrphanUproperty.h"), NoSpecs);
	...
	// instead lint an in-memory header carrying UCLASS(NotARealSpecifier).
	{
		...
		TArray<FAxonSourceActions::FLintFinding> F =
			FAxonSourceActions::LintHeaderLines(FakePath, Lines, Specs);
		...
		TArray<FAxonSourceActions::FLintFinding> F2 =
			FAxonSourceActions::LintHeaderLines(FakePath, Lines, TSet<FString>());
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"Axon.Source.CppErgonomics.GenerateClassStubText",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCppErgoGenerateClassStubTextTest::RunTest(const FString& /*Parameters*/)
{
	...
	{
		FString H, C;
		FAxonSourceActions::GenerateClassStubText(
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"Axon.Source.CppErgonomics.GenerateClassStubNeverWrites",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCppErgoGenerateClassStubNeverWritesTest::RunTest(const FString& /*Parameters*/)
{
	...
	// Confirm the registered schema exposes no write_to_disk param.
	{
		...
		{
			const TArray<FAxonActionInfo> Actions = Registry.GetActions(TEXT("source"));
			for (const FAxonActionInfo& Info : Actions)
			{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"Axon.Source.CppErgonomics.AllmanClassIndexing",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCppErgoAllmanClassIndexingTest::RunTest(const FString& /*Parameters*/)
{
	...
	auto ClassRow = [&](const TCHAR* Name) -> TOptional<FAxonSourceSymbol>
	{
		// Try both kinds — GetSymbolsByName with empty kind returns class OR struct.
		TArray<FAxonSourceSymbol> Rows = DB.GetSymbolsByName(FString(Name));
		for (const FAxonSourceSymbol& R : Rows)
		{
		...
		return TOptional<FAxonSourceSymbol>();
	};

	// 1) Plain allman class FAllmanPlain -> class row with a real line span, members extracted.
	{
		TOptional<FAxonSourceSymbol> Row = ClassRow(TEXT("FAllmanPlain"));
		...
		for (const FAxonSourceSymbol& M : DB.GetSymbolsByName(TEXT("Member")))
		{
			if (M.Kind == TEXT("variable") && M.QualifiedName == TEXT("FAllmanPlain::Member")) { bFoundVar = true; }
		}
		for (const FAxonSourceSymbol& M : DB.GetSymbolsByName(TEXT("GetMember")))
		{
		...
		for (int32 N = 0; N <= 11; ++N)
		{
			...
			bool bFound = false;
			for (const FAxonSourceSymbol& M : DB.GetSymbolsByName(MemberName))
			{
	...
	{
		TOptional<FAxonSourceSymbol> Row = ClassRow(TEXT("FAllmanDerived"));
		TestTrue(TEXT("FAllmanDerived indexed"), Row.IsSet());
		if (Row.IsSet())
		{
			TArray<FAxonSourceInheritance> Parents = DB.GetParents(Row.GetValue().Id);
			bool bHasBase = false;
			for (const FAxonSourceInheritance& P : Parents)
			{
	...
	{
		TOptional<FAxonSourceSymbol> Row = ClassRow(TEXT("FOneLiner"));
	...
	//    (template-param guard), but the real FAllmanTpl declaration must index.
	{
		...
		bool bBogusT = false;
		for (const FAxonSourceSymbol& R : DB.GetSymbolsByName(TEXT("T")))
		{
	...
	//    implementation records the class row (best-effort) but extracts NO members.
	{
		...
		for (const FAxonSourceSymbol& M : DB.GetSymbolsByName(TEXT("NeverReached")))
		{
