...
//
// DEVIATION NOTE: plan §6 file-table specifies `Source/AxonSource/Tests/...`.
// This file lives under `Source/AxonSource/Private/Tests/...` so UBT's
// auto-include of `Private/` picks it up without a Build.cs change. Matches
// the established precedent (see `AxonFuzzyMatchTest.cpp` deviation note).
//
// Test names use the `Axon.CursorPagination.*` prefix to match Axon
// convention (NOT `Leviathan.Axon.*`).
...
namespace AxonCursorPaginationTestDetail
{
	...
	static bool HasNextCursor(const FAxonActionResult& R, FString& OutCursor)
	{
		OutCursor.Reset();
		if (!R.bSuccess || !R.Result.IsValid()) return false;
	...
	static bool IsSourceIndexAvailable()
	{
		...
		FAxonActionResult Probe = Call(TEXT("class"), 1);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FAxonCursorPaginationRoundTripTest,
	"Axon.CursorPagination.RoundTrip",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FAxonCursorPaginationRoundTripTest::RunTest(const FString& /*Parameters*/)
{
	AxonCursorCodec::FCursorState In;
	In.QueryHash = 0xDEADBEEFu;
	In.SymbolPage = 2;
	In.SourcePage = 3;
	In.CachedTotalEstimate = 420;

	const FString Encoded = AxonCursorCodec::Encode(In);
	TestFalse(TEXT("Encoded cursor is non-empty"), Encoded.IsEmpty());

	AxonCursorCodec::FCursorState Out;
	const bool bDecoded = AxonCursorCodec::Decode(Encoded, Out);
	...
	AxonCursorCodec::FCursorState Junk;
	TestFalse(TEXT("Garbage base64 rejected cleanly"),
		AxonCursorCodec::Decode(TEXT("this-is-not-base64!!!"), Junk));

	// Empty string → false out.
	TestFalse(TEXT("Empty cursor rejected"),
		AxonCursorCodec::Decode(TEXT(""), Junk));
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FAxonCursorPaginationQueryMismatchRejectionTest,
	"Axon.CursorPagination.QueryMismatchRejection",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FAxonCursorPaginationQueryMismatchRejectionTest::RunTest(const FString& /*Parameters*/)
{
	using namespace AxonCursorPaginationTestDetail;
	...
	AxonCursorCodec::FCursorState Bogus;
	Bogus.QueryHash = 0x12345678u;
	Bogus.SymbolPage = 1;
	Bogus.SourcePage = 1;
	Bogus.CachedTotalEstimate = 99;
	const FString BogusCursor = AxonCursorCodec::Encode(Bogus);

	FAxonActionResult R = Call(TEXT("FooBarBaz_QueryHashMismatchProbe_2026"), 10, BogusCursor);
	...
	FAxonActionResult R2 = Call(TEXT("FooBarBaz_QueryHashMismatchProbe_2026"), 10,
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FAxonCursorPaginationHardCapTest,
	"Axon.CursorPagination.HardCap",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FAxonCursorPaginationHardCapTest::RunTest(const FString& /*Parameters*/)
{
	using namespace AxonCursorPaginationTestDetail;
	...
	FAxonActionResult R = Call(TEXT("class"), 2000);
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FAxonCursorPaginationTotalEstimatePageZeroOnlyTest,
	"Axon.CursorPagination.TotalEstimatePageZeroOnly",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FAxonCursorPaginationTotalEstimatePageZeroOnlyTest::RunTest(const FString& /*Parameters*/)
{
	using namespace AxonCursorPaginationTestDetail;
	if (!IsSourceIndexAvailable())
	{
	...
	FAxonActionResult Page0 = Call(TEXT("class"), 5);
	...
	FAxonActionResult Page1 = Call(TEXT("class"), 5, NextCursor);
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FAxonCursorPaginationPageBoundaryNoNextCursorTest,
	"Axon.CursorPagination.PageBoundaryNoNextCursor",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FAxonCursorPaginationPageBoundaryNoNextCursorTest::RunTest(const FString& /*Parameters*/)
{
	using namespace AxonCursorPaginationTestDetail;
	if (!IsSourceIndexAvailable())
	{
	...
	FAxonActionResult R = Call(TEXT("FZeroToOne_VeryUnlikelyTokenName_2026"), 500);
