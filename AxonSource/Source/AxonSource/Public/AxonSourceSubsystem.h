...
#include "AxonSourceSubsystem.generated.h"

class FAxonSourceIndexer;
 ...
 */
UCLASS()
class AXONSOURCE_API UAxonSourceSubsystem : public UEditorSubsystem
{
	GENERATED_BODY()

public:
	virtual ~UAxonSourceSubsystem();
	virtual void Initialize(FSubsystemCollectionBase& Collection) override;
	virtual void Deinitialize() override;

	/** Get the source database (read-only). May be null if DB doesn't exist. */
	FAxonSourceDatabase* GetDatabase() { return Database.IsValid() ? Database.Get() : nullptr; }
...
private:
	...
	TUniquePtr<FAxonSourceDatabase> Database;
	FAxonSourceIndexer* Indexer = nullptr;
