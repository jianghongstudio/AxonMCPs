...
class FAxonSourceDatabase;

/**
 * 9 engine source intelligence actions + 1 reindex trigger.
 * Ports the Python unreal-source-mcp server tools to native C++.
 */
class FAxonSourceActions
{
public:
	static void RegisterAll();

	// Public for unit testing (AxonCppErgonomicsTest.cpp) — pure, stateless helpers.
	/**
	...
	//     handlers; public for unit testing — see AxonCppErgonomicsTest.cpp).
	 ...
	 */
	static bool ResolveIncludeForSymbol(FAxonSourceDatabase* DB, const FString& Symbol,
	 ...
	 * has a class row).
	 */
	static bool ResolveFirstSignature(FAxonSourceDatabase* DB, const FString& Symbol,
	 ...
	 */
	static bool SymbolExists(FAxonSourceDatabase* DB, const FString& Symbol);
...
private:
	// Action handlers
	static FAxonActionResult HandleReadSource(const TSharedPtr<FJsonObject>& Params);
	static FAxonActionResult HandleFindReferences(const TSharedPtr<FJsonObject>& Params);
	static FAxonActionResult HandleFindCallers(const TSharedPtr<FJsonObject>& Params);
	static FAxonActionResult HandleFindCallees(const TSharedPtr<FJsonObject>& Params);
	static FAxonActionResult HandleSearchSource(const TSharedPtr<FJsonObject>& Params);
	static FAxonActionResult HandleGetClassHierarchy(const TSharedPtr<FJsonObject>& Params);
	static FAxonActionResult HandleGetModuleInfo(const TSharedPtr<FJsonObject>& Params);
	static FAxonActionResult HandleGetSymbolContext(const TSharedPtr<FJsonObject>& Params);
	static FAxonActionResult HandleReadFile(const TSharedPtr<FJsonObject>& Params);
	static FAxonActionResult HandleTriggerReindex(const TSharedPtr<FJsonObject>& Params);
	static FAxonActionResult HandleTriggerProjectReindex(const TSharedPtr<FJsonObject>& Params);

	// Phase 1 — demand-proven lookups (LLM C++ authoring ergonomics)
	static FAxonActionResult HandleGetIncludePath(const TSharedPtr<FJsonObject>& Params);    // item 1
	static FAxonActionResult HandleGetSignature(const TSharedPtr<FJsonObject>& Params);      // item 2
	static FAxonActionResult HandleCheckDeprecations(const TSharedPtr<FJsonObject>& Params); // item 3

	// Phase 2 — round-trip compression
	static FAxonActionResult HandleVerifySymbols(const TSharedPtr<FJsonObject>& Params);     // item 4
	static FAxonActionResult HandleFindExampleUsage(const TSharedPtr<FJsonObject>& Params);  // item 5

	// Phase 3 — pre-flight lint + stub gen
	static FAxonActionResult HandleLintHeader(const TSharedPtr<FJsonObject>& Params);        // item 7
	static FAxonActionResult HandleGenerateClassStub(const TSharedPtr<FJsonObject>& Params); // item 9

	// Helpers
	static FAxonSourceDatabase* GetDB();
	...
	static bool ResolveOwningModule(FAxonSourceDatabase* DB, const FString& Symbol, FString& OutModule, FString& OutBuildCsNote);
	...
	static void WalkAncestors(FAxonSourceDatabase* DB, int64 SymId, TArray<FString>& Lines, int32 Indent, int32 MaxDepth, FHierarchyCounter& Counter, TSet<int64>& Visited);
	static void WalkDescendants(FAxonSourceDatabase* DB, int64 SymId, TArray<FString>& Lines, int32 Indent, int32 MaxDepth, FHierarchyCounter& Counter, TSet<int64>& Visited);
};
