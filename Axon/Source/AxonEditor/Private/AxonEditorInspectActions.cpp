...
//
// Phase 2 of plan: 2026-05-26-axon-editor-preview-expansion.md.
//
...
// AxonEditorActions.cpp::RegisterActions; declarations in the public
// AxonEditorActions.h header.
//
...
// in the plan) — keys go directly on FAxonActionResult::Result, not
...
FAxonActionResult FAxonEditorActions::HandleInspectMaterialPBR(const TSharedPtr<FJsonObject>& Params)
{
	if (!Params.IsValid())
	{
...
FAxonActionResult FAxonEditorActions::HandleInspectTextureChannels(const TSharedPtr<FJsonObject>& Params)
{
	if (!Params.IsValid())
	{
		return FAxonActionResult::Error(TEXT("Missing params"));
	...
	if (bEmitSplits)
	{
		...
		if (OutputDir.IsEmpty())
		{
			OutputDir = FPaths::ProjectDir()
				/ TEXT("Saved/Tests/Axon/InspectTexture")
				/ Texture->GetName();
		}
	...
	// AxonEditor never reach here (the whole module is Editor-typed), but
