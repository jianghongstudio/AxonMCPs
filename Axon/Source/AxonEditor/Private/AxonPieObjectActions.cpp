...
DEFINE_LOG_CATEGORY_STATIC(LogAxonPieObject, Log, All);
...
namespace
{
		...
		const TSharedPtr<FJsonObject>& Json, UObject* Owner, int32 Depth, FString& OutError)
	{
		...
		for (const TPair<FString, TSharedPtr<FJsonValue>>& Field : Json->Values)
		{
			...
			// (The shared AxonStructField resolver needs a UObject container; a bare
...
void FAxonPieObjectActions::RegisterActions(FAxonToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("editor"), TEXT("pie_get_object_properties"),
		...
		FAxonActionHandler::CreateStatic(&HandleGetObjectProperties),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("pie_call_function"),
		...
		FAxonActionHandler::CreateStatic(&HandleCallFunction),
		FParamSchemaBuilder()
	...
	UE_LOG(LogAxonPieObject, Log, TEXT("AxonEditor: registered 2 live-PIE object actions"));
}

// ── pie_get_object_properties ──────────────────────────────────────────

FAxonActionResult FAxonPieObjectActions::HandleGetObjectProperties(const TSharedPtr<FJsonObject>& Params)
{
	const AxonPieObject::FResolvedObject Resolved = AxonPieObject::Resolve(Params);
	if (!Resolved.bSuccess)
	{
		return FAxonActionResult::Error(Resolved.Error);
	}
	...
	{
		return FAxonActionResult::Error(TEXT("pie_get_object_properties requires a non-empty 'properties' array of dotted paths"));
	}
	...
	for (const TSharedPtr<FJsonValue>& V : *PropsArr)
	{
		...
		FString TypeName;
		TSharedPtr<FJsonValue> Val = AxonPieObject::ReadDottedValue(Resolved.Object, Path, TypeName);
		if (Val.IsValid())
		{
...
FAxonActionResult FAxonPieObjectActions::HandleCallFunction(const TSharedPtr<FJsonObject>& Params)
{
	const AxonPieObject::FResolvedObject Resolved = AxonPieObject::Resolve(Params);
	if (!Resolved.bSuccess)
	{
	...
	{
		return FAxonActionResult::Error(TEXT("pie_call_function requires a 'function' name"));
	}
	...
	if (!Frame.Memory)
	{
		return FAxonActionResult::Error(TEXT("Failed to allocate the function parameter frame"));
	}
	...
	for (TFieldIterator<FProperty> It(Function); It && (It->PropertyFlags & CPF_Parm); ++It)
	{
		...
		FString TypeName;
		TSharedPtr<FJsonValue> Val = AxonPieObject::ReadLeafValue(Prop, ValuePtr, Obj, TypeName);
