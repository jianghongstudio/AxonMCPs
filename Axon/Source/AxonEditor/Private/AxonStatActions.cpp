...
DEFINE_LOG_CATEGORY_STATIC(LogAxonStat, Log, All);
...
void FAxonStatActions::RegisterActions(FAxonToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("editor"), TEXT("get_stat_group_values"),
		...
		FAxonActionHandler::CreateStatic(&HandleGetStatGroupValues),
		FParamSchemaBuilder()
	...
	UE_LOG(LogAxonStat, Log, TEXT("AxonEditor: registered 1 stat-group readout action"));
}

#if STATS

FAxonActionResult FAxonStatActions::HandleGetStatGroupValues(const TSharedPtr<FJsonObject>& Params)
{
	FString GroupNameRaw;
	if (!Params->TryGetStringField(TEXT("group_name"), GroupNameRaw) || GroupNameRaw.IsEmpty())
	{
		return FAxonActionResult::Error(TEXT("get_stat_group_values requires 'group_name' (e.g. 'STATGROUP_Anim' or 'Anim')"));
	}
	...
	if (SampleFrames > 240)
	{
		return FAxonActionResult::Error(TEXT("sample_frames too large (max 240) — the stats history ring only retains a small number of frames"));
	}
