...
//
// Phase 3 of plan: 2026-05-26-axon-editor-preview-expansion.md.
//
...
// AxonEditorActions.cpp::RenderAndSaveCapture. Declarations live in the
// public AxonEditorActions.h header (Phase 3 block); registrations live
// in AxonEditorActions.cpp::RegisterActions.
//
...
DEFINE_LOG_CATEGORY_STATIC(LogAxonPreviewActions, Log, All);
...
namespace
{
	...
	/** Build default timestamp-suffixed output path under Saved/Screenshots/Axon/<Bucket>/. */
	static FString DefaultOutputPath(const TCHAR* Bucket)
	{
		const FString Timestamp = FDateTime::Now().ToString(TEXT("%Y%m%d_%H%M%S"));
		return FPaths::ProjectDir() / TEXT("Saved/Screenshots/Axon") / Bucket /
...
FAxonActionResult FAxonEditorActions::HandleCaptureMaterialGrid(
	const TSharedPtr<FJsonObject>& Params)
{
	if (!Params.IsValid())
	{
		return FAxonActionResult::Error(TEXT("Params object is null"));
	}
	...
	{
		return FAxonActionResult::Error(TEXT("material_paths array is required"));
	}
	if (MaterialPathsArr->Num() == 0)
	{
		return FAxonActionResult::Error(TEXT("material_paths array is empty"));
	}
	if (MaterialPathsArr->Num() > 16)
	{
		return FAxonActionResult::Error(TEXT("material_paths array exceeds 16-entry limit"));
	}
	...
	for (const TSharedPtr<FJsonValue>& Val : *MaterialPathsArr)
	{
		...
		if (!Material)
		{
			UE_LOG(LogAxonPreviewActions, Warning,
	...
	if (!World)
	{
		return FAxonActionResult::Error(TEXT("PreviewScene has no UWorld"));
	}
	...
	if (!bSuccess)
	{
		return FAxonActionResult::Error(TEXT("capture_material_grid: render or save failed"));
	}
...
FAxonActionResult FAxonEditorActions::HandleCaptureWithOverlay(
	const TSharedPtr<FJsonObject>& Params)
{
	if (!Params.IsValid())
	{
	...
	if (!bSuccess)
	{
		return FAxonActionResult::Error(TEXT("capture_with_overlay: render or save failed"));
	}
