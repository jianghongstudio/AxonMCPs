...
//                                    (already used elsewhere in AxonEditor).
...
//                                    (mirrors AxonCommonUIHelpers.cpp:70-75
//                                     and AxonEditorActions.cpp asset-save
//                                     sites).
//
...
//   * get_module_status: empty input list (returns all Axon modules),
...
namespace
{
	...
	/** Asset-registry + in-memory FindObject existence check. Mirrors AxonAI::EnsureAssetPathFree. */
...
void FAxonEditorMapActions::RegisterActions(FAxonToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("editor"), TEXT("create_empty_map"),
		...
		FAxonActionHandler::CreateStatic(&HandleCreateEmptyMap),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("path"), TEXT("Asset path under /Game/... where the new UWorld is saved (e.g. /Game/Tests/Axon/Audio/Map_Test)"))
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("get_module_status"),
		TEXT("Report plugin-enabled + module-loaded status for Axon (or arbitrary) modules. Wraps IPluginManager + FModuleManager. If module_names is omitted or empty, returns rows for all Axon modules. Unknown module names return a row with enabled=false, loaded=false (no error)."),
		FAxonActionHandler::CreateStatic(&HandleGetModuleStatus),
		FParamSchemaBuilder()
			.Optional(TEXT("module_names"), TEXT("array"), TEXT("Optional array of module name strings. Omit to query all Axon modules."))
			.Build());
}
...
FAxonActionResult FAxonEditorMapActions::HandleCreateEmptyMap(const TSharedPtr<FJsonObject>& Params)
{
	FString InPath;
	...
	if (!Factory)
	{
		return FAxonActionResult::Error(TEXT("Failed to construct UWorldFactory"));
	}
...
FAxonActionResult FAxonEditorMapActions::HandleGetModuleStatus(const TSharedPtr<FJsonObject>& Params)
{
	...
	//    that belongs to the Axon plugin (the canonical use case for
	...
	{
		TSharedPtr<IPlugin> AxonPlugin = IPluginManager::Get().FindPlugin(TEXT("Axon"));
		if (AxonPlugin.IsValid())
		{
			for (const FModuleDescriptor& ModDesc : AxonPlugin->GetDescriptor().Modules)
			{
