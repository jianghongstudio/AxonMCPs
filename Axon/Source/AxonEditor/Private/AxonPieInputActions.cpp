...
DEFINE_LOG_CATEGORY_STATIC(LogAxonPieInput, Log, All);
...
namespace
{
	...
	// Mirrors AxonPieObject::FindPieWorld so the input actions share one notion of
...
void FAxonPieInputActions::RegisterPieEndHook()
{
...
void FAxonPieInputActions::UnregisterPieEndHook()
{
	if (GPrePieEndedHandle.IsValid())
	{
...
namespace
{
	...
	const UInputAction* ResolveInputAction(const FString& Spec, FString& OutError)
	{
		...
		if (FirstMatch)
		{
			if (MatchCount > 1)
			{
				UE_LOG(LogAxonPieInput, Warning,
...
void FAxonPieInputActions::RegisterActions(FAxonToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("editor"), TEXT("pie_set_control_rotation"),
		...
		FAxonActionHandler::CreateStatic(&HandleSetControlRotation),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("pie_inject_input_action"),
		...
		FAxonActionHandler::CreateStatic(&HandleInjectInputAction),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("pie_possess_spectator_free"),
		...
		FAxonActionHandler::CreateStatic(&HandlePossessSpectatorFree),
		FParamSchemaBuilder()
			.Required(TEXT("enable"), TEXT("bool"), TEXT("true = detach to a free-fly spectator; false = re-possess the original pawn."))
			.Optional(TEXT("player_index"), TEXT("number"), TEXT("Local player index (default 0)."), TEXT("0"))
			.Build());

	UE_LOG(LogAxonPieInput, Log, TEXT("AxonEditor: registered 3 PIE input/control actions"));
}
...
FAxonActionResult FAxonPieInputActions::HandleSetControlRotation(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* PieWorld = GetPieWorld();
	if (!PieWorld)
	{
		return FAxonActionResult::Error(TEXT("PIE not running — start Play-In-Editor first"));
	}
...
FAxonActionResult FAxonPieInputActions::HandleInjectInputAction(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* PieWorld = GetPieWorld();
	if (!PieWorld)
	{
	...
	{
		return FAxonActionResult::Error(TEXT("pie_inject_input_action requires 'input_action' (asset path or name)"));
	}
	...
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("pie_inject_input_action 'value' invalid: %s"), *ValueError));
	}
...
FAxonActionResult FAxonPieInputActions::HandlePossessSpectatorFree(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* PieWorld = GetPieWorld();
	if (!PieWorld)
	{
	...
	{
		return FAxonActionResult::Error(TEXT("pie_possess_spectator_free requires 'enable' (bool)"));
	}
