...
// Item 8: fix_hints — pattern-based compiler fix suggestions (no source-index DB).
...
FAxonLogCapture* FAxonEditorActions::CachedLogCapture = nullptr;
double FAxonEditorActions::LastCompileTimestamp = 0.0;
FString FAxonEditorActions::LastCompileResult = TEXT("none");
bool FAxonEditorActions::bIsCompiling = false;
bool FAxonEditorActions::bPatchApplied = false;
double FAxonEditorActions::LastCompileEndTimestamp = 0.0;

// --- Log capture ---

void FAxonLogCapture::Serialize(const TCHAR* V, ELogVerbosity::Type Verbosity, const FName& Category)
{
...
TArray<FAxonLogEntry> FAxonLogCapture::GetRecentEntries(int32 Count) const
{
	FScopeLock ScopeLock(&Lock);
	TArray<FAxonLogEntry> Result;
...
TArray<FAxonLogEntry> FAxonLogCapture::SearchEntries(const FString& Pattern, const FString& CategoryFilter, ELogVerbosity::Type MaxVerbosity, int32 Limit) const
{
	FScopeLock ScopeLock(&Lock);
	TArray<FAxonLogEntry> Result;
	...
	for (int32 i = 0; i < Total && Result.Num() < Limit; ++i)
	{
		...
		const FAxonLogEntry& Entry = RingBuffer[Idx];
...
TArray<FString> FAxonLogCapture::GetActiveCategories() const
{
	FScopeLock ScopeLock(&Lock);
	TSet<FString> Categories;
	for (const FAxonLogEntry& Entry : RingBuffer)
	{
...
int32 FAxonLogCapture::GetCountByVerbosity(ELogVerbosity::Type Verbosity) const
{
	FScopeLock ScopeLock(&Lock);
	switch (Verbosity)
	{
...
int32 FAxonLogCapture::GetTotalCount() const
{
	FScopeLock ScopeLock(&Lock);
	return TotalFatal + TotalError + TotalWarning + TotalLog + TotalVerbose;
}

TArray<FAxonLogEntry> FAxonLogCapture::GetEntriesSince(double SinceTimestamp, const TArray<FName>& CategoryFilter, ELogVerbosity::Type MaxVerbosity, int32 Limit) const
{
	FScopeLock ScopeLock(&Lock);
	TArray<FAxonLogEntry> Result;
...
int32 FAxonLogCapture::CountErrorsSince(double SinceTimestamp) const
{
	FScopeLock ScopeLock(&Lock);
	int32 Count = 0;
	int32 Total = RingBuffer.Num();
...
static TSharedPtr<FJsonObject> LogEntryToJson(const FAxonLogEntry& Entry)
{
...
void FAxonEditorActions::InitLiveCodingDelegate()
{
#if PLATFORM_WINDOWS
	ILiveCodingModule* LC = FModuleManager::GetModulePtr<ILiveCodingModule>(LIVE_CODING_MODULE_NAME);
	if (LC)
	{
		LC->GetOnPatchCompleteDelegate().AddStatic(&FAxonEditorActions::OnLiveCodingPatchComplete);
	}
#endif
}

void FAxonEditorActions::OnLiveCodingPatchComplete()
{
	bIsCompiling = false;
	bPatchApplied = true;
...
void FAxonEditorActions::RegisterActions(FAxonLogCapture* LogCapture)
{
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("trigger_build"),
		...
		FAxonActionHandler::CreateStatic(&HandleTriggerBuild),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("get_build_errors"),
		TEXT("Get build errors and warnings, scoped to a time window and bucketed into compile_errors (LogLiveCoding/LogCompile/LogLinker) vs other_errors. By default excludes LogPython + LogAxon capture noise from the headline error_count (still visible under other_errors). Window precedence: since_marker > since_iso > since_seconds/since > last compile."),
		FAxonActionHandler::CreateStatic(&HandleGetBuildErrors),
		FParamSchemaBuilder()
			...
			.Optional(TEXT("exclude_categories"), TEXT("array"), TEXT("Categories bucketed under other_errors and kept OUT of the headline error_count (still returned — never hidden). Default [LogPython, LogAxon]."))
			.Build());
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("get_recent_logs"),
		...
		FAxonActionHandler::CreateStatic(&HandleGetRecentLogs),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("search_logs"),
		...
		FAxonActionHandler::CreateStatic(&HandleSearchLogs),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("tail_log"),
		...
		FAxonActionHandler::CreateStatic(&HandleTailLog),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("get_log_categories"),
		...
		FAxonActionHandler::CreateStatic(&HandleGetLogCategories),
		MakeShared<FJsonObject>());

	Registry.RegisterAction(TEXT("editor"), TEXT("get_log_stats"),
		TEXT("Get log statistics by verbosity level"),
		FAxonActionHandler::CreateStatic(&HandleGetLogStats),
		MakeShared<FJsonObject>());
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("get_compile_output"),
		...
		FAxonActionHandler::CreateStatic(&HandleGetCompileOutput),
		MakeShared<FJsonObject>());

	Registry.RegisterAction(TEXT("editor"), TEXT("get_crash_context"),
		TEXT("Get last crash/ensure context information"),
		FAxonActionHandler::CreateStatic(&HandleGetCrashContext),
		MakeShared<FJsonObject>());
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("run_console_command"),
		...
		FAxonActionHandler::CreateStatic(&HandleRunConsoleCommand),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("list_dirty_packages"),
		...
		FAxonActionHandler::CreateStatic(&HandleListDirtyPackages),
		FParamSchemaBuilder()
			.Optional(TEXT("scope_paths"), TEXT("array"), TEXT("Array of /Game path prefixes to filter by (e.g. [\"/Game/Tests/Axon\"]). Omit for all dirty packages. Transient/in-memory packages are excluded unless include_transient=true."))
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("save_packages"),
		...
		FAxonActionHandler::CreateStatic(&HandleSavePackages),
		FParamSchemaBuilder()
			.Required(TEXT("packages"), TEXT("array"), TEXT("Array of long package names (e.g. [\"/Game/Tests/Axon/DA_Foo\"]) to save."))
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("run_pie_smoke"),
		...
		FAxonActionHandler::CreateStatic(&HandleRunPieSmoke),
		FParamSchemaBuilder()
			.OptionalAssetPath(TEXT("map"), TEXT("Level asset path to load before PIE (e.g. /Game/Tests/Axon/Maps/M_Harness). Omit to use the current editor level."))
			.Optional(TEXT("marker"), TEXT("string"), TEXT("Marker token emitted to the log; post-marker pattern matching counts only lines after it. Default AXON_SMOKE."), TEXT("AXON_SMOKE"))
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("list_errored_blueprints"),
		...
		FAxonActionHandler::CreateStatic(&HandleListErroredBlueprints),
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("capture_pie_movement_clip"),
		...
		FAxonActionHandler::CreateStatic(&HandleCapturePieMovementClip),
		FParamSchemaBuilder()
			.OptionalAssetPath(TEXT("map"), TEXT("Level asset path to load before PIE. Omit to use the current editor level."))
			.Optional(TEXT("marker"), TEXT("string"), TEXT("Log marker token. Default AXON_CLIP."), TEXT("AXON_CLIP"))
			.Optional(TEXT("duration"), TEXT("number"), TEXT("Seconds the editor loop advances PIE before the session auto-completes (clamped 0-120). Default 5."), TEXT("5"))
			...
			.OptionalDiskPath(TEXT("output_path"), TEXT("Directory for frame PNGs. Disk-relative, absolute, OR a virtual /Game/... path (resolved to <project>/Content/...). Default Saved/Screenshots/Axon/PieClip/<timestamp>/. The resolved absolute dir is echoed as resolved_output_dir; a virtual path that can't be written is a hard error (no silent no-op)."))
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("create_nav_harness_map"),
		...
		FAxonActionHandler::CreateStatic(&HandleCreateNavHarnessMap),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("path"), TEXT("Asset path for the new UWorld (e.g. /Game/Tests/Axon/Maps/M_NavHarness)."))
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("author_map_settings"),
		...
		FAxonActionHandler::CreateStatic(&HandleAuthorMapSettings),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("capture_scene_preview"),
		...
		FAxonActionHandler::CreateStatic(&HandleCaptureScenePreview),
		FParamSchemaBuilder()
	...
	// --- Inspect actions (Phase 2: 2026-05-26-axon-editor-preview-expansion plan) ---
	// Pure reflection / source-mip-read. No render path. Bodies in
	// AxonEditorInspectActions.cpp.
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("inspect_material_pbr"),
		...
		FAxonActionHandler::CreateStatic(&HandleInspectMaterialPBR),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("inspect_texture_channels"),
		...
		FAxonActionHandler::CreateStatic(&HandleInspectTextureChannels),
		FParamSchemaBuilder()
			...
			.OptionalDiskPath(TEXT("output_dir"), TEXT("Output directory for split PNGs (default: Saved/Tests/Axon/InspectTexture/<TextureName>/)"))
			.Build());

	// --- Composite-capture actions (Phase 3: 2026-05-26-axon-editor-preview-expansion plan) ---
	// Multi-asset / show-flag overlay captures. Bodies live in
	// AxonEditorPreviewActions.cpp.
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("capture_material_grid"),
		...
		FAxonActionHandler::CreateStatic(&HandleCaptureMaterialGrid),
		FParamSchemaBuilder()
			.Required(TEXT("material_paths"), TEXT("array"), TEXT("Array of UMaterialInterface asset paths (1..16). Each becomes one grid cell."))
			.OptionalDiskPath(TEXT("output_path"), TEXT("Output PNG path. Default: Saved/Screenshots/Axon/CaptureMaterialGrid/<timestamp>.png"))
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("capture_with_overlay"),
		...
		FAxonActionHandler::CreateStatic(&HandleCaptureWithOverlay),
		FParamSchemaBuilder()
			...
			.OptionalDiskPath(TEXT("output_path"), TEXT("Output PNG path. Default: Saved/Screenshots/Axon/CaptureWithOverlay/<timestamp>.png"))
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("capture_sequence_frames"),
		...
		FAxonActionHandler::CreateStatic(&HandleCaptureSequenceFrames),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Asset path to preview"))
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("capture_anim_frames"),
		...
		FAxonActionHandler::CreateStatic(&HandleCaptureAnimFrames),
		FParamSchemaBuilder()
			...
			.Optional(TEXT("resolution"), TEXT("array"), TEXT("[width, height]. Default [512,512]."), TEXT("[512,512]"))
			.OptionalDiskPath(TEXT("output_dir"), TEXT("Directory for frame PNGs. Disk-relative, absolute, OR a virtual /Game/... path (resolved to <project>/Content/...). Default Saved/Screenshots/Axon/AnimFrames/<timestamp>_<asset>/. The resolved absolute dir is echoed as resolved_output_dir."))
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("import_texture"),
		...
		FAxonActionHandler::CreateStatic(&HandleImportTexture),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("stitch_flipbook"),
		...
		FAxonActionHandler::CreateStatic(&HandleStitchFlipbook),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("delete_assets"),
		...
		FAxonActionHandler::CreateStatic(&HandleDeleteAssets),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("get_viewport_info"),
		...
		FAxonActionHandler::CreateStatic(&HandleGetViewportInfo),
		MakeShared<FJsonObject>());
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("capture_system_gif"),
		...
		FAxonActionHandler::CreateStatic(&HandleCaptureSystemGif),
		FParamSchemaBuilder()
			...
			.OptionalDiskPath(TEXT("output_path"), TEXT("Output directory (default: Saved/Screenshots/Axon/GIF_<timestamp>)"))
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("list_automation_tests"),
		...
		FAxonActionHandler::CreateStatic(&HandleListAutomationTests),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("run_automation_tests"),
		...
		FAxonActionHandler::CreateStatic(&HandleRunAutomationTests),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("run_python"),
		...
		FAxonActionHandler::CreateStatic(&HandleRunPython),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("load_level"),
		...
		FAxonActionHandler::CreateStatic(&HandleLoadLevel),
		FParamSchemaBuilder()
...
FAxonActionResult FAxonEditorActions::HandleTriggerBuild(const TSharedPtr<FJsonObject>& Params)
{
#if PLATFORM_WINDOWS
	ILiveCodingModule* LiveCoding = FModuleManager::GetModulePtr<ILiveCodingModule>(LIVE_CODING_MODULE_NAME);
	if (!LiveCoding)
	{
		return FAxonActionResult::Error(TEXT("Live Coding module not available"));
	}
	...
	{
		return FAxonActionResult::Error(TEXT("A compile is already in progress"));
	}
...
#else
	return FAxonActionResult::Error(TEXT("Live Coding is only available on Windows"));
#endif
}
...
FAxonActionResult FAxonEditorActions::HandleGetBuildErrors(const TSharedPtr<FJsonObject>& Params)
{
	TSharedPtr<FJsonObject> Root = MakeShared<FJsonObject>();
	...
	if (Params->TryGetStringField(TEXT("since_marker"), SinceMarker) && !SinceMarker.IsEmpty() && CachedLogCapture)
	{
		...
		TArray<FAxonLogEntry> Marks = CachedLogCapture->SearchEntries(
			SinceMarker, FString(), ELogVerbosity::VeryVerbose, FAxonLogCapture::MaxEntries);
		if (Marks.Num() > 0)
		{
	...
	{
		ExcludeCategories = { TEXT("LogPython"), TEXT("LogAxon") };
	}
	...
	if (CachedLogCapture)
	{
		TArray<FAxonLogEntry> Entries = CachedLogCapture->GetEntriesSince(
			SinceTimestamp, CategoryFilter, ELogVerbosity::Warning, 500);

		for (const FAxonLogEntry& Entry : Entries)
		{
...
TArray<TSharedPtr<FJsonValue>> FAxonEditorActions::BuildFixHints(const TArray<TSharedPtr<FJsonValue>>& ErrorObjs)
{
	...
	for (int32 Idx = 0; Idx < ErrorObjs.Num(); ++Idx)
	{
		...
		if (Message.Contains(TEXT("LNK2019")) && Message.Contains(TEXT("Z_Construct_")))
		{
			...
			Hint = TEXT("Unresolved UClass constructor (Z_Construct_*) - add the module that declares the referenced UCLASS to your Build.cs dependency list.");
		}
		else if (Message.Contains(TEXT("LNK2019")) && (Message.Contains(TEXT("UDeveloperSettings")) || Message.Contains(TEXT("DeveloperSettings"))))
		{
			Pattern = TEXT("LNK2019_DeveloperSettings");
			Hint = TEXT("UDeveloperSettings lives in the 'DeveloperSettings' module. Add it to your Build.cs dependency list.");
		}
		else if (Message.Contains(TEXT("C4996")))
		{
			Pattern = TEXT("C4996_deprecation");
			Hint = TEXT("Deprecated API used (C4996). Read the deprecation message in the compiler warning for the replacement.");
		}
		else if (Message.Contains(TEXT("generated.h")) && Message.Contains(TEXT("last")))
		{
			Pattern = TEXT("generated_h_not_last");
			Hint = TEXT("The '*.generated.h' include must be the last #include in the header.");
		}
		if (!Hint.IsEmpty())
		{
...
FAxonActionResult FAxonEditorActions::HandleGetBuildStatus(const TSharedPtr<FJsonObject>& Params)
{
	TSharedPtr<FJsonObject> Root = MakeShared<FJsonObject>();
...
FAxonActionResult FAxonEditorActions::HandleGetBuildSummary(const TSharedPtr<FJsonObject>& Params)
{
	TSharedPtr<FJsonObject> Root = MakeShared<FJsonObject>();
...
FAxonActionResult FAxonEditorActions::HandleSearchBuildOutput(const TSharedPtr<FJsonObject>& Params)
{
	FString Pattern = Params->GetStringField(TEXT("pattern"));
	if (Pattern.IsEmpty())
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: pattern"));
	}
	...
	if (CachedLogCapture)
	{
		...
		TArray<FAxonLogEntry> Entries = CachedLogCapture->SearchEntries(
...
FAxonActionResult FAxonEditorActions::HandleGetCompileOutput(const TSharedPtr<FJsonObject>& Params)
{
	TSharedPtr<FJsonObject> Root = MakeShared<FJsonObject>();
...
FAxonActionResult FAxonEditorActions::HandleGetRecentLogs(const TSharedPtr<FJsonObject>& Params)
{
	int32 Count = 100;
	...
	if (CachedLogCapture)
	{
		TArray<FAxonLogEntry> Entries = CachedLogCapture->GetRecentEntries(Count);
		for (const FAxonLogEntry& Entry : Entries)
		{
...
FAxonActionResult FAxonEditorActions::HandleSearchLogs(const TSharedPtr<FJsonObject>& Params)
{
	FString Pattern = Params->GetStringField(TEXT("pattern"));
	...
	if (CachedLogCapture)
	{
		TArray<FAxonLogEntry> Entries = CachedLogCapture->SearchEntries(Pattern, Category, MaxVerbosity, Limit);
		for (const FAxonLogEntry& Entry : Entries)
		{
			LogArr.Add(MakeShared<FJsonValueObject>(LogEntryToJson(Entry)));
		}
...
FAxonActionResult FAxonEditorActions::HandleTailLog(const TSharedPtr<FJsonObject>& Params)
{
	int32 Count = 50;
	if (Params->HasField(TEXT("count")))
	{
...
FAxonActionResult FAxonEditorActions::HandleGetLogCategories(const TSharedPtr<FJsonObject>& Params)
{
	TSharedPtr<FJsonObject> Root = MakeShared<FJsonObject>();
...
FAxonActionResult FAxonEditorActions::HandleGetLogStats(const TSharedPtr<FJsonObject>& Params)
{
	TSharedPtr<FJsonObject> Root = MakeShared<FJsonObject>();
...
FAxonActionResult FAxonEditorActions::HandleGetCrashContext(const TSharedPtr<FJsonObject>& /*Params*/)
{
	return FAxonActionResult::Error(TEXT("Axon has no source index; get_crash_context unavailable"));
}
// ---------------------------------------------------------------------------
// run_console_command — execute a console command on the active world
// ---------------------------------------------------------------------------
FAxonActionResult FAxonEditorActions::HandleRunConsoleCommand(const TSharedPtr<FJsonObject>& Params)
{
	FString Command;
	if (!Params->TryGetStringField(TEXT("command"), Command) || Command.IsEmpty())
	{
		return FAxonActionResult::Error(TEXT("Missing required field: command"));
	}
	...
	if (!TargetWorld)
	{
		return FAxonActionResult::Error(TEXT("No usable world found (no PIE active and no editor world)"));
	}
	...
	else
	{
		if (!GEngine)
		{
			return FAxonActionResult::Error(TEXT("GEngine is null — run_console_command requires engine context."));
		}
...
// ---------------------------------------------------------------------------
UWorld* FAxonEditorActions::FindActivePieWorld()
{
	if (!GEditor)
	{
		return nullptr;
...
namespace
{
	...
	bool EnsureNoResidentPieWorldBeforeMapLoad(FString& OutError)
	{
		...
		UWorld* ResidentPie = FAxonEditorActions::FindActivePieWorld();
		if (!ResidentPie)
		{
		...
		{
			if (!FAxonEditorActions::FindActivePieWorld())
			{
...
bool FAxonEditorActions::StartPieInternal(FString& OutError, bool bSuppressModals)
{
	if (!GUnrealEd)
	{
...
bool FAxonEditorActions::StopPieInternal()
{
	if (!GEditor)
	{
		return false;
...
FAxonActionResult FAxonEditorActions::HandleStartPIE(const TSharedPtr<FJsonObject>& Params)
{
	if (!GUnrealEd) return FAxonActionResult::Error(TEXT("GUnrealEd not available"));
	...
	if (FindActivePieWorld())
	{
		...
		return FAxonActionResult::Success(AlreadyRunning);
	}

	FString StartError;
	if (!StartPieInternal(StartError))
	{
		return FAxonActionResult::Error(StartError);
	}
...
FAxonActionResult FAxonEditorActions::HandleStopPIE(const TSharedPtr<FJsonObject>& Params)
{
	if (!GEditor) return FAxonActionResult::Error(TEXT("GEditor not available"));
...
bool FAxonEditorActions::RenderAndSaveCapture(
	...
	const FString& OutputPath)
{
	...
	if (!RTResource)
	{
		UE_LOG(LogAxon, Error, TEXT("CaptureScenePreview: Failed to get RT resource"));
		return false;
	}
	...
	{
		UE_LOG(LogAxon, Error, TEXT("CaptureScenePreview: ReadPixels failed (read=%d, count=%d)"),
			bReadOk, Pixels.Num());
		return false;
	}
...
bool FAxonEditorActions::CaptureNiagaraFrame(
...
bool FAxonEditorActions::CaptureMaterialFrame(
	...
	const FLinearColor& BackgroundColor)
{
	...
	else
	{
		...
		if (!Mesh)
		{
			UE_LOG(LogAxon, Error, TEXT("CaptureMaterialFrame: Failed to load mesh %s"), *MeshPath);
			return false;
		}
...
FAxonActionResult FAxonEditorActions::HandleCaptureScenePreview(
	const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FAxonActionResult::Error(TEXT("asset_path and asset_type are required"));
	}
	...
	if (Params->HasField(TEXT("camera")))
	{
		...
		if (!Params->TryGetObjectField(TEXT("camera"), CameraObj))
		{
			...
			{
				ParsedCamera = FAxonJsonUtils::Parse(CameraStr);
	...
	else
	{
		...
		OutputPath = FPaths::ProjectDir() / TEXT("Saved/Screenshots/Axon") /
	...
	if (!bSuccess)
	{
		return FAxonActionResult::Error(TEXT("Capture failed — check log for details"));
	}
...
FAxonActionResult FAxonEditorActions::HandleCaptureSequenceFrames(
	const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	FString AssetType = Params->GetStringField(TEXT("asset_type"));
	...
	{
		return FAxonActionResult::Error(TEXT("timestamps array is required"));
	}
	...
	if (Timestamps.Num() == 0)
	{
		return FAxonActionResult::Error(TEXT("timestamps array is empty"));
	}
	...
	else
	{
		FString Timestamp = FDateTime::Now().ToString(TEXT("%Y%m%d_%H%M%S"));
		FString SafeName = FPaths::GetBaseFilename(AssetPath);
		OutputDir = FPaths::ProjectDir() / TEXT("Saved/Screenshots/Axon") /
	...
	if (!AssetType.Equals(TEXT("niagara"), ESearchCase::IgnoreCase))
	{
		return FAxonActionResult::Error(TEXT("capture_sequence_frames currently only supports asset_type: niagara"));
	}
...
FAxonActionResult FAxonEditorActions::HandleCaptureAnimFrames(
	const TSharedPtr<FJsonObject>& Params)
{
	check(IsInGameThread());
	...
	{
		return FAxonActionResult::Error(TEXT("asset_path is required"));
	}
	if (!FApp::CanEverRender())
	{
		return FAxonActionResult::Error(
	...
	if (!Asset)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Failed to load asset: %s"), *AssetPath));
	}
	...
	if (!Params->TryGetStringField(TEXT("output_dir"), OutputDir) || OutputDir.IsEmpty())
	{
		...
		OutputDir = FPaths::ProjectDir() / TEXT("Saved/Screenshots/Axon/AnimFrames") /
	...
	{
		return FAxonActionResult::Error(ResolvedError);
	}
	FString FilenamePrefix = TEXT("frame");
...
FAxonActionResult FAxonEditorActions::HandleImportTexture(
	const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FAxonActionResult::Error(TEXT("source_path and destination are required"));
	}
	...
	{
		return FAxonActionResult::Error(TEXT("Import failed — no assets imported"));
	}

	UTexture2D* Texture = Cast<UTexture2D>(ImportedAssets[0]);
	if (!Texture)
	{
		return FAxonActionResult::Error(TEXT("Imported asset is not a Texture2D"));
	}
	...
	if (Params->HasField(TEXT("settings")))
	{
		...
		else
		{
			...
			{
				ParsedSettings = FAxonJsonUtils::Parse(SettingsStr);
			}
		}
...
FAxonActionResult FAxonEditorActions::HandleStitchFlipbook(
	const TSharedPtr<FJsonObject>& Params)
{
	...
	if (DestPath.IsEmpty())
	{
		return FAxonActionResult::Error(TEXT("dest_path is required"));
	}
	...
	{
		return FAxonActionResult::Error(TEXT("frame_paths array is required and must not be empty"));
	}
	...
	if (FramePaths.Num() == 0)
	{
		return FAxonActionResult::Error(TEXT("No valid file paths in frame_paths"));
	}
	...
	{
		return FAxonActionResult::Error(TEXT("grid must be an array of [columns, rows]"));
	}
	...
	{
		return FAxonActionResult::Error(TEXT("grid columns and rows must be positive"));
	}
	...
	if (!Texture)
	{
		return FAxonActionResult::Error(TEXT("Failed to create UTexture2D"));
	}
	...
	if (!bSaved)
	{
		return FAxonActionResult::Error(TEXT("Failed to save flipbook texture package"));
	}
...
FAxonActionResult FAxonEditorActions::HandleGetViewportInfo(
	const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!ViewportClient)
	{
		return FAxonActionResult::Error(TEXT("No active viewport found"));
	}
...
FAxonActionResult FAxonEditorActions::HandleDeleteAssets(
	const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FAxonActionResult::Error(TEXT("asset_paths array is required and must not be empty"));
	}
	...
	if (AssetPaths.Num() == 0)
	{
		return FAxonActionResult::Error(TEXT("No valid paths in asset_paths"));
	}
...
// ============================================================================
FAxonActionResult FAxonEditorActions::HandleCaptureSystemGif(
	const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	...
	UNiagaraSystem* System = LoadObject<UNiagaraSystem>(nullptr, *AssetPath);
	if (!System)
		return FAxonActionResult::Error(FString::Printf(TEXT("Failed to load Niagara system: %s"), *AssetPath));
	...
	// Capture frames
	FAxonActionResult CaptureResult = HandleCaptureSequenceFrames(CaptureParams);
	if (!CaptureResult.bSuccess)
		return FAxonActionResult::Error(FString::Printf(TEXT("Frame capture failed: %s"), *CaptureResult.ErrorMessage));
...
namespace AxonAutomationDetail
{
...
FAxonActionResult FAxonEditorActions::HandleListAutomationTests(const TSharedPtr<FJsonObject>& Params)
{
	FString Prefix;
	if (Params.IsValid())
	{
	...
	AxonAutomationDetail::CollectMatchingTests(Prefix, Tests);
	...
	{
		TSharedPtr<FJsonObject> Obj = MakeShared<FJsonObject>();
		Obj->SetStringField(TEXT("full_path"), AxonAutomationDetail::GetTestFullPath(Info));
...
FAxonActionResult FAxonEditorActions::HandleRunAutomationTests(const TSharedPtr<FJsonObject>& Params)
{
	FString Prefix;
	if (!Params.IsValid() || !Params->TryGetStringField(TEXT("prefix"), Prefix) || Prefix.IsEmpty())
	{
		return FAxonActionResult::Error(TEXT("Required parameter: prefix (string, e.g. 'MazeLegends.Bow')"));
	}
	...
	AxonAutomationDetail::CollectMatchingTests(Prefix, MatchingTests);
	...
	for (int32 i = 0; i < TestsToRun; ++i)
	{
		...
		const FString FullPath = AxonAutomationDetail::GetTestFullPath(Info);
...
FAxonActionResult FAxonEditorActions::HandleRunPython(const TSharedPtr<FJsonObject>& Params)
{
	FString Command;
	if (!Params.IsValid() || !Params->TryGetStringField(TEXT("command"), Command) || Command.IsEmpty())
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: command"));
	}
...
FAxonActionResult FAxonEditorActions::HandleLoadLevel(const TSharedPtr<FJsonObject>& Params)
{
	FString Path;
	...
	if (!GEditor)
	{
		return FAxonActionResult::Error(TEXT("GEditor is null — load_level requires editor context."));
	}

	ULevelEditorSubsystem* LevelEd = GEditor->GetEditorSubsystem<ULevelEditorSubsystem>();
	if (!LevelEd)
	{
		return FAxonActionResult::Error(TEXT("ULevelEditorSubsystem is unavailable."));
	}
	...
	{
		return FAxonActionResult::Error(PieGuardError);
	}
...
FAxonActionResult FAxonEditorActions::HandleListDirtyPackages(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonEditorPackages;
...
FAxonActionResult FAxonEditorActions::HandleSavePackages(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonEditorPackages;
	...
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: packages (non-empty array of long package names)"));
	}
	...
	{
		return FAxonActionResult::Error(TEXT("packages array contained no valid package names"));
	}
...
namespace AxonEditorPieSmoke
{
	...
	static bool EntryMatches(const FAxonLogEntry& Entry, const FString& Pattern)
	{
	...
	static TSharedPtr<FJsonObject> CountGroupInBucket(const TArray<FAxonLogEntry>& Bucket,
		const TArray<FString>& Patterns, int32& OutGroupTotal, int32& OutDistinctMatched)
	{
		...
		{
			int32 Count = 0;
			for (const FAxonLogEntry& Entry : Bucket)
			{
	...
	static bool AnalysePostMarkerGrouped(FAxonLogCapture* LogCapture, double MarkerTimestamp,
		const FPieSmokeSession& S, TSharedPtr<FJsonObject>& Report)
	{
		TArray<FAxonLogEntry> ActiveBucket;
		TArray<FAxonLogEntry> TeardownBucket;

		if (LogCapture)
		{
			const TArray<FAxonLogEntry> Entries = LogCapture->GetEntriesSince(
				MarkerTimestamp, /*CategoryFilter*/{}, ELogVerbosity::VeryVerbose, FAxonLogCapture::MaxEntries);
		...
		{
			int32 Count = 0;
			for (const FAxonLogEntry& Entry : ActiveBucket)
			{
				if (EntryMatches(Entry, Pattern)) { ++Count; }
			}
			if (Count > 0)
	...
	{
		const bool bPieResident = (FAxonEditorActions::FindActivePieWorld() != nullptr);
...
FAxonActionResult FAxonEditorActions::HandleRunPieSmoke(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonEditorPieSmoke;

	if (!GEditor || !GUnrealEd)
	{
		return FAxonActionResult::Error(TEXT("run_pie_smoke requires editor context (GEditor/GUnrealEd)."));
	}
	if (FindActivePieWorld())
	{
		return FAxonActionResult::Error(TEXT("A PIE session is already running — stop it before run_pie_smoke."));
	}

	FString Marker = TEXT("AXON_SMOKE");
	...
	UE_LOG(LogAxon, Display, TEXT("%s begin (map=%s)"), *Marker,
...
FAxonActionResult FAxonEditorActions::StartTimeseriesSession(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonEditorPieSmoke;
	...
	if (VarPaths.Num() == 0)
	{
		return FAxonActionResult::Error(TEXT("sample_pie_timeseries requires a non-empty 'variables' array of dotted paths"));
	}
	...
	UWorld* PieWorld = FindActivePieWorld();
	const FString Marker = TEXT("AXON_TIMESERIES");
	UE_LOG(LogAxon, Display, TEXT("%s begin (map=%s)"), *Marker,
		PieWorld ? *PieWorld->GetMapName() : TEXT("<current>"));
...
FAxonActionResult FAxonEditorActions::HandlePollPieSmoke(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonEditorPieSmoke;
	...
	if (!Session)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Unknown PIE-smoke session '%s'."), *SessionId));
	}
	...
	return FAxonActionResult::Success(
		BuildSessionReport(*Session, bFull, Mgr.GetLogCapture()));
}

FAxonActionResult FAxonEditorActions::HandleStopPieSmoke(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonEditorPieSmoke;
...
FAxonActionResult FAxonEditorActions::HandleListErroredBlueprints(const TSharedPtr<FJsonObject>& Params)
{
	TArray<FErroredBlueprintEntry> Errored;
	ScanErroredBlueprints(Errored);
...
FAxonActionResult FAxonEditorActions::HandleCapturePieMovementClip(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonEditorPieSmoke;

	if (!GEditor || !GUnrealEd)
	{
		return FAxonActionResult::Error(TEXT("capture_pie_movement_clip requires editor context (GEditor/GUnrealEd)."));
	}
	if (FindActivePieWorld())
	{
		return FAxonActionResult::Error(TEXT("A PIE session is already running — stop it before capture_pie_movement_clip."));
	}

	FString Marker = TEXT("AXON_CLIP");
	Params->TryGetStringField(TEXT("marker"), Marker);
	...
	{
		const FString Stamp = FDateTime::Now().ToString(TEXT("%Y%m%d_%H%M%S"));
		OutputDir = FPaths::ProjectDir() / TEXT("Saved/Screenshots/Axon/PieClip") / Stamp;
	}
	...
	{
		return FAxonActionResult::Error(OutputDirError);
	}
	...
	UWorld* PieWorld = FindActivePieWorld();
	UE_LOG(LogAxon, Display, TEXT("%s begin"), *Marker);
	if (PieWorld)
	{
		RunScripts(Params, PieWorld);
	}
...
// runtime string dispatch (FAxonToolRegistry::ExecuteAction) so AxonEditor
// takes NO compile-time dependency on AxonAI / the NavigationSystem module.
// ---------------------------------------------------------------------------

namespace AxonEditorNavHarness
{
	...
	static bool TryApplyLeaf(FProperty* Prop, void* ValuePtr, const TSharedPtr<FJsonValue>& Value, UObject* Owner, FString& OutWhyUnsupported)
	{
		...
		else if (FStructProperty* StructProp = CastField<FStructProperty>(Prop))
		{
			...
			// reflection text importer — same pattern as AxonMaterialActions.
	...
	static void ApplyProperties(AActor* Actor, const TSharedPtr<FJsonObject>& PropObj, TArray<FString>& OutApplied, TArray<FString>& OutSkipped)
	{
		...
		for (const auto& Pair : PropObj->Values)
		{
			...
			// AxonReflectionWalker.cpp (ctor UnrealType.h:4455, EmptyValues,
			// AddUninitializedValues, GetRawPtr) + the object-inner set in
			// AxonAIStateTreeActions.cpp:2622-2627.
...
FAxonActionResult FAxonEditorActions::HandleCreateNavHarnessMap(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonEditorNavHarness;

	if (!GEditor)
	{
		return FAxonActionResult::Error(TEXT("create_nav_harness_map requires editor context (GEditor)."));
	}
	...
	//    then load it as the active editor world so spawns + nav target it.
	{
		...
		const FAxonActionResult CreateRes = Registry.ExecuteAction(TEXT("editor"), TEXT("create_empty_map"), CreateParams);
	...
	if (!World)
	{
		return FAxonActionResult::Error(TEXT("No editor world after loading the harness map."));
	}
	...
	// 6. Nav bounds — default sized to the floor footprint unless overridden.
	{
		...
		const FAxonActionResult NavRes = Registry.ExecuteAction(TEXT("ai"), TEXT("add_nav_bounds_volume"), NavParams);
	...
	//    for async tile generation).
	{
		...
		const FAxonActionResult RebuildRes = Registry.ExecuteAction(TEXT("ai"), TEXT("rebuild_navigation"), RebuildParams);
	...
	if (NavPoints.Num() > 0)
	{
		...
		const FAxonActionResult ValidateRes = Registry.ExecuteAction(TEXT("ai"), TEXT("validate_nav_points"), ValidateParams);
	...
	// 9. Save the level package now that actors + nav exist.
	{
		...
		const FAxonActionResult SaveRes = Registry.ExecuteAction(TEXT("editor"), TEXT("save_packages"), SaveParams);
...
// ---------------------------------------------------------------------------
FAxonActionResult FAxonEditorActions::HandleAuthorMapSettings(const TSharedPtr<FJsonObject>& Params)
{
	using namespace AxonEditorNavHarness;

	if (!GEditor)
	{
		return FAxonActionResult::Error(TEXT("author_map_settings requires editor context (GEditor)."));
	}
	if (!Params.IsValid())
	{
		return FAxonActionResult::Error(TEXT("author_map_settings requires a params object."));
	}
	...
	if (!World)
	{
		return FAxonActionResult::Error(TEXT("No editor world to author."));
	}
	...
	if (bSave)
	{
		...
		SaveParams->SetArrayField(TEXT("packages"), PkgArr);
		const FAxonActionResult SaveRes =
			FAxonToolRegistry::Get().ExecuteAction(TEXT("editor"), TEXT("save_packages"), SaveParams);
		Result->SetBoolField(TEXT("saved"), SaveRes.bSuccess);
		if (!SaveRes.bSuccess)
		{
			Result->SetStringField(TEXT("save_error"), SaveRes.ErrorMessage);
