...
// AxonEditor). FCsvProfiler is gated behind CSV_PROFILER (WITH_ENGINE && !UE_BUILD_SHIPPING
...
namespace AxonPieSmokeSessionPrivate
{
	...
	// selector fields. Mirrors AxonPieObject::Resolve's matching (actor_label/object_name
	...
	void FireActorSetup(FPieSmokeSession& Session, UWorld* PieWorld)
	{
		...
		for (const FPieSmokeActorSetupEntry& Entry : Session.ActorSetups)
		{
			...
			for (int32 i = 0; i < SpawnCount; ++i)
			{
				...
				Spawned->SetFolderPath(FName(TEXT("Axon/ActorSetup")));
				++Result.SpawnedCount;
...
void FPieSmokeSessionManager::EnsureObserver()
{
	...
	TickerHandle = FTSTicker::GetCoreTicker().AddTicker(
		TEXT("AxonPieSmokeObserver"), 0.0f,
	...
	if (Session.bTimeseries)
	{
		...
		if (bDue && Session.TimeseriesSamples.Num() < Session.MaxSamples)
		{
			...
			if (Target)
			{
				...
				{
					FString TypeName;
					TSharedPtr<FJsonValue> Val = AxonPieObject::ReadDottedValue(Target, Path, TypeName);
					if (Val.IsValid())
					{
	...
	if (Session.bCaptureFrames && !Session.bCaptureDeferred)
	{
		...
		if (bDue)
		{
			...
			else if (Session.CaptureFrameIndex == 0 && SampleTime > Session.CaptureInterval * 2.0)
			{
				...
				UE_LOG(LogAxonPieSmoke, Warning,
