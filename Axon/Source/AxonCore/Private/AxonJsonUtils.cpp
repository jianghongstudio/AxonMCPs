...
DEFINE_LOG_CATEGORY(LogAxon);

TSharedPtr<FJsonObject> FAxonJsonUtils::SuccessResponse(const TSharedPtr<FJsonValue>& Id, const TSharedPtr<FJsonValue>& Result)
{
...
TSharedPtr<FJsonObject> FAxonJsonUtils::ErrorResponse(const TSharedPtr<FJsonValue>& Id, int32 Code, const FString& Message, const TSharedPtr<FJsonValue>& Data)
{
...
TSharedPtr<FJsonObject> FAxonJsonUtils::SuccessObject(const TSharedPtr<FJsonValue>& Id, const TSharedPtr<FJsonObject>& ResultObj)
{
	return SuccessResponse(Id, MakeShared<FJsonValueObject>(ResultObj));
}

TSharedPtr<FJsonObject> FAxonJsonUtils::SuccessString(const TSharedPtr<FJsonValue>& Id, const FString& Message)
{
...
FString FAxonJsonUtils::Serialize(const TSharedPtr<FJsonObject>& JsonObject)
{
...
TSharedPtr<FJsonObject> FAxonJsonUtils::Parse(const FString& JsonString)
{
...
TSharedRef<FJsonValueArray> FAxonJsonUtils::StringArrayToJson(const TArray<FString>& Strings)
{
...
namespace AxonResponseShapingDetail
{
	...
	static bool ReadStringArrayParam(const TSharedPtr<FJsonObject>& Params, const TCHAR* Key, TSet<FString>& Out)
	{
		...
		if (Params->TryGetStringField(Key, StrValue) && !StrValue.IsEmpty())
		{
			...
			if (FJsonSerializer::Deserialize(Reader, ParsedValue) && ParsedValue.IsValid() && ParsedValue->Type == EJson::Array)
			{
				...
				UE_LOG(LogAxon, Verbose, TEXT("ReadStringArrayParam: recovered stringified JSON array for key '%s' (%d entries)"), Key, Out.Num());
				return Out.Num() > 0;
			}
		}
		...
		TArray<FString>& Warnings)
	{
		...
		for (const auto& Pair : Response->Values)
		{
			...
			{
				ListPayloadKeys.Add(AxonKeyToString(Pair.Key));
			}
		}
		...
		for (const TSharedPtr<FJsonValue>& RowVal : *RowsPtr)
		{
			...
			for (const auto& KVP : RowObj->Values) { ExistingKeys.Add(AxonKeyToString(KVP.Key)); }
		...
		TArray<FString>& Warnings)
	{
		...
		if (!bAnyPathResolved)
		{
			...
			for (const auto& KVP : Response->Values) { TopLevelKeys.Add(AxonKeyToString(KVP.Key)); }
			TopLevelKeys.Sort();
			Warnings.Add(FString::Printf(
		...
		{
			Response->SetField(AxonKeyToString(Pair.Key), Pair.Value);
		}
	}
	...
	TArray<FString>& Warnings)
{
	...
	const bool bHasFields     = AxonResponseShapingDetail::ReadStringArrayParam(Params, TEXT("_fields"),      FieldsSet);
	const bool bHasOmit       = AxonResponseShapingDetail::ReadStringArrayParam(Params, TEXT("_omit"),        OmitSet);
	...
	if (bHasRowFields)
	{
		AxonResponseShapingDetail::ReadStringArrayParam(Params, TEXT("_row_fields"), RowFieldsSet);
	}
	if (bHasPathFields)
	{
		AxonResponseShapingDetail::ReadStringArrayParam(Params, TEXT("_path_fields"), PathFieldsSet);
	}
	...
	if (!Params->TryGetBoolField(TEXT("_compact_json"), bCompact))
	{
		...
		if (Params->TryGetStringField(TEXT("_compact_json"), CompactStr) && !CompactStr.IsEmpty())
		{
			...
			if (bCompact)
			{
				UE_LOG(LogAxon, Verbose, TEXT("ApplyResponseShaping: recovered stringified bool for '_compact_json' ('%s')"), *CompactStr);
			}
		}
	}
	...
	if (bHasFields)
	{
		...
		for (const auto& KVP : Response->Values) { Existing.Add(AxonKeyToString(KVP.Key)); }
	...
	if (bHasRowFields)
	{
		AxonResponseShapingDetail::ApplyRowFieldsFilter(Response, RowFieldsSet, Warnings);
	}
	...
	if (bHasPathFields)
	{
		AxonResponseShapingDetail::ApplyPathFieldsFilter(Response, PathFieldsSet, Warnings);
	}
	...
	if (bCompact)
	{
		...
		for (const FString& K : Existing)
		{
			...
			if (AxonResponseShapingDetail::IsEmptyForCompact(Val))
			{
				Response->RemoveField(K);
			}
		}
