...
// FAxonReflectionWalker implementation. Phase 0 framework primitive.
...
// ---------------------------------------------------------------------------
// Lookup helper. Mirrors AxonBlueprintCDOActions.cpp:385-396 — exact match
...
// ---------------------------------------------------------------------------
FProperty* FAxonReflectionWalker::FindPropertyForwarding(UStruct* Struct, const FString& Name)
{
	if (!Struct)
	{
		return nullptr;
...
// ---------------------------------------------------------------------------
FAxonReflectionWalker::FPathResolveResult FAxonReflectionWalker::ResolvePath(
	UStruct* TopStruct,
	void* Container,
	const FString& Path,
...
// ---------------------------------------------------------------------------
UEnum* FAxonReflectionWalker::RecoverUserDefinedEnum(const FProperty* Prop)
{
#if WITH_EDITOR
	if (!Prop)
	{
...
// ---------------------------------------------------------------------------
bool FAxonReflectionWalker::ResolveUserDefinedEnumToken(const FProperty* Prop, const FString& Token, int64& OutValue)
{
#if WITH_EDITOR
...
// ---------------------------------------------------------------------------
void FAxonReflectionWalker::DispatchByPropertyType(
	FProperty* Prop,
	void* ValuePtr,
	const TSharedPtr<FJsonValue>& JsonVal,
	UObject* Owner,
