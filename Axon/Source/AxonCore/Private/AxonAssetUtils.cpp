...
FString FAxonAssetUtils::ResolveAssetPath(const FString& InPath)
{
	FString Path = InPath;
...
UPackage* FAxonAssetUtils::LoadPackageByPath(const FString& AssetPath)
{
	FString Resolved = ResolveAssetPath(AssetPath);
	UPackage* Package = LoadPackage(nullptr, *Resolved, LOAD_None);
	if (!Package)
	{
		UE_LOG(LogAxon, Warning, TEXT("Failed to load package: %s"), *Resolved);
	}
	return Package;
}

UObject* FAxonAssetUtils::LoadAssetByPath(const FString& AssetPath)
{
...
UObject* FAxonAssetUtils::LoadAssetByPath(UClass* ExpectedClass, const FString& AssetPath)
{
	...
	UE_LOG(LogAxon, Warning, TEXT("Failed to load asset: %s (tried: %s)"), *AssetPath, *NormalizedFull);
	return nullptr;
}

bool FAxonAssetUtils::AssetExists(const FString& AssetPath)
{
	FString Resolved = ResolveAssetPath(AssetPath);
...
TArray<FAssetData> FAxonAssetUtils::GetAssetsByClass(const FTopLevelAssetPath& ClassPath, const FString& PackagePath)
{
	IAssetRegistry& AssetRegistry = FModuleManager::LoadModuleChecked<FAssetRegistryModule>("AssetRegistry").Get();
...
FString FAxonAssetUtils::GetAssetName(const FString& AssetPath)
{
