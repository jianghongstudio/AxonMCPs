...
void FAxonGuideTool::RegisterAll()
{
	...
	Registry.RegisterAction(
		TEXT("axon"), TEXT("guide"),
		TEXT("Editorial cross-namespace workflow guide for AI agents: onboarding script, cross-namespace recipes, X-vs-Y decision matrices, error-to-recovery maps, namespace->Skill pointers, and Axon-specific gotchas. Section-keyed (pass section=\"recipes\") to bound context cost. Hand-authored markdown + a live registry overlay (per-namespace action counts, plugin version)."),
		FAxonActionHandler::CreateStatic(&FAxonGuideTool::HandleGuide),
		Schema
	);
	...
	Registry.SetActionAnnotations(TEXT("axon"), TEXT("guide"),
		/*bReadOnly=*/true, /*bDestructive=*/false, /*bIdempotent=*/true,
		TEXT("Axon usage guide"));
}

bool FAxonGuideTool::LoadGuideMarkdown(FString& OutMarkdown, FString& OutErrorMessage)
{
	...
	// Resolve the plugin base dir, then Docs/axon_guide.md.
	TSharedPtr<IPlugin> Plugin = IPluginManager::Get().FindPlugin(TEXT("Axon"));
	if (!Plugin.IsValid())
	{
		OutErrorMessage = TEXT("Could not resolve the 'Axon' plugin via IPluginManager — the plugin is not mounted. This should be impossible from inside a Axon action; report at github.com/tumourlove/Axon.");
		return false;
	}

	const FString GuidePath = FPaths::Combine(Plugin->GetBaseDir(), TEXT("Docs"), TEXT("axon_guide.md"));

	FString FileContents;
	if (!FFileHelper::LoadFileToString(FileContents, *GuidePath))
	{
		OutErrorMessage = FString::Printf(
			TEXT("Guide markdown not found or unreadable at '%s'. Restore Docs/axon_guide.md (it ships in the release zip) or pull the latest Axon release; then call axon_guide() again."),
			*GuidePath);
		return false;
	}
...
void FAxonGuideTool::SplitSections(const FString& Markdown, TMap<FString, FString>& OutSections, TArray<FString>& OutOrderedNames)
{
	OutSections.Reset();
...
TSharedPtr<FJsonObject> FAxonGuideTool::BuildLiveOverlay()
{
...
FAxonActionResult FAxonGuideTool::HandleGuide(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FAxonActionResult::Error(LoadError, FAxonJsonUtils::ErrInternalError);
	}
