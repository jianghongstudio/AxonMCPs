...
FAxonHttpServer::FAxonHttpServer()
{
}
...
bool FAxonHttpServer::Start(int32 Port)
{
	if (bIsRunning)
	{
		UE_LOG(LogAxon, Warning, TEXT("HTTP server already running on port %d"), BoundPort);
		return true;
	}
	...
	for (int32 Attempt = 1; Attempt <= MaxAttempts; ++Attempt)
	{
		if (Attempt > 1)
		{
			UE_LOG(LogAxon, Warning, TEXT("HTTP bind attempt %d/%d on port %d — waiting %.1fs"),
		...
		if (!HttpRouter.IsValid())
		{
			UE_LOG(LogAxon, Warning, TEXT("GetHttpRouter failed on port %d (attempt %d)"), Port, Attempt);
			continue;
		}
		...
		if (ProbePort(Port))
		{
			...
			UE_LOG(LogAxon, Log, TEXT("Axon MCP server listening on port %d (attempt %d)"), Port, Attempt);
			return true;
		}

		UE_LOG(LogAxon, Warning, TEXT("Port %d not listening after StartAllListeners (attempt %d)"), Port, Attempt);
	}

	UE_LOG(LogAxon, Error, TEXT("Failed to bind Axon MCP server on port %d after %d attempts (~%ds total)"),
...
void FAxonHttpServer::Stop()
{
	...
	bIsRunning = false;
	bHasSeenClient = false;
	LastClientActivity = FDateTime();
	UE_LOG(LogAxon, Log, TEXT("Axon MCP server stopped"));
}

void FAxonHttpServer::NoteClientActivity()
{
	bHasSeenClient = true;
	LastClientActivity = FDateTime::UtcNow();
}

EAxonMcpStatus FAxonHttpServer::GetMcpStatus() const
{
	if (!bIsRunning)
	{
	...
	if (!bHasSeenClient)
	{
		return EAxonMcpStatus::Listening;
	}
	const double Age = (FDateTime::UtcNow() - LastClientActivity).GetTotalSeconds();
	if (Age > ClientActivityTimeoutSeconds)
	{
		return EAxonMcpStatus::Listening;
	}
	return EAxonMcpStatus::Connected;
}

double FAxonHttpServer::GetSecondsSinceLastClientActivity() const
{
	if (!bHasSeenClient)
	{
		return -1.0;
	}
	return (FDateTime::UtcNow() - LastClientActivity).GetTotalSeconds();
}

void FAxonHttpServer::BindRoutes()
{
	...
	RouteHandles.Add(HttpRouter->BindRoute(
		...
		FHttpRequestHandler::CreateRaw(this, &FAxonHttpServer::HandlePostMcp)));

	RouteHandles.Add(HttpRouter->BindRoute(
		FHttpPath(TEXT("/mcp")),
		EHttpServerRequestVerbs::VERB_GET,
		FHttpRequestHandler::CreateRaw(this, &FAxonHttpServer::HandleGetMcp)));

	RouteHandles.Add(HttpRouter->BindRoute(
		FHttpPath(TEXT("/mcp")),
		EHttpServerRequestVerbs::VERB_DELETE,
		FHttpRequestHandler::CreateRaw(this, &FAxonHttpServer::HandleDeleteMcp)));

	RouteHandles.Add(HttpRouter->BindRoute(
		FHttpPath(TEXT("/mcp")),
		EHttpServerRequestVerbs::VERB_OPTIONS,
		FHttpRequestHandler::CreateRaw(this, &FAxonHttpServer::HandleOptions)));

	RouteHandles.Add(HttpRouter->BindRoute(
		FHttpPath(TEXT("/health")),
		EHttpServerRequestVerbs::VERB_GET,
		FHttpRequestHandler::CreateRaw(this, &FAxonHttpServer::HandleHealthCheck)));
...
bool FAxonHttpServer::ProbePort(int32 Port)
{
	...
	FSocket* Socket = SocketSubsystem->CreateSocket(NAME_Stream, TEXT("AxonProbe"), false);
...
bool FAxonHttpServer::Restart(int32 Port)
{
	// Unbind our routes
	if (HttpRouter.IsValid())
	{
...
bool FAxonHttpServer::HandlePostMcp(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
	...
	{
		TSharedPtr<FJsonObject> Err = FAxonJsonUtils::ErrorResponse(
			nullptr, FAxonJsonUtils::ErrParseError, TEXT("Empty request body — send a JSON-RPC 2.0 request, e.g. {\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"tools/list\"}."));
		auto Response = MakeJsonResponse(FAxonJsonUtils::Serialize(Err), EHttpServerResponseCodes::BadRequest);
	...
	// Try parse as JSON
	TSharedPtr<FJsonObject> JsonRequest = FAxonJsonUtils::Parse(BodyString);
	...
	else
	{
		...
		{
			TSharedPtr<FJsonObject> Err = FAxonJsonUtils::ErrorResponse(
				nullptr, FAxonJsonUtils::ErrParseError, TEXT("Invalid JSON — body must be a valid JSON-RPC 2.0 request object or an array of them for batch."));
			auto Response = MakeJsonResponse(FAxonJsonUtils::Serialize(Err), EHttpServerResponseCodes::BadRequest);
			AddCorsHeaders(*Response, Request);
			OnComplete(MoveTemp(Response));
			return true;
	...
	{
		ResponseBody = FAxonJsonUtils::Serialize(Responses[0]);
	}
	else
	{
...
bool FAxonHttpServer::HandleGetMcp(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
...
bool FAxonHttpServer::HandleDeleteMcp(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
	auto Response = FHttpServerResponse::Ok();
	AddCorsHeaders(*Response, Request);
	OnComplete(MoveTemp(Response));
...
bool FAxonHttpServer::HandleOptions(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
	auto Response = FHttpServerResponse::Ok();
	AddCorsHeaders(*Response, Request);
	OnComplete(MoveTemp(Response));
...
bool FAxonHttpServer::HandleHealthCheck(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
	...
	Health->SetStringField(TEXT("version"), AXON_VERSION);
	...
	Health->SetNumberField(TEXT("tools_registered"), FAxonToolRegistry::Get().GetActionCount());
...
TSharedPtr<FJsonObject> FAxonHttpServer::ProcessJsonRpcRequest(const TSharedPtr<FJsonObject>& Request)
{
	if (!Request.IsValid())
	{
		return FAxonJsonUtils::ErrorResponse(nullptr, FAxonJsonUtils::ErrInvalidRequest, TEXT("Invalid request object — must be a JSON object with jsonrpc, method, and id fields."));
	}
	...
	{
		return FAxonJsonUtils::ErrorResponse(nullptr, FAxonJsonUtils::ErrInvalidRequest, TEXT("Missing or invalid jsonrpc version — set \"jsonrpc\" to the string \"2.0\"."));
	}
	...
	{
		return FAxonJsonUtils::ErrorResponse(nullptr, FAxonJsonUtils::ErrInvalidRequest, TEXT("Missing method field — set \"method\" to one of: initialize, tools/list, tools/call, ping."));
	}
	...
	UE_LOG(LogAxon, Verbose, TEXT("JSON-RPC: %s (id=%s)"), *Method, Id.IsValid() ? *Id->AsString() : TEXT("notification"));
	...
	{
		Response = FAxonJsonUtils::ErrorResponse(Id, FAxonJsonUtils::ErrMethodNotFound,
...
TSharedPtr<FJsonObject> FAxonHttpServer::HandleInitialize(const TSharedPtr<FJsonValue>& Id, const TSharedPtr<FJsonObject>& Params)
{
	...
	ServerInfo->SetStringField(TEXT("name"), TEXT("Axon"));
	ServerInfo->SetStringField(TEXT("version"), AXON_VERSION);
	...
	Result->SetStringField(TEXT("instructions"),
		TEXT("Axon MCP server for Unreal Engine. ")
		TEXT("Before calling a domain action, check its schema instead of guessing: ")
		TEXT("axon_discover() lists namespaces, axon_discover('<namespace>') lists a ")
		...
		TEXT("exact parameter schema. axon_guide(section='recipes') gives cross-namespace ")
		TEXT("workflows, decision matrices, and gotchas."));

	return FAxonJsonUtils::SuccessResponse(Id, MakeShared<FJsonValueObject>(Result));
}

TSharedPtr<FJsonObject> FAxonHttpServer::HandleToolsList(const TSharedPtr<FJsonValue>& Id, const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		TArray<FAxonActionInfo> Actions = Registry.GetActions(Namespace);
		...
		if (Namespace == TEXT("axon"))
		{
			// Core tools are individual: axon_discover, axon_status
			for (const FAxonActionInfo& ActionInfo : Actions)
			{
				TSharedPtr<FJsonObject> CoreTool = MakeShared<FJsonObject>();
				CoreTool->SetStringField(TEXT("name"), FString::Printf(TEXT("axon_%s"), *ActionInfo.Action));
		...
		else
		{
			...
			// the list on demand via axon_discover("<namespace>").
			...
			const FString Description = FString::Printf(
				TEXT("Query the %s domain. Call axon_discover(\"%s\") for the action list (name + description); pass detail=true or call describe_query action_schema for an action's full param schema."),
				*Namespace, *Namespace);
			...
			// describe_query action_schema (or axon_discover detail=true).
			...
			ParamsProp->SetStringField(TEXT("description"),
				FString::Printf(TEXT("Parameters for the action. axon_discover(\"%s\") is terse by default (name + description); pass detail=true for full param schemas, or call describe_query action_schema for one action."), *Namespace));
			...
			// FAxonToolRegistry::SetDispatcherAnnotations at module init).
			// Untagged dispatchers leave IsAnyNonDefault()==false → no
			// `annotations` block on the wire.
			const FAxonDispatcherAnnotations DispatcherAnn = Registry.GetDispatcherAnnotations(Namespace);
...
TSharedPtr<FJsonObject> FAxonHttpServer::HandleToolsCall(const TSharedPtr<FJsonValue>& Id, const TSharedPtr<FJsonObject>& Params)
{
	if (!Params.IsValid())
	{
		return FAxonJsonUtils::ErrorResponse(Id, FAxonJsonUtils::ErrInvalidParams, TEXT("Missing params — tools/call params must include \"name\" and optionally \"arguments\"."));
	}

	FString ToolName;
	if (!Params->TryGetStringField(TEXT("name"), ToolName))
	{
		return FAxonJsonUtils::ErrorResponse(Id, FAxonJsonUtils::ErrInvalidParams, TEXT("Missing tool name — set params.name to a tool like axon_discover or <namespace>_query."));
	}
	...
	if (ToolName.StartsWith(TEXT("axon_")))
	{
		// Core tool: axon_discover -> namespace="axon", action="discover"
		Namespace = TEXT("axon");
		Action = ToolName.Mid(5);
	...
	else if (ToolName.EndsWith(TEXT("_query")) || ToolName.EndsWith(TEXT(".query")))
	{
		...
		{
			return FAxonJsonUtils::ErrorResponse(Id, FAxonJsonUtils::ErrInvalidParams,
				TEXT("Missing 'action' in arguments — for *_query tools, set arguments.action; call axon_discover(\"<namespace>\") to enumerate."));
		}
	...
	{
		return FAxonJsonUtils::ErrorResponse(Id, FAxonJsonUtils::ErrMethodNotFound,
			FString::Printf(TEXT("Unknown tool: %s — tool must start with Axon_ or end with _query; call tools/list to enumerate."), *ToolName));
	}
	...
	FAxonActionResult ActionResult = FAxonToolRegistry::Get().ExecuteAction(Namespace, Action, Arguments);

	// Calculate duration
	double DurationMs = (FPlatformTime::Seconds() - ActionStartTimeSeconds) * 1000.0;
	UE_LOG(LogAxon, Verbose, TEXT("Axon action %s.%s completed in %.2f ms"), *Namespace, *Action, DurationMs);
	...
	if (ActionResult.bSuccess)
	{
		...
		{
			TextContent->SetStringField(TEXT("text"), FAxonJsonUtils::Serialize(ActionResult.Result));
		}
		else
		{
...
TSharedPtr<FJsonObject> FAxonHttpServer::HandlePing(const TSharedPtr<FJsonValue>& Id)
{
	return FAxonJsonUtils::SuccessResponse(Id, MakeShared<FJsonValueObject>(MakeShared<FJsonObject>()));
}
...
TUniquePtr<FHttpServerResponse> FAxonHttpServer::MakeJsonResponse(const FString& JsonBody, EHttpServerResponseCodes Code)
{
...
TUniquePtr<FHttpServerResponse> FAxonHttpServer::MakeSseResponse(const TArray<TSharedPtr<FJsonObject>>& Messages)
{
	FString SseBody;
	for (const TSharedPtr<FJsonObject>& Msg : Messages)
	{
		SseBody += TEXT("event: message\ndata: ");
		SseBody += FAxonJsonUtils::Serialize(Msg);
		SseBody += TEXT("\n\n");
	}
...
void FAxonHttpServer::AddCorsHeaders(FHttpServerResponse& Response, const FHttpServerRequest& Request)
{
