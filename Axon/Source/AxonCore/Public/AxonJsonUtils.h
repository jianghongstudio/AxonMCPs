...
AXONCORE_API DECLARE_LOG_CATEGORY_EXTERN(LogAxon, Log, All);
 ...
 *
 * `AxonKeyToString(Pair.Key)` yields an `FString` from either key type, keeping
 ...
 */
FORCEINLINE const FString& AxonKeyToString(const FString& Key) { return Key; }
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 8
FORCEINLINE FString AxonKeyToString(const UE::FSharedString& Key) { return FString(*Key); }
#endif
 ...
 */
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 8
FORCEINLINE UE::FSharedString AxonMakeJsonKey(const FString& Key) { return UE::FSharedString(*Key); }
#else
FORCEINLINE const FString& AxonMakeJsonKey(const FString& Key) { return Key; }
#endif

class AXONCORE_API FAxonJsonUtils
{
public:
	...
	// --- Axon server-defined error codes (JSON-RPC -32000..-32099 range) ---
	//
	...
	/** Scaffold stub / not yet implemented action handler. */
 ...
 */
AXONCORE_API void ApplyResponseShaping(
