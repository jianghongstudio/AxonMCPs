 ...
 *
 * Lives in AxonCore (Public) so module-internal readers in either AxonAnimation
 * (anim-instance sampling) or AxonEditor (PIE object property read + function-arg
 * marshalling) share one resolver. Exported via AXONCORE_API.
 */
namespace AxonStructField
{
	 ...
	 */
	AXONCORE_API FResolved Resolve(const UObject* Container, const FString& Path);
}
