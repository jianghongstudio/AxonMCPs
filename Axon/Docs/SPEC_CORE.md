...
Domain extensions are sibling plugins. In-tree companion module `AxonEditor` (same `Axon.uplugin`) owns PIE timeseries/session actions; keep domain work (animation, etc.) out of Core.
...
| `axon_create_extension` | scaffold sibling plugin (requires actions or `skeleton_only`) |
