...
DEFINE_LOG_CATEGORY_STATIC(LogAxonAnimRuntime, Log, All);
...
namespace
{
	...
	FAnimPIELookup FindAnimInstanceInPIE(const TSharedPtr<FJsonObject>& Params)
	{
		...
		{
			Result.Error = FAxonActionResult::Error(TEXT("Missing required param 'actor'"));
			return Result;
		}
		...
		{
			Result.Error = FAxonActionResult::Error(TEXT("PIE not running — start Play-In-Editor first"));
			return Result;
		}
		...
		if (!Result.Actor)
		{
			Result.Error = FAxonActionResult::Error(FString::Printf(TEXT("Actor '%s' not found in PIE world"), *ActorName));
			return Result;
		}
		...
		if (!Result.MeshComp)
		{
			Result.Error = FAxonActionResult::Error(FString::Printf(
	...
	TSharedPtr<FJsonValue> ReadPropertyValue(UObject* Obj, const FString& VarName, FString& OutTypeName)
	{
		...
		const AxonStructField::FResolved Resolved = AxonStructField::Resolve(Obj, VarName);
		if (!Resolved.Leaf)
		{
			OutTypeName = TEXT("<not found>");
			return nullptr;
		...
		const FParityTolerance& Tol, const TSharedPtr<FJsonObject>& OutEntry)
	{
		...
		const AxonStructField::FResolved ResA = AxonStructField::Resolve(A, VarName);
		const AxonStructField::FResolved ResB = AxonStructField::Resolve(B, VarName);
...
void FAxonAnimationRuntimeActions::RegisterActions(FAxonToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("animation"), TEXT("sample_pie_anim_instance"),
		...
		FAxonActionHandler::CreateStatic(&HandleSamplePIEAnimInstance),
		FParamSchemaBuilder()
	...
	UE_LOG(LogAxonAnimRuntime, Log, TEXT("AxonAnimation Runtime: registered 1 action"));
}

// ── sample_pie_anim_instance ────────────────────────────────────────

FAxonActionResult FAxonAnimationRuntimeActions::HandleSamplePIEAnimInstance(const TSharedPtr<FJsonObject>& Params)
{
