...
bool FAxonSkeletonRetargetActions::ParseTranslationRetargetMode(
		...
		const UScriptStruct* ConcreteType, const void* StructMemory, const UObject* Owner)
	{
		...
		for (TFieldIterator<FProperty> It(ConcreteType); It; ++It)
		{
			...
			Out->SetField(Prop->GetName(),
				FAxonReflectionReader::PropertyToJsonValue(Prop, ValuePtr, Owner));
		}
		return Out;
	}
...
void FAxonSkeletonRetargetActions::RegisterActions(FAxonToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("animation"), TEXT("set_bone_translation_retargeting"),
		...
		FAxonActionHandler::CreateStatic(&HandleSetBoneTranslationRetargeting),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("get_bone_translation_retargeting"),
		TEXT("Read the current per-bone translation retargeting mode for a Skeleton. Returns one entry "
			 ...
			 "those bones."),
		FAxonActionHandler::CreateStatic(&HandleGetBoneTranslationRetargeting),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("skeleton_path"), TEXT("Skeleton asset path"))
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("set_ik_rig_bone_settings"),
		...
		FAxonActionHandler::CreateStatic(&HandleSetIkRigBoneSettings),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("get_ik_rig_bone_settings"),
		...
		FAxonActionHandler::CreateStatic(&HandleGetIkRigBoneSettings),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("ik_rig_path"), TEXT("IK Rig (UIKRigDefinition) asset path"))
...
FAxonActionResult FAxonSkeletonRetargetActions::HandleSetBoneTranslationRetargeting(
	const TSharedPtr<FJsonObject>& Params)
{
	...
	if (Params->TryGetArrayField(TEXT("entries"), EntriesArr))
	{
		for (const TSharedPtr<FJsonValue>& Val : *EntriesArr)
		{
			...
			{
				return FAxonActionResult::Error(TEXT("Each item in 'entries' must be an object {bone, mode}."));
			}

			FString BoneName;
			if (!(*EntryObj)->TryGetStringField(TEXT("bone"), BoneName) || BoneName.IsEmpty())
			{
				return FAxonActionResult::Error(TEXT("Each entry requires a non-empty 'bone' field."));
			}
...
FAxonActionResult FAxonSkeletonRetargetActions::HandleGetBoneTranslationRetargeting(
	const TSharedPtr<FJsonObject>& Params)
{
	const FString SkeletonPath = Params->GetStringField(TEXT("skeleton_path"));
...
FAxonActionResult FAxonSkeletonRetargetActions::HandleSetIkRigBoneSettings(
	const TSharedPtr<FJsonObject>& Params)
{
	const FString IkRigPath = Params->GetStringField(TEXT("ik_rig_path"));

	UIKRigDefinition* IkRig = FAxonAssetUtils::LoadAssetByPath<UIKRigDefinition>(IkRigPath);
	if (!IkRig)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("IK Rig not found: %s"), *IkRigPath));
	}

	FString BoneNameStr;
	if (!Params->TryGetStringField(TEXT("bone_name"), BoneNameStr) || BoneNameStr.IsEmpty())
	{
		return FAxonActionResult::Error(TEXT("'bone_name' is required and must be non-empty."));
	}
	...
	{
		return FAxonActionResult::Error(TEXT("'settings' is required and must be an object of {field: value} pairs."));
	}
	const TSharedPtr<FJsonObject>& SettingsObj = *SettingsObjPtr;
	if (SettingsObj->Values.Num() == 0)
	{
		return FAxonActionResult::Error(TEXT("'settings' must contain at least one field to set."));
	}
	...
	for (int32 SolverIndex = 0; SolverIndex < NumSolvers; ++SolverIndex)
	{
		...
		for (const TPair<FString, TSharedPtr<FJsonValue>>& Pair : SettingsObj->Values)
		{
			...
			AppliedObj->SetField(TEXT("value"),
				FAxonReflectionReader::PropertyToJsonValue(Prop, ValuePtr, IkRig));
...
FAxonActionResult FAxonSkeletonRetargetActions::HandleGetIkRigBoneSettings(
	const TSharedPtr<FJsonObject>& Params)
{
	const FString IkRigPath = Params->GetStringField(TEXT("ik_rig_path"));
