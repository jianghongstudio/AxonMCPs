...
void FAxonControlRigWriteActions::RegisterActions(FAxonToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("get_control_rig_graph"),
		...
		FAxonActionHandler::CreateStatic(&HandleGetControlRigGraph),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("add_control_rig_node"),
		...
		FAxonActionHandler::CreateStatic(&HandleAddControlRigNode),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("ControlRigBlueprint asset path"))
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("connect_control_rig_pins"),
		...
		FAxonActionHandler::CreateStatic(&HandleConnectControlRigPins),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("ControlRigBlueprint asset path"))
...
FAxonActionResult FAxonControlRigWriteActions::HandleGetControlRigGraph(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!CRB) return FAxonActionResult::Error(Error);

	URigVMGraph* Graph = GetGraphFromBlueprint(CRB, GraphName, Error);
	if (!Graph) return FAxonActionResult::Error(Error);
...
FAxonActionResult FAxonControlRigWriteActions::HandleAddControlRigNode(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	FString StructPath = Params->GetStringField(TEXT("struct_path"));

	if (StructPath.IsEmpty())
		return FAxonActionResult::Error(TEXT("struct_path is required"));
	...
	URigVMGraph* Graph = CRB->GetDefaultModel();
	if (!Graph) return FAxonActionResult::Error(TEXT("Control Rig has no default graph"));
	...
	if (!NewNode)
	{
		...
		return FAxonActionResult::Error(FString::Printf(TEXT("Failed to add unit node from struct path: %s"), *StructPath));
	}
	...
	if (Params->TryGetObjectField(TEXT("pin_defaults"), PinDefaultsObj) && PinDefaultsObj && (*PinDefaultsObj)->Values.Num() > 0)
	{
		for (const auto& KV : (*PinDefaultsObj)->Values)
		{
			...
			else
			{
				...
				FJsonSerializer::Serialize(KV.Value, AxonKeyToString(KV.Key), Writer);
				Value = JsonStr;
			}
			...
			if (!bSetOk)
			{
				UE_LOG(LogTemp, Warning, TEXT("Axon: Failed to set pin default %s = %s"), *PinPath, *Value);
			}
		}
	}
...
FAxonActionResult FAxonControlRigWriteActions::HandleConnectControlRigPins(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	...
	if (SourcePin.IsEmpty() || TargetPin.IsEmpty())
		return FAxonActionResult::Error(TEXT("source_pin and target_pin are required"));
