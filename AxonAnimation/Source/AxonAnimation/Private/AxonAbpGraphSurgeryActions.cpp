...
// Resolvers shared with AxonAbpWriteActions.cpp (same module, internal linkage there).
...
// ---------------------------------------------------------------------------
// Compile-with-error-harvest (mirrors AxonBlueprintCompileActions.cpp:98-144)
// ---------------------------------------------------------------------------
...
void FAxonAbpGraphSurgeryActions::RegisterActions(FAxonToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("rebuild_evaluate_chooser_node"),
		...
		FAxonActionHandler::CreateStatic(&HandleRebuildEvaluateChooserNode),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("replace_evaluate_chooser_nodes"),
		...
		FAxonActionHandler::CreateStatic(&HandleReplaceEvaluateChooserNodes),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("anim_blueprint"), TEXT("Animation Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("add_evaluate_chooser_node"),
		...
		FAxonActionHandler::CreateStatic(&HandleAddEvaluateChooserNode),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("wire_chooser_to_motion_matching"),
		...
		FAxonActionHandler::CreateStatic(&HandleWireChooserToMotionMatching),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("abp_path"), TEXT("Animation Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("bind_chooser_database_via_threadsafe"),
		...
		FAxonActionHandler::CreateStatic(&HandleBindChooserDatabaseViaThreadSafe),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("abp_path"), TEXT("Animation Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("bind_threadsafe_update_function"),
		...
		FAxonActionHandler::CreateStatic(&HandleBindThreadsafeUpdateFunction),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("abp_path"), TEXT("Animation Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("duplicate_reparent_and_sanitize"),
		...
		FAxonActionHandler::CreateStatic(&HandleDuplicateReparentAndSanitize),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("source_abp"), TEXT("Source Animation Blueprint asset path"))
			.RequiredAssetPath(TEXT("destination_path"), TEXT("Destination ABP asset path for the duplicate (e.g. /Game/Tests/Axon/ABP_Copy)"))
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("find_node_slice"),
		...
		FAxonActionHandler::CreateStatic(&HandleFindNodeSlice),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("anim_blueprint"), TEXT("Animation Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("remove_node_slice"),
		...
		FAxonActionHandler::CreateStatic(&HandleRemoveNodeSlice),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("anim_blueprint"), TEXT("Animation Blueprint asset path"))
			.Required(TEXT("graph_name"), TEXT("string"), TEXT("Graph containing the seed node"))
			.Required(TEXT("seed_node_ref"), TEXT("string"), TEXT("UObject name of the seed node"))
...
FAxonActionResult FAxonAbpGraphSurgeryActions::HandleFindNodeSlice(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!ABP)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Animation Blueprint not found: %s"), *AssetPath));
	}
	...
	if (!Graph)
	{
		return FAxonActionResult::Error(GraphErr);
	}

	UEdGraphNode* Seed = FindNodeByNameBP(ABP, SeedRef, Graph);
	if (!Seed)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Seed node '%s' not found in graph '%s'"), *SeedRef, *GraphName));
	}
...
FAxonActionResult FAxonAbpGraphSurgeryActions::HandleRemoveNodeSlice(const TSharedPtr<FJsonObject>& Params)
{
	const FString AssetPath = Params->GetStringField(TEXT("anim_blueprint"));
	const FString GraphName = Params->GetStringField(TEXT("graph_name"));
	const FString SeedRef = Params->GetStringField(TEXT("seed_node_ref"));
...
FAxonActionResult FAxonAbpGraphSurgeryActions::HandleDuplicateReparentAndSanitize(const TSharedPtr<FJsonObject>& Params)
{
	...
	UAnimBlueprint* SourceABP = FAxonAssetUtils::LoadAssetByPath<UAnimBlueprint>(SourcePath);
	if (!SourceABP)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Source Animation Blueprint not found: %s"), *SourcePath));
	}
	...
	if (!NewParent)
	{
		return FAxonActionResult::Error(ParentErr);
	}
	...
	if (!bDryRun)
	{
		...
		{
			return FAxonActionResult::Error(FString::Printf(TEXT("Destination already exists: %s"), *DestPath));
		}
		UObject* Dup = UEditorAssetLibrary::DuplicateAsset(SourcePath, DestPath);
		UAnimBlueprint* DupABP = Cast<UAnimBlueprint>(Dup);
		if (!DupABP)
		{
			return FAxonActionResult::Error(FString::Printf(TEXT("DuplicateAsset failed: %s -> %s"), *SourcePath, *DestPath));
		}
...
FAxonActionResult FAxonAbpGraphSurgeryActions::HandleRebuildEvaluateChooserNode(const TSharedPtr<FJsonObject>& Params)
{
	const FString AssetPath = Params->GetStringField(TEXT("anim_blueprint"));
	const FString GraphName = Params->GetStringField(TEXT("graph_name"));
	...
	UChooserTable* Chooser = FAxonAssetUtils::LoadAssetByPath<UChooserTable>(ChooserPath);
	if (!Chooser)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Chooser table not found: %s"), *ChooserPath));
	}
	...
	if (!OldNode)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Node '%s' not found in graph '%s'"), *NodeRef, *GraphName));
	}
...
FAxonActionResult FAxonAbpGraphSurgeryActions::HandleReplaceEvaluateChooserNodes(const TSharedPtr<FJsonObject>& Params)
{
	const FString AssetPath = Params->GetStringField(TEXT("anim_blueprint"));
	bool bDryRun = true;
	Params->TryGetBoolField(TEXT("dry_run"), bDryRun);
	...
	{
		return FAxonActionResult::Error(TEXT("replacements array is required"));
	}
...
FAxonActionResult FAxonAbpGraphSurgeryActions::HandleAddEvaluateChooserNode(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!ABP)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Animation Blueprint not found: %s"), *AbpPath));
	}
	...
	if (!NewNode)
	{
		return FAxonActionResult::Error(TEXT("Reflective UK2Node_EvaluateChooser2 spawn failed (no 'Chooser' FObjectProperty or node could not be created)."));
	}
...
FAxonActionResult FAxonAbpGraphSurgeryActions::HandleWireChooserToMotionMatching(const TSharedPtr<FJsonObject>& Params)
{
	const FString AbpPath        = Params->GetStringField(TEXT("abp_path"));
	...
	if (!ChooserNode)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Chooser node '%s' not found in graph '%s'"), *ChooserNodeRef, *GraphName));
	}
	UEdGraphNode* MMNode = FindNodeByNameBP(ABP, MMNodeRef, Graph);
	if (!MMNode)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Motion Matching node '%s' not found in graph '%s'"), *MMNodeRef, *GraphName));
	}
	...
	if (!SourcePin)
	{
		return FAxonActionResult::Error(TEXT("Chooser node has no output result pin to wire"));
	}
	...
	if (!TargetPin)
	{
		return FAxonActionResult::Error(TEXT("Motion Matching node has no 'Database' input pin. Ensure the node is a UAnimGraphNode_MotionMatching with the Database pin shown."));
	}

	const UEdGraphSchema* Schema = Graph->GetSchema();
	if (!Schema)
	{
		return FAxonActionResult::Error(TEXT("Graph has no schema"));
	}
...
FAxonActionResult FAxonAbpGraphSurgeryActions::HandleBindChooserDatabaseViaThreadSafe(const TSharedPtr<FJsonObject>& Params)
{
	const FString AbpPath     = Params->GetStringField(TEXT("abp_path"));
	const FString ChooserPath = Params->GetStringField(TEXT("chooser_path"));
	const FString MMNodeRef   = Params->GetStringField(TEXT("mm_node"));
	...
	if (DbVar.IsEmpty()) return FAxonActionResult::Error(TEXT("selected_database_var is required"));
	if (MMNodeRef.IsEmpty()) return FAxonActionResult::Error(TEXT("mm_node is required"));
	...
	if (!FuncGraph)
	{
		...
		if (!FuncGraph)
		{
			return FAxonActionResult::Error(FString::Printf(TEXT("Failed to create function graph: %s"), *FuncName));
		}
	...
	if (FObjectProperty* ChooserProp = FindFProperty<FObjectProperty>(EvalClass, TEXT("Chooser")))
	{
		...
		{
			return FAxonActionResult::Error(TEXT("Chooser binding is null after spawn — node would be pruned. Aborting."));
		}
	}

	const UEdGraphSchema* Schema = FuncGraph->GetSchema();
	if (!Schema) return FAxonActionResult::Error(TEXT("Function graph has no schema"));
...
FAxonActionResult FAxonAbpGraphSurgeryActions::HandleRebuildEvaluateChooserNode(const TSharedPtr<FJsonObject>& /*Params*/)
{
	return FAxonActionResult::Error(TEXT("Chooser plugin not available (WITH_CHOOSER=0). Evaluate-Chooser node surgery is disabled in this build."));
}
...
FAxonActionResult FAxonAbpGraphSurgeryActions::HandleAddEvaluateChooserNode(const TSharedPtr<FJsonObject>& /*Params*/)
{
	return FAxonActionResult::Error(TEXT("Chooser plugin not available (WITH_CHOOSER=0). add_evaluate_chooser_node is disabled in this build."));
}

FAxonActionResult FAxonAbpGraphSurgeryActions::HandleWireChooserToMotionMatching(const TSharedPtr<FJsonObject>& /*Params*/)
{
	return FAxonActionResult::Error(TEXT("Chooser plugin not available (WITH_CHOOSER=0). wire_chooser_to_motion_matching is disabled in this build."));
}

FAxonActionResult FAxonAbpGraphSurgeryActions::HandleBindChooserDatabaseViaThreadSafe(const TSharedPtr<FJsonObject>& /*Params*/)
{
	return FAxonActionResult::Error(TEXT("Chooser plugin not available (WITH_CHOOSER=0). bind_chooser_database_via_threadsafe is disabled in this build."));
}
 ...
 * Mirrors the established Axon resolution order: TryFindTypeSlow (path or short name) ->
...
FAxonActionResult FAxonAbpGraphSurgeryActions::HandleBindThreadsafeUpdateFunction(const TSharedPtr<FJsonObject>& Params)
{
	const FString AbpPath = Params->GetStringField(TEXT("abp_path"));
	...
	{
		return FAxonActionResult::Error(TEXT("function is required and must be an object { class, name }"));
	}
	...
	{
		return FAxonActionResult::Error(TEXT("function.class and function.name are both required"));
	}
	...
	{
		return FAxonActionResult::Error(TEXT("result_target is required and must be an object { var, [node], [pin] }"));
	}
	...
	{
		return FAxonActionResult::Error(TEXT("result_target.var is required (the self variable to store the call result into; it must already exist on the ABP)"));
	}
	...
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Unsupported function signature for a thread-safe call: %s"), *GuardError));
	}
	...
	if (!FuncGraph)
	{
		...
		if (!FuncGraph)
		{
			return FAxonActionResult::Error(FString::Printf(TEXT("Failed to create function graph: %s"), *FuncGraphName));
		}
		FBlueprintEditorUtils::AddFunctionGraph<UClass>(ABP, FuncGraph, /*bIsUserCreated=*/true, /*SignatureFromObject=*/(UClass*)nullptr);
		bFuncCreated = true;
	}
	...
	if (!EntryNode)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("No function entry node in graph '%s'"), *FuncGraphName));
	}
	EntryNode->Modify();
	EntryNode->MetaData.bThreadSafe = true;
	...
	if (Params->TryGetArrayField(TEXT("arg_bindings"), ArgBindings) && ArgBindings)
	{
		for (const TSharedPtr<FJsonValue>& V : *ArgBindings)
		{
			...
			if (!V->TryGetObject(EntryPtr) || !EntryPtr)
			{
				return FAxonActionResult::Error(TEXT("each arg_bindings entry must be an object { pin, value } or { pin, self }"));
			}
			const TSharedPtr<FJsonObject>& Entry = *EntryPtr;
			FString PinName;
			...
			if (PinName.IsEmpty())
			{
				return FAxonActionResult::Error(TEXT("arg_bindings entry missing required 'pin'"));
			}
			...
			{
				return FAxonActionResult::Error(FString::Printf(TEXT("arg_bindings pin '%s' is an exec pin — only data input pins are bindable"), *PinName));
			}
			...
			if (bWantSelf)
			{
				...
				{
					return FAxonActionResult::Error(FString::Printf(TEXT("failed to wire Self into arg pin '%s'"), *PinName));
				}
				++Wired;
			}
			else
