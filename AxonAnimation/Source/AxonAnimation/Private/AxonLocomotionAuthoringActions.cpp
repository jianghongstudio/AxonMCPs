...
void FAxonLocomotionAuthoringActions::RegisterActions(FAxonToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("get_root_motion_speed"),
		...
		FAxonActionHandler::CreateStatic(&HandleGetRootMotionSpeed),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("bake_distance_curve"),
		...
		FAxonActionHandler::CreateStatic(&HandleBakeDistanceCurve),
		FParamSchemaBuilder()
...
namespace
{
	 ...
	 *  reflective resolution (AxonAnimationActions.cpp:4300). Returns nullptr if the plugin is not present. */
...
FAxonActionResult FAxonLocomotionAuthoringActions::HandleGetRootMotionSpeed(const TSharedPtr<FJsonObject>& Params)
{
	const FString AnimPath = Params->GetStringField(TEXT("anim_path"));

	UAnimSequence* Seq = FAxonAssetUtils::LoadAssetByPath<UAnimSequence>(AnimPath);
	if (!Seq)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("AnimSequence not found: %s"), *AnimPath));
	}
...
FAxonActionResult FAxonLocomotionAuthoringActions::HandleBakeDistanceCurve(const TSharedPtr<FJsonObject>& Params)
{
	const FString AnimPath = Params->GetStringField(TEXT("anim_path"));
	...
	if (!UAnimationModifiersAssetUserData::AddAnimationModifierOfClass(Seq, ModifierClass))
	{
		...
		return FAxonActionResult::Error(TEXT("Failed to register DistanceCurveModifier into the AnimationModifiers stack"));
	}
	...
	{
		GEditor->EndTransaction();
		return FAxonActionResult::Error(TEXT("AnimationModifiersAssetUserData missing after AddAnimationModifierOfClass"));
	}
	...
	{
		GEditor->EndTransaction();
		return FAxonActionResult::Error(TEXT("DistanceCurveModifier instance not found in stack after registration"));
	}
