...
void FAxonMirrorTableActions::RegisterActions(FAxonToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("create_mirror_data_table"),
		...
		FAxonActionHandler::CreateStatic(&HandleCreateMirrorDataTable),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("set_schema_mirror_data_table"),
		...
		FAxonActionHandler::CreateStatic(&HandleSetSchemaMirrorDataTable),
		FParamSchemaBuilder()
...
/** Access the private 'Skeletons' UPROPERTY on UPoseSearchSchema via reflection (mirror of the
  * Channels reflection helper in AxonPoseSearchActions.cpp). Returns nullptr on failure. */
...
FAxonActionResult FAxonMirrorTableActions::HandleCreateMirrorDataTable(const TSharedPtr<FJsonObject>& Params)
{
	...
	// FindReplaceMirroredNames() needs a valid RowStruct; the engine factory uses FMirrorTableRow.
...
FAxonActionResult FAxonMirrorTableActions::HandleSetSchemaMirrorDataTable(const TSharedPtr<FJsonObject>& Params)
{
	...
	UMirrorDataTable* MDT = FAxonAssetUtils::LoadAssetByPath<UMirrorDataTable>(MirrorTablePath);
	if (!MDT)
		return FAxonActionResult::Error(FString::Printf(TEXT("UMirrorDataTable not found: %s"), *MirrorTablePath));

	TArray<FPoseSearchRoledSkeleton>* Skeletons = GetSchemaSkeletonsMutable(Schema);
	if (!Skeletons)
		return FAxonActionResult::Error(TEXT("Failed to access Skeletons property on schema via reflection"));
