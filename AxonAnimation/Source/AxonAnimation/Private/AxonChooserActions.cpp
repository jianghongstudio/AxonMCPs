...
// DuplicateAsset (EditorScriptingUtilities — already a AxonAnimation dep).
...
void FAxonChooserActions::RegisterActions(FAxonToolRegistry& Registry)
{
	// --- inspect_chooser ---
	Registry.RegisterAction(TEXT("chooser"), TEXT("inspect_chooser"),
		...
		FAxonActionHandler::CreateStatic(&HandleInspectChooser),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("chooser"), TEXT("duplicate_chooser_tree"),
		...
		FAxonActionHandler::CreateStatic(&HandleDuplicateChooserTree),
		FParamSchemaBuilder()
			.Required(TEXT("source_assets"), TEXT("array"), TEXT("Array of source UChooserTable asset paths to duplicate"))
			.Required(TEXT("destination_folder"), TEXT("string"), TEXT("Destination content folder, e.g. /Game/Tests/Axon"))
	...
	Registry.RegisterAction(TEXT("chooser"), TEXT("set_context_object_class"),
		...
		FAxonActionHandler::CreateStatic(&HandleSetContextObjectClass),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("UChooserTable asset path"))
	...
	Registry.RegisterAction(TEXT("chooser"), TEXT("set_result_asset_reference"),
		...
		FAxonActionHandler::CreateStatic(&HandleSetResultAssetReference),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("UChooserTable asset path"))
	...
	Registry.RegisterAction(TEXT("chooser"), TEXT("set_evaluate_chooser_result_reference"),
		...
		FAxonActionHandler::CreateStatic(&HandleSetEvaluateChooserResultReference),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("chooser"), TEXT("validate_chooser"),
		...
		FAxonActionHandler::CreateStatic(&HandleValidateChooser),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("UChooserTable asset path"))
...
namespace
{
	FAxonActionResult ChooserUnavailable()
	{
		return FAxonActionResult::Error(TEXT("Chooser plugin not available"));
	}
}

FAxonActionResult FAxonChooserActions::HandleInspectChooser(const TSharedPtr<FJsonObject>&)        { return ChooserUnavailable(); }
FAxonActionResult FAxonChooserActions::HandleDuplicateChooserTree(const TSharedPtr<FJsonObject>&)  { return ChooserUnavailable(); }
FAxonActionResult FAxonChooserActions::HandleSetContextObjectClass(const TSharedPtr<FJsonObject>&) { return ChooserUnavailable(); }
FAxonActionResult FAxonChooserActions::HandleSetResultAssetReference(const TSharedPtr<FJsonObject>&){ return ChooserUnavailable(); }
FAxonActionResult FAxonChooserActions::HandleSetEvaluateChooserResultReference(const TSharedPtr<FJsonObject>&){ return ChooserUnavailable(); }
FAxonActionResult FAxonChooserActions::HandleValidateChooser(const TSharedPtr<FJsonObject>&)       { return ChooserUnavailable(); }
...
namespace
{
	...
	FString MakeDestAssetPath(const FString& DestFolder, const FString& SourceAssetPath)
	{
		...
		const FString ShortName = FAxonAssetUtils::GetAssetName(SourceAssetPath);
	 ...
	 * Mirrors ApplyInputBinding (AxonChooserAuthoringActions.cpp) const-side: reach the
...
FAxonActionResult FAxonChooserActions::HandleInspectChooser(const TSharedPtr<FJsonObject>& Params)
{
	const FString AssetPath = Params->GetStringField(TEXT("asset_path"));

	UChooserTable* Table = FAxonAssetUtils::LoadAssetByPath<UChooserTable>(AssetPath);
	if (!Table)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("ChooserTable not found: %s"), *AssetPath));
	}
	...
	if (bRecursive)
	{
		...
		Root->SetObjectField(TEXT("tree"), AxonChooserTree::CollectTree(Table, VisitedTables));
	}
...
FAxonActionResult FAxonChooserActions::HandleDuplicateChooserTree(const TSharedPtr<FJsonObject>& Params)
{
	const TArray<TSharedPtr<FJsonValue>>* SourcesPtr = nullptr;
	if (!Params->TryGetArrayField(TEXT("source_assets"), SourcesPtr) || !SourcesPtr)
	{
		return FAxonActionResult::Error(TEXT("Missing required array parameter: source_assets"));
	}
	const FString DestFolder = Params->GetStringField(TEXT("destination_folder"));
	if (DestFolder.IsEmpty())
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: destination_folder"));
	}
	...
	if (Remap.Num() > 0)
	{
				...
				(FInstancedStruct& S, int32 RowIdx, const FString& Where)
			{
				if (FAssetChooser* Hard = S.GetMutablePtr<FAssetChooser>())
				{
					if (Hard->Asset)
					{
						...
						if (NewRef != OldRef)
						{
							if (UObject* Loaded = FAxonAssetUtils::LoadAssetByPath(NewRef))
							{
								Hard->Asset = Loaded;
								++RefsRemapped;
				...
				else if (FEvaluateChooser* Eval = S.GetMutablePtr<FEvaluateChooser>())
				{
					...
					if (Eval->Chooser)
					{
						...
						if (NewRef != OldRef)
						{
							if (UChooserTable* NewChild = FAxonAssetUtils::LoadAssetByPath<UChooserTable>(NewRef))
							{
		...
		for (FDupRecord& Rec : DupRecords)
		{
			...
			if (DupTable->ParentTable)
			{
				...
				if (UChooserTable* P = FAxonAssetUtils::LoadAssetByPath<UChooserTable>(NewParent))
				{
			...
			if (DupTable->RootChooser)
			{
				...
				if (UChooserTable* R = FAxonAssetUtils::LoadAssetByPath<UChooserTable>(NewRoot))
				{
...
FAxonActionResult FAxonChooserActions::HandleSetContextObjectClass(const TSharedPtr<FJsonObject>& Params)
{
	const FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	...
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: class_path"));
	}
	...
	if (!NewClass)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Could not resolve class: %s"), *ClassPath));
	}
	...
	if (!Owner)
	{
		return FAxonActionResult::Error(TEXT("ChooserTable has no context owner"));
	}
	...
	else
	{
		...
		if (!ClassEntry)
		{
			return FAxonActionResult::Error(TEXT("No class-typed context parameter (FContextObjectTypeClass) found on this chooser"));
		}
	}
...
FAxonActionResult FAxonChooserActions::HandleSetResultAssetReference(const TSharedPtr<FJsonObject>& Params)
{
	const FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	...
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: row_or_column"));
	}
	const int32 Row = static_cast<int32>(RowVal);

	if (NewAssetPath.IsEmpty())
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: asset_path_value"));
	}
	...
	if (FAssetChooser* Hard = RowStruct.GetMutablePtr<FAssetChooser>())
	{
		...
		UObject* Loaded = FAxonAssetUtils::LoadAssetByPath(NewAssetPath);
		if (!Loaded)
		{
			return FAxonActionResult::Error(FString::Printf(TEXT("Could not load asset for hard reference: %s"), *NewAssetPath));
		}
		Hard->Asset = Loaded;
	}
	...
	return FAxonActionResult::Success(Root);
#else
	return FAxonActionResult::Error(TEXT("ResultsStructs is editor-only data; not available in this build"));
#endif
}
...
FAxonActionResult FAxonChooserActions::HandleSetEvaluateChooserResultReference(const TSharedPtr<FJsonObject>& Params)
{
	const FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	...
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: row"));
	}
	const int32 Row = static_cast<int32>(RowVal);

	if (ChildChooserPath.IsEmpty())
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: child_chooser_path"));
	}
	...
	UChooserTable* ChildTable = FAxonAssetUtils::LoadAssetByPath<UChooserTable>(ChildChooserPath);
	if (!ChildTable)
	{
		return FAxonActionResult::Error(FString::Printf(
...
FAxonActionResult FAxonChooserActions::HandleValidateChooser(const TSharedPtr<FJsonObject>& Params)
{
	const FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	...
	for (int32 r = 0; r < Table->ResultsStructs.Num(); ++r)
	{
		...
		else if (!AssetRef.IsEmpty() && !FAxonAssetUtils::AssetExists(AssetRef))
		{
			Issues.Add(MakeShared<FJsonValueString>(FString::Printf(
