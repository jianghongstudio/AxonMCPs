...
/**
 * AnimBlueprint graph-surgery actions for Axon (animation namespace).
 *
 * Higher-risk graph repair kept separate from the stable AxonAbpWriteActions
 ...
 */
class AXONANIMATION_API FAxonAbpGraphSurgeryActions
{
public:
...
private:
	...
	static FAxonActionResult HandleRebuildEvaluateChooserNode(const TSharedPtr<FJsonObject>& Params);
	static FAxonActionResult HandleReplaceEvaluateChooserNodes(const TSharedPtr<FJsonObject>& Params);
	...
	static FAxonActionResult HandleAddEvaluateChooserNode(const TSharedPtr<FJsonObject>& Params);
	static FAxonActionResult HandleWireChooserToMotionMatching(const TSharedPtr<FJsonObject>& Params);
	...
	static FAxonActionResult HandleBindChooserDatabaseViaThreadSafe(const TSharedPtr<FJsonObject>& Params);
	...
	static FAxonActionResult HandleBindThreadsafeUpdateFunction(const TSharedPtr<FJsonObject>& Params);

	// F11 — duplicate + reparent + dependency classification
	static FAxonActionResult HandleDuplicateReparentAndSanitize(const TSharedPtr<FJsonObject>& Params);

	// F12 — directional node-slice compute / removal
	static FAxonActionResult HandleFindNodeSlice(const TSharedPtr<FJsonObject>& Params);
	static FAxonActionResult HandleRemoveNodeSlice(const TSharedPtr<FJsonObject>& Params);
};
