...
/**
 * Locomotion AUTHORING actions for Axon — registered under the existing
 * `animation` namespace (alongside the other AxonAnimation action classes).
 *
 * 2 actions:
 ...
 * Library appears anywhere in this class, so AxonAnimation.Build.cs needs no
 ...
 */
class AXONANIMATION_API FAxonLocomotionAuthoringActions
{
public:
...
private:
	static FAxonActionResult HandleGetRootMotionSpeed(const TSharedPtr<FJsonObject>& Params);   // T1-L2
	static FAxonActionResult HandleBakeDistanceCurve(const TSharedPtr<FJsonObject>& Params);     // T1-L3
};
