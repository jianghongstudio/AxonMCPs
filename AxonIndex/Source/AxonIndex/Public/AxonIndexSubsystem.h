...
#include "AxonIndexSubsystem.generated.h"
...
/**
 * Editor subsystem that orchestrates the Axon project index.
 * Owns the SQLite database, manages indexers, runs background indexing.
 */
UCLASS()
class AXONINDEX_API UAxonIndexSubsystem : public UEditorSubsystem
{
	GENERATED_BODY()
...
public:
	...
	FAxonIndexDatabase* GetDatabase() { return Database.Get(); }
	...
	void RegisterIndexer(TSharedPtr<IAxonIndexer> Indexer);
...
private:
	...
	{
	public:
		FIndexingTask(UAxonIndexSubsystem* InOwner);
	...
	TUniquePtr<FAxonIndexDatabase> Database;
	TArray<TSharedPtr<IAxonIndexer>> Indexers;
	TMap<FString, TSharedPtr<IAxonIndexer>> ClassToIndexer;
