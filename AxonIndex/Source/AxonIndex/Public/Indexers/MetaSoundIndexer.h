 ...
 * state (the writer-side actions in AxonAudio use the builder subsystem
 ...
 * thread, and uses FAxonMemoryHelper for batched GC + editor yielding.
 */
class FMetaSoundIndexer : public IAxonIndexer
{
public:
...
private:
	 ...
	 */
	void IndexMetaSoundAsset(UObject* MetaSoundAsset, const FString& AssetClassLabel, FAxonIndexDatabase& DB, int64 AssetId);
};
