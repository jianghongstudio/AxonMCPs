...
void FAxonCompilerSafeDispatch::RunOnGameThreadWhenCompilerIdle(
            ...
            if (RemainingAssets == 0 || bTimedOut)
            {
                if (bTimedOut && RemainingAssets != 0)
                {
                    UE_LOG(LogAxonIndex, Warning,
                        TEXT("FAxonCompilerSafeDispatch timed out after %.1fs with %d assets still compiling — running work anyway."),
