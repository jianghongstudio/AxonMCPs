...
#include "AxonMemoryHelper.h"
#include "AxonCompilerSafeDispatch.h"
...
static FAutoConsoleCommand GAxonStartIndexCommand(
	TEXT("Axon.StartIndex"),
	TEXT("Starts a full Axon project index. Use after bDeferFirstTimeIndex skipped the automatic first-time index."),
	FConsoleCommandDelegate::CreateLambda([]()
	{
		if (!GEditor)
		{
			UE_LOG(LogAxonIndex, Warning, TEXT("Axon.StartIndex: GEditor not available — cannot start index"));
			return;
		}
		...
		if (!Subsystem)
		{
			UE_LOG(LogAxonIndex, Warning, TEXT("Axon.StartIndex: AxonIndex subsystem not available — cannot start index"));
			return;
		}

		UE_LOG(LogAxonIndex, Log, TEXT("Axon.StartIndex: manual full index requested"));
		Subsystem->StartFullIndex();
	}));

void UAxonIndexSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	Database = MakeUnique<FAxonIndexDatabase>();
	FString DbPath = GetDatabasePath();

	if (!Database->Open(DbPath))
	{
		UE_LOG(LogAxonIndex, Error, TEXT("Failed to open index database at %s"), *DbPath);
		return;
	}
	...
	if (!GetDefault<UAxonSettings>()->bEnableIndex)
	{
		UE_LOG(LogAxonIndex, Log, TEXT("AxonIndex: indexing disabled via bEnableIndex=false; skipping index run"));
		return;
	}
	...
	if (ShouldAutoIndex())
	{
		...
		// empty until 'Axon.StartIndex' is run manually (escape hatch for the
		// GC worker-context crash class on huge / high-core-count environments).
		if (GetDefault<UAxonSettings>()->bDeferFirstTimeIndex)
		{
			UE_LOG(LogAxonIndex, Log, TEXT("AxonIndex: first-time index deferred via bDeferFirstTimeIndex; run Axon.StartIndex to begin"));
			return;
		}

		UE_LOG(LogAxonIndex, Log, TEXT("First launch — deferring full index until AR ready"));
		if (AR.IsLoadingAssets())
			AR.OnFilesLoaded().AddUObject(this, &UAxonIndexSubsystem::OnAssetRegistryFilesLoaded);
		else
			StartFullIndex();
	}
	else if (CanDoIncrementalIndex())
	{
		UE_LOG(LogAxonIndex, Log, TEXT("Existing index found — deferring incremental catch-up until AR ready"));
		if (AR.IsLoadingAssets())
			AR.OnFilesLoaded().AddUObject(this, &UAxonIndexSubsystem::StartIncrementalIndex);
		else
			StartIncrementalIndex();
	}
	else
	{
		UE_LOG(LogAxonIndex, Log, TEXT("Schema v1 DB — forcing full reindex to populate hashes"));
		if (AR.IsLoadingAssets())
			AR.OnFilesLoaded().AddUObject(this, &UAxonIndexSubsystem::OnAssetRegistryFilesLoaded);
		else
			StartFullIndex();
...
void UAxonIndexSubsystem::OnAssetRegistryFilesLoaded()
{
	...
	if (ShouldAutoIndex())
	{
		UE_LOG(LogAxonIndex, Log, TEXT("Asset Registry fully loaded -- starting full project index"));
		StartFullIndex();
	}
	else
	{
		UE_LOG(LogAxonIndex, Log, TEXT("Asset Registry loaded but auto-index no longer needed (already indexed)"));
	}
}

void UAxonIndexSubsystem::Deinitialize()
{
	...
	if (IndexingTaskPtr.IsValid())
	{
		if (bIsIndexing)
		{
			UE_LOG(LogAxonIndex, Warning, TEXT("Indexing was still in progress during shutdown — force-stopped"));
		}
...
void UAxonIndexSubsystem::RegisterIndexer(TSharedPtr<IAxonIndexer> Indexer)
{
	...
	UE_LOG(LogAxonIndex, Verbose, TEXT("Registered indexer: %s (%d classes)"),
		*Indexer->GetName(), Indexer->GetSupportedClasses().Num());
}

void UAxonIndexSubsystem::RegisterDefaultIndexers()
{
	const UAxonSettings* Settings = GetDefault<UAxonSettings>();
	...
	UE_LOG(LogAxonIndex, Log, TEXT("Registered %d indexers"), Indexers.Num());
}

void UAxonIndexSubsystem::StartFullIndex()
{
	if (bIsIndexing)
	{
		UE_LOG(LogAxonIndex, Warning, TEXT("Indexing already in progress"));
		return;
	}
	...
	UE_LOG(LogAxonIndex, Log, TEXT("Background indexing started"));
}

float UAxonIndexSubsystem::GetProgress() const
{
...
TArray<FSearchResult> UAxonIndexSubsystem::Search(const FString& Query, int32 Limit)
{
	if (!Database.IsValid() || !Database->IsOpen()) return {};
	return Database->FullTextSearch(Query, Limit);
}

TSharedPtr<FJsonObject> UAxonIndexSubsystem::FindReferences(const FString& PackagePath)
{
	if (!Database.IsValid() || !Database->IsOpen()) return nullptr;
	return Database->FindReferences(PackagePath);
}

TArray<FIndexedAsset> UAxonIndexSubsystem::FindByType(const FString& AssetClass, int32 Limit, int32 Offset)
{
	if (!Database.IsValid() || !Database->IsOpen()) return {};
	return Database->FindByType(AssetClass, Limit, Offset);
}

TSharedPtr<FJsonObject> UAxonIndexSubsystem::GetStats()
{
	if (!Database.IsValid() || !Database->IsOpen()) return nullptr;
	return Database->GetStats();
}

TSharedPtr<FJsonObject> UAxonIndexSubsystem::GetAssetDetails(const FString& PackagePath)
{
	if (!Database.IsValid() || !Database->IsOpen()) return nullptr;
	return Database->GetAssetDetails(PackagePath);
}

TArray<FIndexedPluginInfo> UAxonIndexSubsystem::GatherMarketplacePluginPaths() const
{
    ...
    for (const TSharedRef<IPlugin>& Plugin : ContentPlugins)
    {
        ...
        UE_LOG(LogAxonIndex, Log, TEXT("Marketplace plugin found: %s (mount: %s)"),
            *Info.FriendlyName, *Info.MountPath);

        Result.Add(MoveTemp(Info));
    }

    UE_LOG(LogAxonIndex, Log, TEXT("Found %d marketplace plugins to index"), Result.Num());
    return Result;
}
...
UAxonIndexSubsystem::FIndexingTask::FIndexingTask(UAxonIndexSubsystem* InOwner)
	: Owner(InOwner)
{
}

uint32 UAxonIndexSubsystem::FIndexingTask::Run()
{
	const UAxonSettings* GlobalSettings = GetDefault<UAxonSettings>();
	const bool bLogMemory = GlobalSettings ? GlobalSettings->bLogMemoryStats : true;

	if (bLogMemory)
	{
		FAxonMemoryHelper::LogMemoryStats(TEXT("Full index starting"));
	}
	...
	UE_LOG(LogAxonIndex, Log, TEXT("Indexing %d assets..."), TotalAssets.Load());

	FAxonIndexDatabase* DB = Owner->Database.Get();
	if (!DB || !DB->IsOpen())
	{
	...
	struct FDeepIndexEntry
	{
		...
		int64 AssetId;
		TSharedPtr<IAxonIndexer> Indexer;
	};
	...
	for (int32 i = 0; i < AllAssets.Num(); ++i)
	{
		...
		TSharedPtr<IAxonIndexer>* FoundIndexer = Owner->ClassToIndexer.Find(IndexedAsset.AssetClass);
		...
		if (Indexed % BatchSize == 0)
		{
			...
			UE_LOG(LogAxonIndex, Log, TEXT("Indexed %d / %d assets (%d errors)"),
	...
	UE_LOG(LogAxonIndex, Log, TEXT("Asset class distribution (top 20):"));
	ClassDistribution.ValueSort([](int32 A, int32 B) { return A > B; });
	int32 Shown = 0;
	for (const auto& Pair : ClassDistribution)
	{
		if (Shown++ >= 20) break;
		UE_LOG(LogAxonIndex, Log, TEXT("  %s: %d"), *Pair.Key, Pair.Value);
	}

	UE_LOG(LogAxonIndex, Log, TEXT("Deep index queue: %d assets across %d classes"),
		DeepIndexQueue.Num(), QueuedClassDistribution.Num());
	for (const auto& Pair : QueuedClassDistribution)
	{
		UE_LOG(LogAxonIndex, Log, TEXT("  Queued %s: %d"), *Pair.Key, Pair.Value);
	}

	DB->CommitTransaction();

	UE_LOG(LogAxonIndex, Log, TEXT("Metadata pass complete: %d assets indexed, %d errors"), Indexed, Errors);
	...
	{
		const UAxonSettings* Settings = GetDefault<UAxonSettings>();
		FAxonMemoryHelper::LogTierStartupOnce();
		const int32 DeepBatchSize = FMath::Max(1, FAxonMemoryHelper::GetResolvedDeepIndexBatchSize());
		const int32 GCFrequency = FMath::Max(1, Settings->GCFrequencyBatches);
		const SIZE_T MemoryBudgetMB = static_cast<SIZE_T>(FAxonMemoryHelper::GetResolvedMemoryBudgetMB());
		const float YieldTime = Settings->YieldTimeSeconds;

		UE_LOG(LogAxonIndex, Log, TEXT("Starting deep indexing pass for %d assets (batch size: %d, GC every %d batches, memory budget: %llu MB)..."),
			DeepIndexQueue.Num(), DeepBatchSize, GCFrequency, MemoryBudgetMB);

		if (bLogMemory)
		{
			FAxonMemoryHelper::LogMemoryStats(TEXT("Deep index start"));
		}
		...
		for (int32 BatchStart = 0; BatchStart < TotalDeep && !bShouldStop; BatchStart += DeepBatchSize)
		{
			...
			if (FAxonMemoryHelper::ShouldThrottle(MemoryBudgetMB))
			{
				UE_LOG(LogAxonIndex, Log, TEXT("Memory budget exceeded, forcing GC and yielding..."));
				
				FEvent* GCEvent = FPlatformProcess::GetSynchEventFromPool(true);
				AsyncTask(ENamedThreads::GameThread, [GCEvent, YieldTime]()
				{
					FAxonMemoryHelper::ForceGarbageCollection(true);
					FAxonMemoryHelper::YieldToEditor();
					if (YieldTime > 0.0f)
					{
				...
				if (bLogMemory)
				{
					FAxonMemoryHelper::LogMemoryStats(TEXT("After throttle GC"));
				}
			}

			// Check for critical memory situation
			if (FAxonMemoryHelper::IsMemoryCritical())
			{
				UE_LOG(LogAxonIndex, Warning, TEXT("Critical memory situation detected (<2GB available). Pausing indexing..."));
			...
			FAxonCompilerSafeDispatch::RunOnGameThreadWhenCompilerIdle(
				[DB, BatchSlice = MoveTemp(BatchSlice), &DeepIndexed, &DeepErrors, FrameBudgetSeconds]()
			{
				...
				for (const FDeepIndexEntry& Entry : BatchSlice)
				{
					...
					if (LoadedAsset)
					{
						...
						{
							DeepErrors++;
							UE_LOG(LogAxonIndex, Warning, TEXT("Deep indexer '%s' failed for: %s"),
								*Entry.Indexer->GetName(),
								*Entry.AssetData.PackageName.ToString());
						}

						// Mark asset for unloading to help GC
						FAxonMemoryHelper::TryUnloadPackage(LoadedAsset);
					}
					else
					{
						DeepErrors++;
						UE_LOG(LogAxonIndex, Warning, TEXT("Failed to load asset for deep indexing: %s (class: %s)"),
							*Entry.AssetData.PackageName.ToString(),
			...
			if (BatchNumber % GCFrequency == 0)
			{
				...
				{
					FAxonMemoryHelper::ForceGarbageCollection(false);
					FAxonMemoryHelper::YieldToEditor();
			...
			if (BatchNumber % 10 == 0)
			{
				UE_LOG(LogAxonIndex, Log, TEXT("Deep indexed %d / %d assets (%d ok, %d errors)"),
					BatchEnd, TotalDeep, DeepIndexed.Load(), DeepErrors.Load());

				if (bLogMemory)
				{
					FAxonMemoryHelper::LogMemoryStats(FString::Printf(TEXT("After batch %d"), BatchNumber));
				}
			}
		}
		...
		UE_LOG(LogAxonIndex, Log, TEXT("Deep indexing complete: %d indexed, %d errors"),
			DeepIndexed.Load(), DeepErrors.Load());

		if (bLogMemory)
		{
			FAxonMemoryHelper::LogMemoryStats(TEXT("Deep index complete"));
		}
	}
	...
	UE_LOG(LogAxonIndex, Log, TEXT("Starting post-pass indexers..."));
	...
	if (!CheckCancellation())
	{
		...
		TSharedPtr<IAxonIndexer>* DepIndexer = Owner->ClassToIndexer.Find(TEXT("__Dependencies__"));
		if (DepIndexer && DepIndexer->IsValid())
		{
			double SentinelStart = FPlatformTime::Seconds();
			UE_LOG(LogAxonIndex, Log, TEXT("Running dependency indexer..."));
			TSharedPtr<IAxonIndexer> DepIndexerCopy = *DepIndexer;
			...
			UE_LOG(LogAxonIndex, Log, TEXT("Dependency indexer completed in %.2fs"), FPlatformTime::Seconds() - SentinelStart);
			GCBetweenIndexers();
		}
	}
	...
	if (!CheckCancellation())
	{
		...
		TSharedPtr<IAxonIndexer>* LevelIndexer = Owner->ClassToIndexer.Find(TEXT("__Levels__"));
		if (LevelIndexer && LevelIndexer->IsValid())
		{
			double SentinelStart = FPlatformTime::Seconds();
			UE_LOG(LogAxonIndex, Log, TEXT("Running level indexer..."));
			TSharedPtr<IAxonIndexer> LevelIndexerCopy = *LevelIndexer;
			...
			UE_LOG(LogAxonIndex, Log, TEXT("Level indexer completed in %.2fs"), FPlatformTime::Seconds() - SentinelStart);
			GCBetweenIndexers();
		}
	}
	...
	if (!CheckCancellation())
	{
		...
		TSharedPtr<IAxonIndexer>* DTIndexer = Owner->ClassToIndexer.Find(TEXT("__DataTables__"));
		if (DTIndexer && DTIndexer->IsValid())
		{
			double SentinelStart = FPlatformTime::Seconds();
			UE_LOG(LogAxonIndex, Log, TEXT("Running DataTable indexer..."));
			TSharedPtr<IAxonIndexer> DTIndexerCopy = *DTIndexer;
			...
			UE_LOG(LogAxonIndex, Log, TEXT("DataTable indexer completed in %.2fs"), FPlatformTime::Seconds() - SentinelStart);
			GCBetweenIndexers();
		}
	}
	...
	if (!CheckCancellation())
	{
		...
		TSharedPtr<IAxonIndexer>* CfgIndexer = Owner->ClassToIndexer.Find(TEXT("__Configs__"));
		if (CfgIndexer && CfgIndexer->IsValid())
		{
			double SentinelStart = FPlatformTime::Seconds();
			UE_LOG(LogAxonIndex, Log, TEXT("Running config indexer..."));
			DB->BeginTransaction();
			FAssetData DummyCfgData;
			(*CfgIndexer)->IndexAsset(DummyCfgData, nullptr, *DB, 0);
			DB->CommitTransaction();
			UE_LOG(LogAxonIndex, Log, TEXT("Config indexer completed in %.2fs"), FPlatformTime::Seconds() - SentinelStart);
		}
	}
	...
	if (!CheckCancellation())
	{
		...
		TSharedPtr<IAxonIndexer>* CppIndexer = Owner->ClassToIndexer.Find(TEXT("__CppSymbols__"));
		if (CppIndexer && CppIndexer->IsValid())
		{
			double SentinelStart = FPlatformTime::Seconds();
			UE_LOG(LogAxonIndex, Log, TEXT("Running C++ symbol indexer..."));
			DB->BeginTransaction();
			FAssetData DummyCppData;
			(*CppIndexer)->IndexAsset(DummyCppData, nullptr, *DB, 0);
			DB->CommitTransaction();
			UE_LOG(LogAxonIndex, Log, TEXT("C++ symbol indexer completed in %.2fs"), FPlatformTime::Seconds() - SentinelStart);
		}
	}
	...
	if (!CheckCancellation())
	{
		...
		TSharedPtr<IAxonIndexer>* AnimIndexer = Owner->ClassToIndexer.Find(TEXT("__Animations__"));
		if (AnimIndexer && AnimIndexer->IsValid())
		{
			double SentinelStart = FPlatformTime::Seconds();
			UE_LOG(LogAxonIndex, Log, TEXT("Running animation indexer..."));
			TSharedPtr<IAxonIndexer> AnimIndexerCopy = *AnimIndexer;
			...
			UE_LOG(LogAxonIndex, Log, TEXT("Animation indexer completed in %.2fs"), FPlatformTime::Seconds() - SentinelStart);
			GCBetweenIndexers();
		}
	}
	...
	if (!CheckCancellation())
	{
		...
		TSharedPtr<IAxonIndexer>* TagIndexer = Owner->ClassToIndexer.Find(TEXT("__GameplayTags__"));
		if (TagIndexer && TagIndexer->IsValid())
		{
			double SentinelStart = FPlatformTime::Seconds();
			UE_LOG(LogAxonIndex, Log, TEXT("Running gameplay tag indexer..."));
			TSharedPtr<IAxonIndexer> TagIndexerCopy = *TagIndexer;
			...
			UE_LOG(LogAxonIndex, Log, TEXT("Gameplay tag indexer completed in %.2fs"), FPlatformTime::Seconds() - SentinelStart);
			GCBetweenIndexers();
		}
	}
	...
	if (!CheckCancellation())
	{
		...
		TSharedPtr<IAxonIndexer>* NiagaraIndexerPtr = Owner->ClassToIndexer.Find(TEXT("__Niagara__"));
		if (NiagaraIndexerPtr && NiagaraIndexerPtr->IsValid())
		{
			double SentinelStart = FPlatformTime::Seconds();
			UE_LOG(LogAxonIndex, Log, TEXT("Running Niagara indexer..."));
			TSharedPtr<IAxonIndexer> NiagaraIndexerCopy = *NiagaraIndexerPtr;
			...
			UE_LOG(LogAxonIndex, Log, TEXT("Niagara indexer completed in %.2fs"), FPlatformTime::Seconds() - SentinelStart);
			GCBetweenIndexers();
		}
	}
	...
	if (!CheckCancellation())
	{
		...
		TSharedPtr<IAxonIndexer>* MeshCatIndexer = Owner->ClassToIndexer.Find(TEXT("__MeshCatalog__"));
		if (MeshCatIndexer && MeshCatIndexer->IsValid())
		{
			double SentinelStart = FPlatformTime::Seconds();
			UE_LOG(LogAxonIndex, Log, TEXT("Running mesh catalog indexer..."));
			TSharedPtr<IAxonIndexer> MeshCatIndexerCopy = *MeshCatIndexer;
			...
			UE_LOG(LogAxonIndex, Log, TEXT("Mesh catalog indexer completed in %.2fs"), FPlatformTime::Seconds() - SentinelStart);
			GCBetweenIndexers();
		}
	}

	UE_LOG(LogAxonIndex, Log, TEXT("Post-pass indexers complete"));
	...
	if (!bShouldStop)
	{
		...
		{
			UE_LOG(LogAxonIndex, Warning, TEXT("Index only found %d assets — Asset Registry may not have been fully loaded. Skipping last_full_index write so next launch will re-index."), Indexed);
		}
		else
		{
			DB->WriteMeta(TEXT("last_full_index"), FDateTime::UtcNow().ToString());
			UE_LOG(LogAxonIndex, Log, TEXT("Wrote last_full_index timestamp (%d assets indexed)"), Indexed);
		}
	}

	if (bLogMemory)
	{
		FAxonMemoryHelper::LogMemoryStats(TEXT("Full index complete"));
	}
...
void UAxonIndexSubsystem::OnIndexingFinished(bool bSuccess)
{
	bIsIndexing = false;
	...
	UE_LOG(LogAxonIndex, Log, TEXT("Indexing %s"),
		bSuccess ? TEXT("completed successfully") : TEXT("failed or was cancelled"));
}

FString UAxonIndexSubsystem::GetDatabasePath() const
{
	TSharedPtr<IPlugin> Plugin = IPluginManager::Get().FindPlugin(TEXT("AxonIndex"));
	if (Plugin.IsValid())
	{
		return Plugin->GetBaseDir() / TEXT("Saved") / TEXT("ProjectIndex.db");
	}
	return FPaths::ProjectPluginsDir() / TEXT("AxonMCPs") / TEXT("AxonIndex") / TEXT("Saved") / TEXT("ProjectIndex.db");
}

bool UAxonIndexSubsystem::ShouldAutoIndex() const
{
...
bool UAxonIndexSubsystem::CanDoIncrementalIndex() const
{
...
void UAxonIndexSubsystem::StartIncrementalIndex()
{
	...
	UE_LOG(LogAxonIndex, Log, TEXT("Starting incremental index..."));
	...
	if (const UAxonSettings* Settings = GetDefault<UAxonSettings>())
	{
		for (const FString& CustomPath : Settings->AdditionalContentPaths)
		{
			if (!CustomPath.IsEmpty())
	...
	{
		UE_LOG(LogAxonIndex, Log, TEXT("No changes detected. Incremental index complete."));
		bIsIndexing = false;
	...
	UE_LOG(LogAxonIndex, Log, TEXT("Incremental index complete."));
	bIsIndexing = false;
	RegisterLiveCallbacks();
}
...
void UAxonIndexSubsystem::ProcessDeepIndexQueue(const TSet<FString>& PathsToIndex)
{
	...
	for (const FString& PackagePath : PathsToIndex)
	{
		...
		for (const FAssetData& AssetData : Assets)
		{
			...
			TSharedPtr<IAxonIndexer>* Indexer = ClassToIndexer.Find(ClassName);
			if (!Indexer) continue;
	...
	UE_LOG(LogAxonIndex, Log, TEXT("Deep-indexed %d assets from %d paths"), Indexed, PathsToIndex.Num());
}

void UAxonIndexSubsystem::RunScopedSentinels(const TSet<FString>& ChangedPaths, const TSet<FString>& RemovedPaths)
{
	...
	for (const auto& Indexer : Indexers)
	{
		if (Indexer->IsSentinel() && Indexer->SupportsIncrementalIndex())
		{
			...
			UE_LOG(LogAxonIndex, Log, TEXT("Scoped sentinel %s completed in %.2fs"), *Indexer->GetName(), Duration);
		}
	}
}

void UAxonIndexSubsystem::RegisterLiveCallbacks()
{
	IAssetRegistry& AR = IAssetRegistry::GetChecked();

	OnAssetsAddedHandle = AR.OnAssetsAdded().AddUObject(this, &UAxonIndexSubsystem::OnAssetsAddedCallback);
	OnAssetsRemovedHandle = AR.OnAssetsRemoved().AddUObject(this, &UAxonIndexSubsystem::OnAssetsRemovedCallback);
	OnAssetRenamedHandle = AR.OnAssetRenamed().AddUObject(this, &UAxonIndexSubsystem::OnAssetRenamedCallback);
	OnAssetsUpdatedOnDiskHandle = AR.OnAssetsUpdatedOnDisk().AddUObject(this, &UAxonIndexSubsystem::OnAssetsUpdatedOnDiskCallback);

	if (GEditor)
	{
		GEditor->GetTimerManager()->SetTimer(
			LiveIndexTimerHandle,
			FTimerDelegate::CreateUObject(this, &UAxonIndexSubsystem::ProcessPendingChanges),
			2.0f, /*bLoop=*/ true);
	}

	UE_LOG(LogAxonIndex, Log, TEXT("Live index callbacks registered."));
}

void UAxonIndexSubsystem::UnregisterLiveCallbacks()
{
...
void UAxonIndexSubsystem::OnAssetsAddedCallback(TConstArrayView<FAssetData> Assets)
{
	if (bIsIndexing) return;
	for (const FAssetData& AssetData : Assets)
	{
...
void UAxonIndexSubsystem::OnAssetsRemovedCallback(TConstArrayView<FAssetData> Assets)
{
	if (bIsIndexing) return;
	for (const FAssetData& AssetData : Assets)
	{
...
void UAxonIndexSubsystem::OnAssetRenamedCallback(const FAssetData& AssetData, const FString& OldObjectPath)
{
	if (bIsIndexing) return;
	PendingChanges.Add({EIndexChangeType::Renamed, AssetData, OldObjectPath});
}

void UAxonIndexSubsystem::OnAssetsUpdatedOnDiskCallback(TConstArrayView<FAssetData> Assets)
{
	if (bIsIndexing) return;
	for (const FAssetData& AssetData : Assets)
...
void UAxonIndexSubsystem::ProcessPendingChanges()
{
	...
	UE_LOG(LogAxonIndex, Log, TEXT("Processing %d pending index changes (%d raw)"),
	...
	for (const FPendingIndexChange& Change : LocalChanges)
	{
		...
		case EIndexChangeType::Renamed:
		{
			...
			{
				UE_LOG(LogAxonIndex, Verbose, TEXT("Asset moved: %s -> %s"), *OldPackageName, *NewPath);
			}
			else
			{
				FIndexedAsset IndexedAsset;
