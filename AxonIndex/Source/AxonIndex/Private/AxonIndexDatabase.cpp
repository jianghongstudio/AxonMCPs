...
DEFINE_LOG_CATEGORY(LogAxonIndex);
...
-- C++ symbols (from tree-sitter via AxonSource)
...
FAxonIndexDatabase::FAxonIndexDatabase()
{
}
...
bool FAxonIndexDatabase::Open(const FString& InDbPath)
{
	if (Database)
	{
		Close();
	...
	{
		UE_LOG(LogAxonIndex, Error, TEXT("Failed to open index database: %s"), *DbPath);
		delete Database;
		Database = nullptr;
		return false;
	}
	...
	if (!CreateTables())
	{
		UE_LOG(LogAxonIndex, Error, TEXT("Failed to create index tables"));
		Close();
		return false;
	}
	...
	UE_LOG(LogAxonIndex, Log, TEXT("Index database opened: %s"), *DbPath);
	return true;
}

void FAxonIndexDatabase::Close()
{
	if (Database)
	{
		Database->Close();
...
bool FAxonIndexDatabase::IsOpen() const
{
	return Database != nullptr && Database->IsValid();
}

bool FAxonIndexDatabase::ResetDatabase()
{
...
bool FAxonIndexDatabase::BeginTransaction()
{
	return ExecuteSQL(TEXT("BEGIN TRANSACTION;"));
}

bool FAxonIndexDatabase::CommitTransaction()
{
	return ExecuteSQL(TEXT("COMMIT;"));
}

bool FAxonIndexDatabase::RollbackTransaction()
{
...
int64 FAxonIndexDatabase::InsertAsset(const FIndexedAsset& Asset)
{
	if (!IsOpen()) return -1;
...
TOptional<FIndexedAsset> FAxonIndexDatabase::GetAssetByPath(const FString& PackagePath)
{
	if (!IsOpen()) return {};
...
int64 FAxonIndexDatabase::GetAssetId(const FString& PackagePath)
{
	if (!IsOpen()) return -1;
...
bool FAxonIndexDatabase::DeleteAssetAndRelated(int64 AssetId)
{
...
int64 FAxonIndexDatabase::InsertNode(const FIndexedNode& Node)
{
	if (!IsOpen()) return -1;
...
TArray<FIndexedNode> FAxonIndexDatabase::GetNodesForAsset(int64 AssetId)
{
...
int64 FAxonIndexDatabase::InsertConnection(const FIndexedConnection& Conn)
{
	if (!IsOpen()) return -1;
...
TArray<FIndexedConnection> FAxonIndexDatabase::GetConnectionsForAsset(int64 AssetId)
{
...
int64 FAxonIndexDatabase::InsertVariable(const FIndexedVariable& Var)
{
	if (!IsOpen()) return -1;
...
TArray<FIndexedVariable> FAxonIndexDatabase::GetVariablesForAsset(int64 AssetId)
{
...
int64 FAxonIndexDatabase::InsertParameter(const FIndexedParameter& Param)
{
	if (!IsOpen()) return -1;
...
int64 FAxonIndexDatabase::InsertDependency(const FIndexedDependency& Dep)
{
	if (!IsOpen()) return -1;
...
TArray<FIndexedDependency> FAxonIndexDatabase::GetDependenciesForAsset(int64 AssetId)
{
...
TArray<FIndexedDependency> FAxonIndexDatabase::GetReferencersOfAsset(int64 AssetId)
{
	TArray<FIndexedDependency> Result;
	if (!IsOpen()) return Result;
...
int64 FAxonIndexDatabase::InsertActor(const FIndexedActor& Actor)
{
	if (!IsOpen()) return -1;
...
int64 FAxonIndexDatabase::InsertTag(const FIndexedTag& Tag)
{
	if (!IsOpen()) return -1;
...
int64 FAxonIndexDatabase::GetOrCreateTag(const FString& TagName, const FString& ParentTag)
{
	if (!IsOpen()) return -1;
...
int64 FAxonIndexDatabase::InsertTagReference(const FIndexedTagReference& Ref)
{
	if (!IsOpen()) return -1;
...
int64 FAxonIndexDatabase::InsertConfig(const FIndexedConfig& Config)
{
	if (!IsOpen()) return -1;
...
int64 FAxonIndexDatabase::InsertCppSymbol(const FIndexedCppSymbol& Symbol)
{
	if (!IsOpen()) return -1;
...
int64 FAxonIndexDatabase::InsertDataTableRow(const FIndexedDataTableRow& Row)
{
	if (!IsOpen()) return -1;
...
bool FAxonIndexDatabase::WriteMeta(const FString& Key, const FString& Value)
{
	if (!IsOpen()) return false;
...
FString FAxonIndexDatabase::ReadMeta(const FString& Key) const
{
...
TArray<FString> FAxonIndexDatabase::GetAllIndexedPaths()
{
...
FString FAxonIndexDatabase::GetSavedHash(const FString& PackagePath)
{
...
TMap<FString, FString> FAxonIndexDatabase::GetAllPathsAndHashes()
{
...
bool FAxonIndexDatabase::DeleteAssetByPath(const FString& PackagePath)
{
	if (!IsOpen()) return false;
...
bool FAxonIndexDatabase::UpdateAssetPath(const FString& OldPath, const FString& NewPath, const FString& NewAssetName)
{
	if (!IsOpen()) return false;
...
bool FAxonIndexDatabase::UpdateAssetMetadata(const FIndexedAsset& Asset)
{
	if (!IsOpen()) return false;
...
bool FAxonIndexDatabase::DeleteChildDataForAsset(int64 AssetId)
{
	if (!IsOpen()) return false;
...
bool FAxonIndexDatabase::UpdateSavedHash(const FString& PackagePath, const FString& HashHex)
{
	if (!IsOpen()) return false;
...
TArray<FSearchResult> FAxonIndexDatabase::FullTextSearch(const FString& Query, int32 Limit)
{
...
TSharedPtr<FJsonObject> FAxonIndexDatabase::GetStats()
{
...
TSharedPtr<FJsonObject> FAxonIndexDatabase::GetAssetDetails(const FString& PackagePath)
{
...
TArray<FIndexedAsset> FAxonIndexDatabase::FindByType(const FString& AssetClass, int32 Limit, int32 Offset)
{
...
TSharedPtr<FJsonObject> FAxonIndexDatabase::FindReferences(const FString& PackagePath)
{
...
bool FAxonIndexDatabase::CreateTables()
{
	...
	for (const FString& Stmt : Statements)
	{
		...
		{
			UE_LOG(LogAxonIndex, Warning, TEXT("Schema statement failed: %s -- Error: %s"),
	...
	if (!bAllSucceeded)
	{
		UE_LOG(LogAxonIndex, Warning, TEXT("Some schema statements failed -- FTS5 may not be available in this SQLite build"));
	}

	return true; // Return true even if FTS fails -- basic tables should work
}

bool FAxonIndexDatabase::ExecuteSQL(const FString& SQL)
{
	if (!Database || !Database->IsValid())
	{
		UE_LOG(LogAxonIndex, Error, TEXT("Cannot execute SQL -- database not open"));
		return false;
	}

	if (!Database->Execute(*SQL))
	{
		UE_LOG(LogAxonIndex, Error, TEXT("SQL execution failed: %s"), *Database->GetLastError());
		return false;
	}
	return true;
}
