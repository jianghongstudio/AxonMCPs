...
DEFINE_LOG_CATEGORY(LogAxonMemory);
...
SIZE_T FAxonMemoryHelper::GetCurrentMemoryUsageMB()
{
	FPlatformMemoryStats Stats = FPlatformMemory::GetStats();
	return Stats.UsedPhysical / (1024 * 1024);
}

SIZE_T FAxonMemoryHelper::GetAvailableMemoryMB()
{
	FPlatformMemoryStats Stats = FPlatformMemory::GetStats();
	return Stats.AvailablePhysical / (1024 * 1024);
}

bool FAxonMemoryHelper::ShouldThrottle(SIZE_T BudgetMB)
{
	SIZE_T CurrentUsageMB = GetCurrentMemoryUsageMB();
	return CurrentUsageMB > BudgetMB;
}

void FAxonMemoryHelper::ForceGarbageCollection(bool bFullPurge)
{
	if (!IsInGameThread())
	{
		UE_LOG(LogAxonMemory, Verbose, TEXT("ForceGarbageCollection called from non-game thread - skipping"));
		return;
	}
	...
	{
		UE_LOG(LogAxonMemory, Verbose, TEXT("GC already in progress - skipping to avoid reentrant GC crash"));
		return;
	}
	...
	{
		UE_LOG(LogAxonMemory, Verbose, TEXT("GC cooldown active (%.2fs since last GC) - skipping"), TimeSinceLastGC);
		return;
	}
	...
	{
		UE_LOG(LogAxonMemory, Verbose, TEXT("TryCollectGarbage returned false - GC deferred by engine"));
		return;
	}
}

bool FAxonMemoryHelper::TryUnloadPackage(UObject* Asset)
{
	if (!Asset)
	{
		return false;
...
void FAxonMemoryHelper::YieldToEditor()
{
	if (!IsInGameThread())
	{
		return;
	...
	// dispatched via FAxonCompilerSafeDispatch's FTSTicker, runs on one real
...
void FAxonMemoryHelper::LogMemoryStats(const FString& Context)
{
	FPlatformMemoryStats Stats = FPlatformMemory::GetStats();
...
// still override per-project via Project Settings > Axon > Indexing > Performance.
...
int32 FAxonMemoryHelper::GetInstalledRamGB()
{
	const FPlatformMemoryStats Stats = FPlatformMemory::GetStats();
	return static_cast<int32>(Stats.TotalPhysical / (1024ULL * 1024ULL * 1024ULL));
}

int32 FAxonMemoryHelper::GetResolvedMemoryBudgetMB()
{
...
int32 FAxonMemoryHelper::GetResolvedDeepIndexBatchSize()
{
	const UAxonSettings* Settings = GetDefault<UAxonSettings>();
...
int32 FAxonMemoryHelper::GetResolvedPostPassBatchSize()
{
	const UAxonSettings* Settings = GetDefault<UAxonSettings>();
...
void FAxonMemoryHelper::LogTierStartupOnce()
{
	...
	UE_LOG(LogAxonMemory, Log,
		TEXT("Indexer tier: %d GB installed -> %d MB budget, batch(deep=%d, post=%d). Override in Project Settings > Axon > Indexing > Performance."),
