 ...
 */
class FGameplayTagIndexer : public IAxonIndexer
{
public:
...
private:
	...
	int64 IndexTagNode(const struct FGameplayTagNode& Node, FAxonIndexDatabase& DB);

	/** Scan all indexed assets for gameplay tag references in their asset registry tags */
	void ScanAssetTagReferences(FAxonIndexDatabase& DB);
};
