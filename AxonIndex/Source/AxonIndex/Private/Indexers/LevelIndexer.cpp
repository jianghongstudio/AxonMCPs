...
bool FLevelIndexer::IndexAsset(const FAssetData& AssetData, UObject* LoadedAsset, FAxonIndexDatabase& DB, int64 AssetId)
{
	...
	UE_LOG(LogAxonIndex, Log, TEXT("LevelIndexer: Found %d World assets to index (batch size: %d)"),
		WorldAssets.Num(), BatchSize);

	if (bLogMemory)
	{
		FAxonMemoryHelper::LogMemoryStats(TEXT("LevelIndexer start"));
	}
	...
	for (int32 i = 0; i < WorldAssets.Num(); i += BatchSize)
	{
		...
		{
			UE_LOG(LogAxonIndex, Log, TEXT("LevelIndexer: Memory budget exceeded, forcing GC..."));
			FAxonMemoryHelper::ForceGarbageCollection(true);
			FAxonMemoryHelper::YieldToEditor();

			if (bLogMemory)
			{
				FAxonMemoryHelper::LogMemoryStats(TEXT("LevelIndexer after throttle GC"));
			}
		}
		...
		for (int32 j = i; j < BatchEnd; ++j)
		{
			...
			if (!World || !World->PersistentLevel)
			{
				...
				FAxonMemoryHelper::TryUnloadPackage(Package);
				continue;
			}
			...
			if (World != EditorWorld)
			{
				...
				else
				{
					...
					FAxonMemoryHelper::TryUnloadPackage(World);
				}
			}
		...
		{
			UE_LOG(LogAxonIndex, Log, TEXT("LevelIndexer: processed %d / %d levels"),
				LevelsProcessed, WorldAssets.Num());

			if (bLogMemory)
			{
				FAxonMemoryHelper::LogMemoryStats(FString::Printf(TEXT("LevelIndexer batch %d"), BatchNumber));
			}
		}
	}
	...
	UE_LOG(LogAxonIndex, Log, TEXT("LevelIndexer: indexed %d levels, %d actors total"),
		LevelsProcessed, ActorsInserted);

	if (bLogMemory)
	{
		FAxonMemoryHelper::LogMemoryStats(TEXT("LevelIndexer complete"));
	}

	return true;
}

int32 FLevelIndexer::IndexActorsInLevel(ULevel* Level, FAxonIndexDatabase& DB, int64 AssetId)
{
	if (!Level) return 0;
