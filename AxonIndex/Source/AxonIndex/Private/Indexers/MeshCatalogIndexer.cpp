...
bool FMeshCatalogIndexer::IndexAsset(const FAssetData& AssetData, UObject* LoadedAsset, FAxonIndexDatabase& DB, int64 AssetId)
{
	FSQLiteDatabase* RawDB = DB.GetRawDatabase();
	if (!RawDB)
	{
		UE_LOG(LogAxonIndex, Error, TEXT("MeshCatalogIndexer: No raw database available"));
		return false;
	}
	...
	UE_LOG(LogAxonIndex, Log, TEXT("MeshCatalogIndexer: Found %d StaticMesh assets to catalog (batch size: %d)"),
		MeshAssets.Num(), BatchSize);

	if (bLogMemory)
	{
		FAxonMemoryHelper::LogMemoryStats(TEXT("MeshCatalogIndexer start"));
	}
	...
	for (int32 i = 0; i < MeshAssets.Num(); i += BatchSize)
	{
		...
		{
			UE_LOG(LogAxonIndex, Log, TEXT("MeshCatalogIndexer: Memory budget exceeded, forcing GC..."));
			FAxonMemoryHelper::ForceGarbageCollection(true);
			FAxonMemoryHelper::YieldToEditor();

			if (bLogMemory)
			{
				FAxonMemoryHelper::LogMemoryStats(TEXT("MeshCatalogIndexer after throttle GC"));
			}
		}
		...
		for (int32 j = i; j < BatchEnd; ++j)
		{
			...
			{
				FAxonMemoryHelper::TryUnloadPackage(Mesh);
				Errors++;
				continue;
			}
		...
		{
			UE_LOG(LogAxonIndex, Log, TEXT("MeshCatalogIndexer: cataloged %d / %d meshes"),
				Indexed, MeshAssets.Num());

			if (bLogMemory)
			{
				FAxonMemoryHelper::LogMemoryStats(FString::Printf(TEXT("MeshCatalogIndexer batch %d"), BatchNumber));
			}
		}
	}
	...
	UE_LOG(LogAxonIndex, Log, TEXT("MeshCatalogIndexer: Cataloged %d meshes (%d errors)"), Indexed, Errors);

	if (bLogMemory)
	{
		FAxonMemoryHelper::LogMemoryStats(TEXT("MeshCatalogIndexer complete"));
	}
