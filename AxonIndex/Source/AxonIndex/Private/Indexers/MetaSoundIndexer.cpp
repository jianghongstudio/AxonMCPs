...
bool FMetaSoundIndexer::IndexAsset(const FAssetData& AssetData, UObject* LoadedAsset, FAxonIndexDatabase& DB, int64 AssetId)
{
	...
	{
		FARFilter Filter;
		for (const FName& ContentPath : UAxonSettings::GetIndexedContentPaths())
		{
	...
	const int32 BatchSize = FMath::Max(1, FAxonMemoryHelper::GetResolvedPostPassBatchSize());
	...
	UE_LOG(LogAxonIndex, Log, TEXT("MetaSoundIndexer: Found %d MetaSound assets to index (batch size: %d)"),
		MetaSoundAssets.Num(), BatchSize);

	if (bLogMemory)
	{
		FAxonMemoryHelper::LogMemoryStats(TEXT("MetaSoundIndexer start"));
	}
	...
	{
		// Compiler-idle gate is enforced by FAxonCompilerSafeDispatch at the call site (mirrors NiagaraIndexer).

		// Memory budget check before each batch
		if (FAxonMemoryHelper::ShouldThrottle(MemoryBudgetMB))
		{
			UE_LOG(LogAxonIndex, Log, TEXT("MetaSoundIndexer: Memory budget exceeded, forcing GC..."));
			FAxonMemoryHelper::ForceGarbageCollection(true);
			FAxonMemoryHelper::YieldToEditor();

			if (bLogMemory)
			{
				FAxonMemoryHelper::LogMemoryStats(TEXT("MetaSoundIndexer after throttle GC"));
			}
		}
		...
		for (int32 j = i; j < BatchEnd; ++j)
		{
			...
			// Mark for unloading
			FAxonMemoryHelper::TryUnloadPackage(Loaded);
		}
		...
		{
			UE_LOG(LogAxonIndex, Log, TEXT("MetaSoundIndexer: processed %d / %d assets"),
				BatchEnd, MetaSoundAssets.Num());

			if (bLogMemory)
			{
				FAxonMemoryHelper::LogMemoryStats(FString::Printf(TEXT("MetaSoundIndexer batch %d"), BatchNumber));
			}
		}
	}
	...
	UE_LOG(LogAxonIndex, Log, TEXT("MetaSoundIndexer: indexed %d MetaSound assets"), AssetsIndexed);

	if (bLogMemory)
	{
		FAxonMemoryHelper::LogMemoryStats(TEXT("MetaSoundIndexer complete"));
	}

	return true;
}

void FMetaSoundIndexer::IndexMetaSoundAsset(UObject* MetaSoundAsset, const FString& AssetClassLabel, FAxonIndexDatabase& DB, int64 AssetId)
{
	...
	{
		UE_LOG(LogAxonIndex, Verbose, TEXT("MetaSoundIndexer: '%s' does not implement IMetaSoundDocumentInterface — skipping."),
