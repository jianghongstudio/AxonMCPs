...
bool FAnimationIndexer::IndexAsset(const FAssetData& AssetData, UObject* LoadedAsset, FAxonIndexDatabase& DB, int64 AssetId)
{
	...
	auto IndexAllOfType = [&](UClass* Class, FAnimIndexCallContext::EType Type, const TCHAR* TypeName) -> int32
	{
		...
		UE_LOG(LogAxonIndex, Log, TEXT("AnimationIndexer: Processing %d %s assets"), Assets.Num(), TypeName);
		...
		for (int32 i = 0; i < Assets.Num(); i += BatchSize)
		{
			...
			{
				UE_LOG(LogAxonIndex, Log, TEXT("AnimationIndexer: Memory budget exceeded, forcing GC..."));
			...
			for (int32 j = i; j < BatchEnd; ++j)
			{
				...
				{
					UE_LOG(LogAxonIndex, Warning, TEXT("AnimationIndexer: crashed loading/indexing '%s' - skipping"), *AD.GetSoftObjectPath().ToString());
				}
#else
			...
			{
				UE_LOG(LogAxonIndex, Log, TEXT("AnimationIndexer: %s progress %d / %d"), TypeName, BatchEnd, Assets.Num());
			}
		}
	...
	if (bLogMemory)
	{
		FAxonMemoryHelper::LogMemoryStats(TEXT("AnimationIndexer start"));
	}
	...
	UE_LOG(LogAxonIndex, Log, TEXT("AnimationIndexer: indexed %d animation assets"), TotalIndexed);

	if (bLogMemory)
	{
		FAxonMemoryHelper::LogMemoryStats(TEXT("AnimationIndexer complete"));
	}

	return true;
}

void FAnimationIndexer::IndexAnimSequence(UAnimSequence* AnimSeq, FAxonIndexDatabase& DB, int64 AssetId)
{
...
void FAnimationIndexer::IndexAnimMontage(UAnimMontage* Montage, FAxonIndexDatabase& DB, int64 AssetId)
{
...
void FAnimationIndexer::IndexBlendSpace(UBlendSpace* BlendSpace, FAxonIndexDatabase& DB, int64 AssetId)
{
...
void FAnimationIndexer::IndexPoseAsset(UPoseAsset* PoseAsset, FAxonIndexDatabase& DB, int64 AssetId)
{
	if (!PoseAsset) return;
