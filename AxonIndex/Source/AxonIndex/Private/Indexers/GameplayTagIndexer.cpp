...
bool FGameplayTagIndexer::IndexAsset(const FAssetData& AssetData, UObject* LoadedAsset, FAxonIndexDatabase& DB, int64 AssetId)
{
	...
	UE_LOG(LogAxonIndex, Log, TEXT("GameplayTagIndexer: indexed %d root tag trees, scanned assets for references"), TagsInserted);
	return true;
}

int64 FGameplayTagIndexer::IndexTagNode(const FGameplayTagNode& Node, FAxonIndexDatabase& DB)
{
...
void FGameplayTagIndexer::ScanAssetTagReferences(FAxonIndexDatabase& DB)
{
	...
	UE_LOG(LogAxonIndex, Log, TEXT("GameplayTagIndexer: inserted %d tag references across assets"), RefsInserted);
}
