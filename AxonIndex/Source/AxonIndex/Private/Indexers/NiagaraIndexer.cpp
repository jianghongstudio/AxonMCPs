...
bool FNiagaraIndexer::IndexAsset(const FAssetData& AssetData, UObject* LoadedAsset, FAxonIndexDatabase& DB, int64 AssetId)
{
	...
	UE_LOG(LogAxonIndex, Log, TEXT("NiagaraIndexer: Found %d Niagara systems to index (batch size: %d)"),
		NiagaraAssets.Num(), BatchSize);

	if (bLogMemory)
	{
		FAxonMemoryHelper::LogMemoryStats(TEXT("NiagaraIndexer start"));
	}
	...
	{
		// Compiler-idle gate is enforced by FAxonCompilerSafeDispatch at the call site (see issue #19).

		// Memory budget check before each batch
		if (FAxonMemoryHelper::ShouldThrottle(MemoryBudgetMB))
		{
			UE_LOG(LogAxonIndex, Log, TEXT("NiagaraIndexer: Memory budget exceeded, forcing GC..."));
			FAxonMemoryHelper::ForceGarbageCollection(true);
			FAxonMemoryHelper::YieldToEditor();

			if (bLogMemory)
			{
				FAxonMemoryHelper::LogMemoryStats(TEXT("NiagaraIndexer after throttle GC"));
			}
		}
		...
		for (int32 j = i; j < BatchEnd; ++j)
		{
			...
			// Mark for unloading
			FAxonMemoryHelper::TryUnloadPackage(System);
		}
		...
		{
			UE_LOG(LogAxonIndex, Log, TEXT("NiagaraIndexer: processed %d / %d systems"),
				BatchEnd, NiagaraAssets.Num());

			if (bLogMemory)
			{
				FAxonMemoryHelper::LogMemoryStats(FString::Printf(TEXT("NiagaraIndexer batch %d"), BatchNumber));
			}
		}
	}
	...
	UE_LOG(LogAxonIndex, Log, TEXT("NiagaraIndexer: indexed %d Niagara systems"), SystemsIndexed);

	if (bLogMemory)
	{
		FAxonMemoryHelper::LogMemoryStats(TEXT("NiagaraIndexer complete"));
	}

	return true;
}

void FNiagaraIndexer::IndexNiagaraSystem(UNiagaraSystem* System, FAxonIndexDatabase& DB, int64 AssetId)
{
	if (!System) return;
