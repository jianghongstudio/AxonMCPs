...
void FAxonGASCueActions::RegisterActions(FAxonToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("gas"), TEXT("create_gameplay_cue_notify"),
		...
		FAxonActionHandler::CreateStatic(&HandleCreateGameplayCueNotify),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("link_cue_to_effect"),
		...
		FAxonActionHandler::CreateStatic(&HandleLinkCueToEffect),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("unlink_cue_from_effect"),
		...
		FAxonActionHandler::CreateStatic(&HandleUnlinkCueFromEffect),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("effect_path"), TEXT("GameplayEffect Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("get_cue_info"),
		...
		FAxonActionHandler::CreateStatic(&HandleGetCueInfo),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("list_gameplay_cues"),
		...
		FAxonActionHandler::CreateStatic(&HandleListGameplayCues),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("set_cue_parameters"),
		...
		FAxonActionHandler::CreateStatic(&HandleSetCueParameters),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("find_cue_triggers"),
		...
		FAxonActionHandler::CreateStatic(&HandleFindCueTriggers),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("validate_cue_coverage"),
		...
		FAxonActionHandler::CreateStatic(&HandleValidateCueCoverage),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("batch_create_cues"),
		...
		FAxonActionHandler::CreateStatic(&HandleBatchCreateCues),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("scaffold_cue_library"),
		...
		FAxonActionHandler::CreateStatic(&HandleScaffoldCueLibrary),
		FParamSchemaBuilder()
...
/** Load a GE Blueprint (wraps AxonGAS::LoadGameplayEffectBP). */
bool LoadGEForCue(
...
{
	FString Error;
	if (!AxonGAS::LoadGameplayEffectBP(EffectPath, OutBP, OutGE, Error))
	{
...
FAxonActionResult FAxonGASCueActions::HandleCreateGameplayCueNotify(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath, CueTag;
	FAxonActionResult Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("save_path"), SavePath, Err)) return Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("cue_tag"), CueTag, Err)) return Err;
...
FAxonActionResult FAxonGASCueActions::HandleLinkCueToEffect(const TSharedPtr<FJsonObject>& Params)
{
	FString EffectPath, CueTag;
	FAxonActionResult Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("effect_path"), EffectPath, Err)) return Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("cue_tag"), CueTag, Err)) return Err;
	...
	{
		TSharedPtr<FJsonObject> Result = AxonGAS::MakeAssetResult(EffectPath,
	...
	else
	{
		...
		UE_LOG(LogAxonGAS, Warning, TEXT("Could not find GameplayCues property on GE CDO"));
		return FAxonActionResult::Error(TEXT("Failed to access GameplayCues property on GameplayEffect CDO"));
	}
...
FAxonActionResult FAxonGASCueActions::HandleUnlinkCueFromEffect(const TSharedPtr<FJsonObject>& Params)
{
	FString EffectPath, CueTag;
	FAxonActionResult Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("effect_path"), EffectPath, Err)) return Err;
...
FAxonActionResult FAxonGASCueActions::HandleGetCueInfo(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	FAxonActionResult Err;
	...
	FString Error;
	UObject* Obj = AxonGAS::LoadAssetFromPath(AssetPath, Error);
	UBlueprint* BP = Cast<UBlueprint>(Obj);
	if (!BP)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Failed to load GCN Blueprint: %s — %s"), *AssetPath, *Error));
	}

	UObject* CDO = (BP->GeneratedClass) ? BP->GeneratedClass->GetDefaultObject() : nullptr;
	if (!CDO)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Failed to get CDO for: %s"), *AssetPath));
	}
	...
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("'%s' is not a GameplayCue Notify"), *AssetPath));
	}
...
FAxonActionResult FAxonGASCueActions::HandleListGameplayCues(const TSharedPtr<FJsonObject>& Params)
{
...
FAxonActionResult FAxonGASCueActions::HandleSetCueParameters(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	FAxonActionResult Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("asset_path"), AssetPath, Err)) return Err;
	...
	for (const FParamMapping& Mapping : Mappings)
	{
		...
		for (const FString& PropName : Mapping.PropertyNames)
		{
			...
			if (Mapping.bIsAssetRef)
			{
				...
				else if (FObjectPropertyBase* ObjProp = CastField<FObjectPropertyBase>(Prop))
				{
					...
					UObject* RefObj = FAxonAssetUtils::LoadAssetByPath(ExpectedClass, ParamValue);
					if (RefObj)
					{
				...
				{
					UObject* RefObj = FAxonAssetUtils::LoadAssetByPath(UClass::StaticClass(), ParamValue);
					if (RefObj)
					{
		...
		if (!bSet)
		{
			UE_LOG(LogAxonGAS, Warning, TEXT("Could not set param '%s' — no matching property found on CDO of %s"), *Mapping.ParamName, *CDO->GetClass()->GetName());
		}
	}

	if (SetProperties.Num() == 0)
	{
		return FAxonActionResult::Error(TEXT("No parameters were set. Check that the property names match the cue type."));
	}
...
FAxonActionResult FAxonGASCueActions::HandleFindCueTriggers(const TSharedPtr<FJsonObject>& Params)
{
	FString CueTag;
	FAxonActionResult Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("cue_tag"), CueTag, Err)) return Err;
...
FAxonActionResult FAxonGASCueActions::HandleValidateCueCoverage(const TSharedPtr<FJsonObject>& Params)
{
...
FAxonActionResult FAxonGASCueActions::HandleBatchCreateCues(const TSharedPtr<FJsonObject>& Params)
{
	const TArray<TSharedPtr<FJsonValue>>* CuesArray;
	if (!Params->TryGetArrayField(TEXT("cues"), CuesArray) || CuesArray->Num() == 0)
	{
		return FAxonActionResult::Error(TEXT("Missing or empty required parameter: cues"));
	}
	...
	for (const TSharedPtr<FJsonValue>& CueVal : *CuesArray)
	{
		...
		FAxonActionResult CreateResult = HandleCreateGameplayCueNotify(CreateParams);
...
FAxonActionResult FAxonGASCueActions::HandleScaffoldCueLibrary(const TSharedPtr<FJsonObject>& Params)
{
	FString Preset, SavePathPrefix;
	FAxonActionResult Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("preset"), Preset, Err)) return Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("save_path_prefix"), SavePathPrefix, Err)) return Err;
	...
	FAxonActionResult BatchResult = HandleBatchCreateCues(BatchParams);
