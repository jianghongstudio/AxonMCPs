...
void FAxonGASScaffoldActions::RegisterActions(FAxonToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("gas"), TEXT("bootstrap_gas_foundation"),
		...
		FAxonActionHandler::CreateStatic(&HandleBootstrapGASFoundation),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("validate_gas_setup"),
		...
		FAxonActionHandler::CreateStatic(&HandleValidateGASSetup),
		FParamSchemaBuilder()
			.Build());
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("scaffold_gas_project"),
		...
		FAxonActionHandler::CreateStatic(&HandleScaffoldGASProject),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("scaffold_damage_pipeline"),
		...
		FAxonActionHandler::CreateStatic(&HandleScaffoldDamagePipeline),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("scaffold_status_effect"),
		...
		FAxonActionHandler::CreateStatic(&HandleScaffoldStatusEffect),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("scaffold_weapon_ability"),
		...
		FAxonActionHandler::CreateStatic(&HandleScaffoldWeaponAbility),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("grant_ability_to_pawn"),
		...
		FAxonActionHandler::CreateStatic(&HandleGrantAbilityToPawn),
		FParamSchemaBuilder()
...
FAxonActionResult FAxonGASScaffoldActions::HandleBootstrapGASFoundation(const TSharedPtr<FJsonObject>& Params)
{
	FString ProjectName;
	FAxonActionResult ErrorResult;
	if (!AxonGAS::RequireStringParam(Params, TEXT("project_name"), ProjectName, ErrorResult))
	{
		return ErrorResult;
	}
	...
	else
	{
		...
		FString HeaderContent = FString::Printf(TEXT(
			"// Auto-generated by Axon GAS Bootstrap\n"
			"#pragma once\n"
			"\n"
...
FAxonActionResult FAxonGASScaffoldActions::HandleValidateGASSetup(const TSharedPtr<FJsonObject>& Params)
{
...
FAxonActionResult FAxonGASScaffoldActions::HandleScaffoldGASProject(const TSharedPtr<FJsonObject>& Params)
{
	FString Preset;
	...
	FString ProjectName = FApp::GetProjectName();
	{
		...
		FAxonActionResult BootstrapResult = HandleBootstrapGASFoundation(BootstrapParams);
	...
	TArray<FString> ActorPaths = AxonGAS::ParseStringArray(Params, TEXT("actor_paths"));
	...
	if (ActorPaths.Num() > 0)
	{
		for (const FString& ActorPath : ActorPaths)
		{
			...
			FString LoadError;
			UObject* Obj = AxonGAS::LoadAssetFromPath(ActorPath, LoadError);
	...
	return FAxonActionResult::Success(ProjectResult);
}
...
FAxonActionResult FAxonGASScaffoldActions::HandleScaffoldDamagePipeline(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath;
	FAxonActionResult Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("save_path"), SavePath, Err)) return Err;

	TArray<FString> DamageTypes = AxonGAS::ParseStringArray(Params, TEXT("damage_types"));
	if (DamageTypes.Num() == 0)
	{
		return FAxonActionResult::Error(TEXT("Missing or empty required parameter: damage_types"));
	}
	...
	for (const FString& DmgType : DamageTypes)
	{
		...
		FString ExistError;
		if (!AxonGAS::EnsureAssetPathFree(GEPath, GEName, ExistError))
		{
	...
	return FAxonActionResult::Success(PipelineResult);
}
...
FAxonActionResult FAxonGASScaffoldActions::HandleScaffoldStatusEffect(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath, Name;
	FAxonActionResult Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("save_path"), SavePath, Err)) return Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("name"), Name, Err)) return Err;
	...
	TArray<FString> RemovesTags = AxonGAS::ParseStringArray(Config, TEXT("removes_tags"));
	...
	if (!GE_CDO)
	{
		return FAxonActionResult::Error(TEXT("Failed to get CDO after compile"));
	}
	...
	return FAxonActionResult::Success(StatusResult);
}
...
FAxonActionResult FAxonGASScaffoldActions::HandleScaffoldWeaponAbility(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath, WeaponType;
	FAxonActionResult Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("save_path"), SavePath, Err)) return Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("weapon_type"), WeaponType, Err)) return Err;
	...
	UGameplayAbility* AbilityCDO = AxonGAS::GetBlueprintCDO<UGameplayAbility>(NewBP);
	if (!AbilityCDO)
	{
		return FAxonActionResult::Error(TEXT("Failed to get Ability CDO after compile"));
	}
	...
	// Configure AbilityTags
	{
		...
		{
			FProperty* Prop = AxonGAS::FindAbilityAssetTagsProperty(AbilityCDO->GetClass());
			if (Prop)
			{
	...
	return FAxonActionResult::Success(WeaponResult);
}
...
namespace
{
	/** Mirror of the resolver in AxonGASASCActions.cpp. Asset-path-first, falls back to native class lookup + asset-registry scan for bare BP names. */
...
FAxonActionResult FAxonGASScaffoldActions::HandleGrantAbilityToPawn(const TSharedPtr<FJsonObject>& Params)
{
	FString PawnPath, AbilityClassId;
	FAxonActionResult Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("pawn_bp_path"), PawnPath, Err)) return Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("ability_class_path"), AbilityClassId, Err)) return Err;
	...
	// AxonGAS::LoadBlueprintFromParams because it expects the legacy
	// "asset_path" key — this F8 action uses "pawn_bp_path" to match the spec.
	FString LoadError;
	UObject* Obj = AxonGAS::LoadAssetFromPath(PawnPath, LoadError);
	UBlueprint* BP = Cast<UBlueprint>(Obj);
	if (!BP)
	{
