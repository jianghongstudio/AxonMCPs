...
#include "AxonPackagePathValidator.h"
...
UBlueprint* LoadBlueprintFromParams(const TSharedPtr<FJsonObject>& Params, FString& OutAssetPath, FString& OutError)
{
	...
	{
		if (UBlueprint* BP = Cast<UBlueprint>(FAxonAssetUtils::LoadAssetByPath(UBlueprint::StaticClass(), FullPath)))
		{
			return BP;
		}
	}
...
{
	UObject* Obj = FAxonAssetUtils::LoadAssetByPath(UObject::StaticClass(), AssetPath);
	if (!Obj)
	{
...
UPackage* GetOrCreatePackage(const FString& SavePath, FString& OutError)
{
	...
	if (const FString ValidationError = AxonCore::ValidatePackagePath(PackageName); !ValidationError.IsEmpty())
	{
		UE_LOG(LogAxonGAS, Warning, TEXT("GetOrCreatePackage rejected path: %s"), *ValidationError);
		...
		for (const auto& Val : *TagArray)
		{
			...
			if (Val->TryGetString(TagStr) && !TagStr.IsEmpty())
			{
				...
				else
				{
					...
					UE_LOG(LogAxonGAS, Warning, TEXT("Gameplay tag not found: '%s' (field=%s)"), *TagStr, *FieldName);
...
bool RequireStringParam(const TSharedPtr<FJsonObject>& Params, const FString& ParamName, FString& OutValue, FAxonActionResult& OutError)
{
