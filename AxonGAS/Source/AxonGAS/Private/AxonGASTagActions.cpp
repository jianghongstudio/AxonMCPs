...
void FAxonGASTagActions::RegisterActions(FAxonToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("gas"), TEXT("add_gameplay_tags"),
		...
		FAxonActionHandler::CreateStatic(&HandleAddGameplayTags),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("get_tag_hierarchy"),
		...
		FAxonActionHandler::CreateStatic(&HandleGetTagHierarchy),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("search_tag_usage"),
		...
		FAxonActionHandler::CreateStatic(&HandleSearchTagUsage),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("scaffold_tag_hierarchy"),
		...
		FAxonActionHandler::CreateStatic(&HandleScaffoldTagHierarchy),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("rename_tag"),
		...
		FAxonActionHandler::CreateStatic(&HandleRenameTag),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("remove_gameplay_tags"),
		...
		FAxonActionHandler::CreateStatic(&HandleRemoveGameplayTags),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("validate_tag_consistency"),
		...
		FAxonActionHandler::CreateStatic(&HandleValidateTagConsistency),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("audit_tag_naming"),
		...
		FAxonActionHandler::CreateStatic(&HandleAuditTagNaming),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("export_tag_hierarchy"),
		...
		FAxonActionHandler::CreateStatic(&HandleExportTagHierarchy),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("import_tag_hierarchy"),
		...
		FAxonActionHandler::CreateStatic(&HandleImportTagHierarchy),
		FParamSchemaBuilder()
...
FAxonActionResult FAxonGASTagActions::HandleAddGameplayTags(const TSharedPtr<FJsonObject>& Params)
{
	// Parse required tags array
	TArray<FString> Tags = AxonGAS::ParseStringArray(Params, TEXT("tags"));
	if (Tags.Num() == 0)
	{
		return FAxonActionResult::Error(TEXT("Missing or empty required parameter: tags"));
	}
	...
	else
	{
		// ── DataTable mode ──
		FString Error;
		UObject* Existing = AxonGAS::LoadAssetFromPath(TablePath, Error);
		...
		if (!DataTable)
		{
			...
			FString ExistError;
			if (!AxonGAS::EnsureAssetPathFree(TablePath, AssetName, ExistError))
			{
				return FAxonActionResult::Error(ExistError);
			}

			UPackage* Package = AxonGAS::GetOrCreatePackage(TablePath, Error);
			if (!Package)
			{
...
FAxonActionResult FAxonGASTagActions::HandleGetTagHierarchy(const TSharedPtr<FJsonObject>& Params)
{
...
FAxonActionResult FAxonGASTagActions::HandleSearchTagUsage(const TSharedPtr<FJsonObject>& Params)
{
	FString TagString;
	FAxonActionResult ErrorResult;
	if (!AxonGAS::RequireStringParam(Params, TEXT("tag"), TagString, ErrorResult))
	{
		return ErrorResult;
	}
...
FAxonActionResult FAxonGASTagActions::HandleScaffoldTagHierarchy(const TSharedPtr<FJsonObject>& Params)
{
	FString Preset;
	FAxonActionResult ErrorResult;
	if (!AxonGAS::RequireStringParam(Params, TEXT("preset"), Preset, ErrorResult))
	{
		return ErrorResult;
	}
	...
	FAxonActionResult AddResult = HandleAddGameplayTags(AddParams);
...
FAxonActionResult FAxonGASTagActions::HandleRenameTag(const TSharedPtr<FJsonObject>& Params)
{
	FString OldTag, NewTag;
	FAxonActionResult Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("old_tag"), OldTag, Err)) return Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("new_tag"), NewTag, Err)) return Err;
...
FAxonActionResult FAxonGASTagActions::HandleRemoveGameplayTags(const TSharedPtr<FJsonObject>& Params)
{
	TArray<FString> Tags = AxonGAS::ParseStringArray(Params, TEXT("tags"));
	if (Tags.Num() == 0)
	{
...
FAxonActionResult FAxonGASTagActions::HandleValidateTagConsistency(const TSharedPtr<FJsonObject>& Params)
{
...
FAxonActionResult FAxonGASTagActions::HandleAuditTagNaming(const TSharedPtr<FJsonObject>& Params)
{
...
FAxonActionResult FAxonGASTagActions::HandleExportTagHierarchy(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!OutputPath.IsEmpty())
	{
		...
		if (!bWritten)
		{
			return FAxonActionResult::Error(FString::Printf(TEXT("Failed to write to: %s"), *OutputPath));
		}
	}
...
FAxonActionResult FAxonGASTagActions::HandleImportTagHierarchy(const TSharedPtr<FJsonObject>& Params)
{
	FString SourcePath;
	FAxonActionResult Err;
	...
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Failed to read file: %s"), *SourcePath));
	}
	...
	if (ImportedTags.Num() == 0)
	{
		return FAxonActionResult::Error(TEXT("No tags found in source file"));
	}
