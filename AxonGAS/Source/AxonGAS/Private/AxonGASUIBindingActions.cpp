...
namespace
{
    ...
    {
        // FAxonAssetUtils::ResolveAssetPath normalizes leading-`/`-missing paths
        ...
        UObject* Raw = FAxonAssetUtils::LoadAssetByPath(AssetPath);
        if (!Raw)
        {
     ...
     */
    bool ParseOwner(const FString& S, EAxonAttrBindOwner& OutOwner, FName& OutSocketTag, FString& OutError)
    {
        if (S.IsEmpty() || S.Equals(TEXT("owning_player_pawn"), ESearchCase::IgnoreCase))
        {
            OutOwner = EAxonAttrBindOwner::OwningPlayerPawn;
            return true;
        }
        if (S.Equals(TEXT("owning_player_state"), ESearchCase::IgnoreCase))
        {
            OutOwner = EAxonAttrBindOwner::OwningPlayerState;
            return true;
        }
        if (S.Equals(TEXT("owning_player_controller"), ESearchCase::IgnoreCase))
        {
            OutOwner = EAxonAttrBindOwner::OwningPlayerController;
            return true;
        }
        if (S.Equals(TEXT("self_actor"), ESearchCase::IgnoreCase))
        {
            OutOwner = EAxonAttrBindOwner::SelfActor;
            return true;
        }
        if (S.StartsWith(TEXT("named_socket"), ESearchCase::IgnoreCase))
        {
            OutOwner = EAxonAttrBindOwner::NamedSocket;
            FString L, R;
    ...
    FString OwnerToString(EAxonAttrBindOwner Owner)
    {
        switch (Owner)
        {
            case EAxonAttrBindOwner::OwningPlayerPawn:       return TEXT("owning_player_pawn");
            case EAxonAttrBindOwner::OwningPlayerState:      return TEXT("owning_player_state");
            case EAxonAttrBindOwner::OwningPlayerController: return TEXT("owning_player_controller");
            case EAxonAttrBindOwner::SelfActor:              return TEXT("self_actor");
            case EAxonAttrBindOwner::NamedSocket:            return TEXT("named_socket");
        }
        return TEXT("owning_player_pawn");
    }

    EAxonAttrBindFormat ParseFormat(const FString& S, FString& OutPayload)
    {
        // Recognized: auto | percent_0_1 | int_text | float_text:N | format_string:<tpl> | gradient:<lo->hi>
        if (S.IsEmpty() || S.Equals(TEXT("auto"), ESearchCase::IgnoreCase)) return EAxonAttrBindFormat::Auto;
        if (S.Equals(TEXT("percent_0_1"), ESearchCase::IgnoreCase)) return EAxonAttrBindFormat::PercentZeroOne;
        if (S.Equals(TEXT("int_text"), ESearchCase::IgnoreCase)) return EAxonAttrBindFormat::IntText;
        if (S.StartsWith(TEXT("float_text"), ESearchCase::IgnoreCase))
        {
            FString L, R;
            if (S.Split(TEXT(":"), &L, &R)) OutPayload = R;
            return EAxonAttrBindFormat::FloatText;
        }
        if (S.StartsWith(TEXT("format_string"), ESearchCase::IgnoreCase))
        {
            FString L, R;
            if (S.Split(TEXT(":"), &L, &R)) OutPayload = R;
            return EAxonAttrBindFormat::FormatString;
        }
        if (S.StartsWith(TEXT("gradient"), ESearchCase::IgnoreCase))
        {
            FString L, R;
            if (S.Split(TEXT(":"), &L, &R)) OutPayload = R;
            return EAxonAttrBindFormat::Gradient;
        }
        if (S.StartsWith(TEXT("threshold"), ESearchCase::IgnoreCase))
        {
            FString L, R;
            if (S.Split(TEXT(":"), &L, &R)) OutPayload = R;
            return EAxonAttrBindFormat::Threshold;
        }
        return EAxonAttrBindFormat::Auto;
    }
    ...
    FString FormatToString(EAxonAttrBindFormat F)
    {
        switch (F)
        {
            case EAxonAttrBindFormat::Auto:           return TEXT("auto");
            case EAxonAttrBindFormat::PercentZeroOne: return TEXT("percent_0_1");
            case EAxonAttrBindFormat::IntText:        return TEXT("int_text");
            case EAxonAttrBindFormat::FloatText:      return TEXT("float_text");
            case EAxonAttrBindFormat::FormatString:   return TEXT("format_string");
            case EAxonAttrBindFormat::Gradient:       return TEXT("gradient");
            case EAxonAttrBindFormat::Threshold:      return TEXT("threshold");
        }
        return TEXT("auto");
    }

    EAxonAttrBindUpdate ParseUpdate(const FString& S, float& OutSmoothing)
    {
        if (S.IsEmpty() || S.Equals(TEXT("on_change"), ESearchCase::IgnoreCase)) return EAxonAttrBindUpdate::OnChange;
        if (S.Equals(TEXT("tick"), ESearchCase::IgnoreCase)) return EAxonAttrBindUpdate::Tick;
        if (S.StartsWith(TEXT("on_change_smoothed"), ESearchCase::IgnoreCase))
        {
            FString L, R;
            if (S.Split(TEXT(":"), &L, &R) && !R.IsEmpty()) OutSmoothing = FCString::Atof(*R);
            return EAxonAttrBindUpdate::OnChangeSmoothed;
        }
        return EAxonAttrBindUpdate::OnChange;
    }

    FString UpdateToString(EAxonAttrBindUpdate U)
    {
        switch (U)
        {
            case EAxonAttrBindUpdate::OnChange:         return TEXT("on_change");
            case EAxonAttrBindUpdate::Tick:             return TEXT("tick");
            case EAxonAttrBindUpdate::OnChangeSmoothed: return TEXT("on_change_smoothed");
        }
        return TEXT("on_change");
    }

    /** Validate (widget_class, target_property) is a supported pair. Adds warnings for borderline cases. */
    bool ValidateWidgetProperty(UWidget* Widget, FName Prop, EAxonAttrBindFormat& Format,
                                bool bHasMaxAttribute, TArray<FString>& Warnings, FString& OutError)
    {
        ...
        if (C->IsChildOf(UProgressBar::StaticClass()) && Prop == TEXT("Percent"))
        {
            ...
            if (Format == EAxonAttrBindFormat::Auto) Format = EAxonAttrBindFormat::PercentZeroOne;
            return true;
        }
        ...
        {
            if (Format == EAxonAttrBindFormat::Auto)
            {
                Format = bHasMaxAttribute ? EAxonAttrBindFormat::FormatString : EAxonAttrBindFormat::IntText;
            }
        ...
        {
            if (Format != EAxonAttrBindFormat::Gradient)
            {
        ...
        {
            if (Format == EAxonAttrBindFormat::Auto) Format = EAxonAttrBindFormat::Threshold;
            return true;
        }
        ...
        {
            if (Format == EAxonAttrBindFormat::Auto) Format = EAxonAttrBindFormat::FloatText;
    ...
    UAxonGASUIBindingBlueprintExtension* GetOrCreateExtension(UWidgetBlueprint* WBP)
    {
        if (!WBP) return nullptr;
        UAxonGASUIBindingBlueprintExtension* Existing =
            UWidgetBlueprintExtension::GetExtension<UAxonGASUIBindingBlueprintExtension>(WBP);
        if (Existing) return Existing;
        return UWidgetBlueprintExtension::RequestExtension<UAxonGASUIBindingBlueprintExtension>(WBP);
    }
    ...
    TSharedPtr<FJsonObject> SerializeBindingRow(const FAxonGASAttributeBindingSpec& Spec, int32 Index, UWidgetBlueprint* WBP)
    {
...
void FAxonGASUIBindingActions::RegisterActions(FAxonToolRegistry& Registry)
{
    ...
    Registry.RegisterAction(TEXT("gas"), TEXT("bind_widget_to_attribute"),    BindDesc,
        FAxonActionHandler::CreateStatic(&HandleBindWidgetToAttribute), BuildBindSchema());
    Registry.RegisterAction(TEXT("gas"), TEXT("unbind_widget_attribute"),    UnbindDesc,
        FAxonActionHandler::CreateStatic(&HandleUnbindWidgetAttribute), BuildUnbindSchema());
    Registry.RegisterAction(TEXT("gas"), TEXT("list_attribute_bindings"),    ListDesc,
        FAxonActionHandler::CreateStatic(&HandleListAttributeBindings), BuildListSchema());
    Registry.RegisterAction(TEXT("gas"), TEXT("clear_widget_attribute_bindings"), ClearDesc,
        FAxonActionHandler::CreateStatic(&HandleClearWidgetAttributeBindings), BuildClearSchema());
...
FAxonActionResult FAxonGASUIBindingActions::HandleBindWidgetToAttribute(const TSharedPtr<FJsonObject>& Params)
{
    FString WbpPath, WidgetNameStr, TargetPropStr, AttrStr;
    FAxonActionResult Err;
    if (!AxonGAS::RequireStringParam(Params, TEXT("wbp_path"),        WbpPath,       Err)) return Err;
    if (!AxonGAS::RequireStringParam(Params, TEXT("widget_name"),     WidgetNameStr, Err)) return Err;
    if (!AxonGAS::RequireStringParam(Params, TEXT("target_property"), TargetPropStr, Err)) return Err;
    ...
    if (!WBP) return FAxonActionResult::Error(LoadErr);
    ...
    {
        return FAxonActionResult::Error(ParseErr);
    }
    ...
    if (!MaxAttrStr.IsEmpty())
    {
        ...
        {
            return FAxonActionResult::Error(FString::Printf(TEXT("max_attribute parse error: %s"), *MaxParseErr));
        }
        bHasMax = true;
    }
    ...
    EAxonAttrBindFormat Format = ParseFormat(FormatStr, FormatPayload);

    // Phase J F3: pre-validate user-supplied format_string payloads for required slots.
    if (Format == EAxonAttrBindFormat::FormatString)
    {
        FString FmtErr;
        if (!ValidateFormatStringPayload(FormatPayload, bHasMax, FmtErr))
        {
            return FAxonActionResult::Error(FmtErr);
        }
    }
    ...
    {
        return FAxonActionResult::Error(ValidErr);
    }
    ...
    if (Format == EAxonAttrBindFormat::FormatString && FormatPayload.IsEmpty())
    {
    ...
    // Update policy.
    float Smoothing = 6.f;
    EAxonAttrBindUpdate UpdatePolicy = ParseUpdate(UpdateStr, Smoothing);
    ...
    EAxonAttrBindOwner Owner = EAxonAttrBindOwner::OwningPlayerPawn;
    FName OwnerTag;
    FString OwnerErr;
    if (!ParseOwner(OwnerStr, Owner, OwnerTag, OwnerErr))
    {
        return FAxonActionResult::Error(OwnerErr);
    }

    // Build the spec.
    FAxonGASAttributeBindingSpec Spec;
    ...
    UAxonGASUIBindingBlueprintExtension* Ext = GetOrCreateExtension(WBP);
    if (!Ext)
    {
        return FAxonActionResult::Error(TEXT("Failed to obtain UAxonGASUIBindingBlueprintExtension"));
    }
    ...
    Result->SetStringField(TEXT("extension_class"), TEXT("UAxonGASAttributeBindingClassExtension"));
...
FAxonActionResult FAxonGASUIBindingActions::HandleUnbindWidgetAttribute(const TSharedPtr<FJsonObject>& Params)
{
    ...
    UAxonGASUIBindingBlueprintExtension* Ext =
        UWidgetBlueprintExtension::GetExtension<UAxonGASUIBindingBlueprintExtension>(WBP);
    if (!Ext)
    {
        return FAxonActionResult::Error(TEXT("No GAS attribute bindings on this WBP"));
    }
...
FAxonActionResult FAxonGASUIBindingActions::HandleListAttributeBindings(const TSharedPtr<FJsonObject>& Params)
{
    FString WbpPath;
    FAxonActionResult Err;
    if (!AxonGAS::RequireStringParam(Params, TEXT("wbp_path"), WbpPath, Err)) return Err;
    ...
    Result->SetStringField(TEXT("note"),
        TEXT("These are GAS attribute bindings (Axon). Distinct from ui::get_widget_bindings which reads UMG FDelegateRuntimeBinding."));

    // Phase J F9: read-only listing — Verbose so it doesn't spam under frequent UI inspection traffic.
    UE_LOG(LogAxonGAS, Verbose,
...
FAxonActionResult FAxonGASUIBindingActions::HandleClearWidgetAttributeBindings(const TSharedPtr<FJsonObject>& Params)
{
    FString WbpPath;
    FAxonActionResult Err;
    if (!AxonGAS::RequireStringParam(Params, TEXT("wbp_path"), WbpPath, Err)) return Err;
