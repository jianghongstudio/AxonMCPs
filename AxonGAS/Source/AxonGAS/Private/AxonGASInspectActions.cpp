...
void FAxonGASInspectActions::RegisterActions(FAxonToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("gas"), TEXT("export_gas_manifest"),
		...
		FAxonActionHandler::CreateStatic(&HandleExportGASManifest),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("snapshot_gas_state"),
		...
		FAxonActionHandler::CreateStatic(&HandleSnapshotGASState),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("get_tag_state"),
		...
		FAxonActionHandler::CreateStatic(&HandleGetTagState),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("get_cooldown_state"),
		...
		FAxonActionHandler::CreateStatic(&HandleGetCooldownState),
		FParamSchemaBuilder()
			.Required(TEXT("actor"), TEXT("string"), TEXT("Actor name, label, or path in PIE world"))
			.Build());
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("trace_ability_activation"),
		...
		FAxonActionHandler::CreateStatic(&HandleTraceAbilityActivation),
		FParamSchemaBuilder()
			.Required(TEXT("actor"), TEXT("string"), TEXT("Actor name, label, or path in PIE world"))
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("compare_gas_states"),
		...
		FAxonActionHandler::CreateStatic(&HandleCompareGASStates),
		FParamSchemaBuilder()
...
FAxonActionResult FAxonGASInspectActions::HandleExportGASManifest(const TSharedPtr<FJsonObject>& Params)
{
	...
	for (const FAssetData& AssetData : AllBPs)
	{
		...
		UGameplayAbility* AbilityCDO = AxonGAS::GetBlueprintCDO<UGameplayAbility>(BP);
	...
	for (const FAssetData& AssetData : AllBPs)
	{
		...
		UObject* Obj = AssetData.GetAsset();
		UBlueprint* BP = Cast<UBlueprint>(Obj);
		if (!BP || !AxonGAS::IsAttributeSetBlueprint(BP)) continue;

		UAttributeSet* SetCDO = AxonGAS::GetBlueprintCDO<UAttributeSet>(BP);
		if (!SetCDO) continue;
	...
	if (!OutputPath.IsEmpty())
	{
		...
		if (!bWritten)
		{
			return FAxonActionResult::Error(FString::Printf(TEXT("Failed to write manifest to: %s"), *OutputPath));
		}
	}
...
FAxonActionResult FAxonGASInspectActions::HandleSnapshotGASState(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* World = AxonGAS::GetPIEWorld();
	if (!World)
	{
...
FAxonActionResult FAxonGASInspectActions::HandleGetTagState(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorIdent;
	FAxonActionResult Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("actor"), ActorIdent, Err))
		return Err;

	AActor* Actor = AxonGAS::FindActorInPIE(ActorIdent);
	if (!Actor)
	{
...
FAxonActionResult FAxonGASInspectActions::HandleGetCooldownState(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorIdent;
	FAxonActionResult Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("actor"), ActorIdent, Err))
...
FAxonActionResult FAxonGASInspectActions::HandleTraceAbilityActivation(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorIdent;
	FAxonActionResult Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("actor"), ActorIdent, Err))
	...
	if (!AxonGAS::RequireStringParam(Params, TEXT("ability_class"), AbilityClassStr, Err))
		return Err;
	...
	// ── Check 6: Blocked by other abilities ──
	{
		...
		FProperty* AbilityTagsProp = AxonGAS::FindAbilityAssetTagsProperty(AbilityCDO->GetClass());
...
FAxonActionResult FAxonGASInspectActions::HandleCompareGASStates(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: snapshot_a (object)"));
	}
	if (!Params->TryGetObjectField(TEXT("snapshot_b"), SnapshotB) || !*SnapshotB)
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: snapshot_b (object)"));
	}
