...
void FAxonGASTargetActions::RegisterActions(FAxonToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("gas"), TEXT("create_target_actor"),
		...
		FAxonActionHandler::CreateStatic(&HandleCreateTargetActor),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("configure_target_actor"),
		...
		FAxonActionHandler::CreateStatic(&HandleConfigureTargetActor),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("add_targeting_to_ability"),
		...
		FAxonActionHandler::CreateStatic(&HandleAddTargetingToAbility),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("scaffold_fps_targeting"),
		...
		FAxonActionHandler::CreateStatic(&HandleScaffoldFPSTargeting),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("ability_path"), TEXT("Ability Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("validate_targeting"),
		...
		FAxonActionHandler::CreateStatic(&HandleValidateTargeting),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("ability_path"), TEXT("Ability Blueprint asset path"))
			.Build());
}
...
FAxonActionResult FAxonGASTargetActions::HandleCreateTargetActor(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!AxonGAS::RequireStringParam(Params, TEXT("targeting_type"), TargetingType, Err)) return Err;
	...
	if (!NewBP)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Failed to create TargetActor Blueprint at: %s"), *SavePath));
	}
...
FAxonActionResult FAxonGASTargetActions::HandleConfigureTargetActor(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	FAxonActionResult Err;
	...
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Failed to load TargetActor BP: %s — %s"), *AssetPath, *Error));
	}

	UObject* CDO = BP->GeneratedClass->GetDefaultObject();
	if (!CDO || !CDO->GetClass()->IsChildOf(AGameplayAbilityTargetActor::StaticClass()))
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("'%s' is not a GameplayAbilityTargetActor"), *AssetPath));
	}
	...
	if (SetProps.Num() == 0)
	{
		return FAxonActionResult::Error(TEXT("No properties were changed. Check parameter names and TargetActor type."));
	}
...
FAxonActionResult FAxonGASTargetActions::HandleAddTargetingToAbility(const TSharedPtr<FJsonObject>& Params)
{
	FString AbilityPath;
	FAxonActionResult Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("ability_path"), AbilityPath, Err)) return Err;

	FString Error;
	UObject* Obj = AxonGAS::LoadAssetFromPath(AbilityPath, Error);
	UBlueprint* BP = Cast<UBlueprint>(Obj);
	if (!BP)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Failed to load ability: %s — %s"), *AbilityPath, *Error));
	}
	if (!AxonGAS::IsAbilityBlueprint(BP))
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("'%s' is not a GameplayAbility Blueprint"), *AbilityPath));
	}
	...
	{
		UObject* ClassObj = FAxonAssetUtils::LoadAssetByPath(UClass::StaticClass(), TargetActorClassPath);
		TargetActorClass = Cast<UClass>(ClassObj);
		if (!TargetActorClass)
		{
			// Try loading as Blueprint
			UBlueprint* TABP = Cast<UBlueprint>(FAxonAssetUtils::LoadAssetByPath(UBlueprint::StaticClass(), TargetActorClassPath));
	...
	if (!EventGraph)
	{
		return FAxonActionResult::Error(TEXT("No EventGraph found in ability Blueprint"));
	}
	...
	TSharedPtr<FJsonObject> Result = AxonGAS::MakeAssetResult(AbilityPath,
...
FAxonActionResult FAxonGASTargetActions::HandleScaffoldFPSTargeting(const TSharedPtr<FJsonObject>& Params)
{
	FString AbilityPath, Mode;
	FAxonActionResult Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("ability_path"), AbilityPath, Err)) return Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("mode"), Mode, Err)) return Err;
	...
	FString Error;
	UObject* AbilityObj = AxonGAS::LoadAssetFromPath(AbilityPath, Error);
	UBlueprint* AbilityBP = Cast<UBlueprint>(AbilityObj);
	if (!AbilityBP || !AxonGAS::IsAbilityBlueprint(AbilityBP))
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("'%s' is not a valid ability Blueprint — %s"), *AbilityPath, *Error));
	}
	...
	FAxonActionResult CreateResult = HandleCreateTargetActor(CreateTAParams);
	...
	else
	{
		...
		UObject* ExistingTA = AxonGAS::LoadAssetFromPath(TASavePath, Error);
		if (ExistingTA)
		{
			TAClassPath = TASavePath;
	...
	FAxonActionResult WireResult = HandleAddTargetingToAbility(WireParams);
...
FAxonActionResult FAxonGASTargetActions::HandleValidateTargeting(const TSharedPtr<FJsonObject>& Params)
{
	FString AbilityPath;
	FAxonActionResult Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("ability_path"), AbilityPath, Err)) return Err;
	...
	if (!BP)
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("Failed to load: %s — %s"), *AbilityPath, *Error));
	}
	if (!AxonGAS::IsAbilityBlueprint(BP))
	{
		return FAxonActionResult::Error(FString::Printf(TEXT("'%s' is not a GameplayAbility Blueprint"), *AbilityPath));
