...
// Pattern: mirrors Phase 1 Plugins/Axon/Source/AxonBlueprint/Private/AxonBlueprintBulkFillAdapter.h.
 ...
 * Self-registers with FAxonBulkFillRegistry from FAxonGASModule::StartupModule.
 *
 ...
 *   - WITH_GBA=0: returns a clean "GAS optional dep not available" error in the FAxonDryRunReport so
 *                `axon_discover("gas")` action surface stays identical across dev + release builds.
 *
 * Targets in v1:
 ...
 *   {
 *     "target_asset": "/Game/Tests/Axon/BulkFill/DT_AttrInit200",
 ...
 */
class FAxonGASBulkFillAdapter
{
public:
	/** Register the adapter pair with FAxonBulkFillRegistry under namespace "gas". */
	static void Register();

	/** Unregister (called from FAxonGASModule::ShutdownModule). */
	static void Unregister();

	/** Internal: bulk_fill.apply handler for target_namespace="gas". */
	static FAxonDryRunReport GasBulkFill(const FAxonBulkFillSpec& Spec);

	/** Internal: describe.schema handler for target_namespace="gas". */
	static FAxonSchemaDescriptor GasDescribe(const FString& TargetAsset);
};
