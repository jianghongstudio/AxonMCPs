// AxonGASAttributeBindingClassExtension.cpp
...
FGameplayAttribute UAxonGASAttributeBindingClassExtension::ResolveAttribute(const FString& AttrSetClassPath, FName PropertyName)
{
...
UAbilitySystemComponent* UAxonGASAttributeBindingClassExtension::ResolveASC(UUserWidget* UW, const FAxonGASAttributeBindingSpec& Spec) const
{
    if (!UW) return nullptr;
    switch (Spec.Owner)
    {
        case EAxonAttrBindOwner::OwningPlayerPawn:
        {
        ...
        case EAxonAttrBindOwner::OwningPlayerState:
        {
            if (APlayerController* PC = UW->GetOwningPlayer())
            {
        ...
        case EAxonAttrBindOwner::OwningPlayerController:
        {
            return GetASCFromActorRuntime(UW->GetOwningPlayer());
        }
        case EAxonAttrBindOwner::SelfActor:
        {
            return GetASCFromActorRuntime(Cast<AActor>(UW->GetOuter()));
        }
        case EAxonAttrBindOwner::NamedSocket:
        {
...
void UAxonGASAttributeBindingClassExtension::Initialize(UUserWidget* UserWidget)
{
...
void UAxonGASAttributeBindingClassExtension::Construct(UUserWidget* UserWidget)
{
...
void UAxonGASAttributeBindingClassExtension::Destruct(UUserWidget* UserWidget)
{
    if (UserWidget)
    {
...
void UAxonGASAttributeBindingClassExtension::SubscribeRow(UUserWidget* UW, int32 RowIndex, const FAxonGASAttributeBindingSpec& Spec, FInstanceState& State)
{
...
void UAxonGASAttributeBindingClassExtension::OnAttributeChanged(const FOnAttributeChangeData& Data, TWeakObjectPtr<UUserWidget> WeakUW, int32 RowIndex)
{
    ...
    FActiveSub& Sub = State->SubsByRow[RowIndex];
    const FAxonGASAttributeBindingSpec& Spec = Bindings[RowIndex];
    ...
    if (Spec.UpdatePolicy != EAxonAttrBindUpdate::OnChangeSmoothed)
    {
        Sub.CurrentDisplayedValue = Sub.TargetValue;
    }
...
void UAxonGASAttributeBindingClassExtension::ApplyValue(UUserWidget* UW, const FAxonGASAttributeBindingSpec& Spec, FActiveSub& Sub) const
{
    ...
    // Phase J F9: per-fire trace at Verbose (shipping-silent by default; enable LogAxonGAS Verbose to surface).
    ...
    EAxonAttrBindFormat Format = Spec.Format;
    ...
    {
        FString Out;
        if (Format == EAxonAttrBindFormat::FormatString && !Spec.FormatPayload.IsEmpty())
        {
        ...
        else if (Format == EAxonAttrBindFormat::IntText
              || (Format == EAxonAttrBindFormat::Auto && FMath::IsNearlyEqual(V, FMath::TruncToFloat(V))))
        {
            Out = FString::Printf(TEXT("%d"), FMath::RoundToInt(V));
        }
        else if (Format == EAxonAttrBindFormat::FloatText)
        {
            int32 Decimals = 0;
