...
// AxonGASBulkFillAdapter — Phase 2 adapter for the FAxonBulkFillRegistry.
...
// `axon_discover("gas")` action surface stays identical across build flavours.
//
// NOTE: AttributeInit DataTable writes do NOT need the
// AxonBlueprintEditCradle (transient-subobject reparent + FOverridableManager
...
namespace AxonGASBulkFillInternal
{
	// Build a single-error FAxonDryRunReport for use when target resolution / validation
	// fails. Keeps the dispatcher's return shape uniform whether the walker ran or not.
	// Mirrors AxonBlueprintBulkFillAdapter.cpp::MakeResolveFailureReport.
	...
	// AxonGASAttributeActions.cpp:217-220 (private to that TU).
	...
	// AxonGASAttributeActions.cpp:455-478 (private to that TU). Accepts both
	...
	static UClass* ResolveAttributeSetClass(const FString& AttrSet)
	{
		...
		{
			UObject* Asset = FAxonAssetUtils::LoadAssetByPath(AttrSet);
	...
	static FProperty* ResolveAttributeOnSet(UClass* SetClass, const FString& AttrName)
	{
		...
		// FindPropertyForwarding pattern at AxonReflectionWalker.h:65.
	...
	//              AxonGASAttributeActions.cpp:2270-2298).
	//
	// Object-form: { "Player.1": { "MaxHealth": { "base": 100.0, "min": 0.0, "max": 9999.0 } } }
	//              — sets BaseValue/MinValue/MaxValue from named keys.
	//
	// Emits a FAxonBulkFillFieldWrite into OutReport per cell. Honors Spec.bDryRun
		...
		FAxonDryRunReport& OutReport)
	{
		...
		// semantics. Mirrors AxonGASAttributeActions.cpp:2270-2298.
	...
	// Build the FAxonSchemaDescriptor subtree for a modifier-magnitude tagged union
	...
	static FAxonSchemaDescriptor BuildModifierMagnitudeDescriptor()
	{
		FAxonSchemaDescriptor Desc;
		...
		FAxonSchemaDescriptor ScalableFloat;
		...
		FAxonSchemaDescriptor AttributeBased;
		...
		FAxonSchemaDescriptor SetByCaller;
		...
		FAxonSchemaDescriptor CustomCalc;
	...
	// Build the FAxonSchemaDescriptor for an FAttributeMetaData row (the AttributeInit
	// DT row struct). Self-contained — does not require a target asset.
	static FAxonSchemaDescriptor BuildAttributeMetaDataDescriptor()
	{
		FAxonSchemaDescriptor Desc;
...
// ---------------------------------------------------------------------------
FAxonDryRunReport FAxonGASBulkFillAdapter::GasBulkFill(const FAxonBulkFillSpec& Spec)
{
	using namespace AxonGASBulkFillInternal;
	...
	FScopedTransaction Transaction(NSLOCTEXT(
		"AxonGASBulkFillAdapter", "GasBulkFill",
		"Axon GAS Bulk Fill"));
	DataTable->Modify();
...
#else // WITH_GBA
	// H5 stub-adapter branch — keeps `axon_discover("gas")` action surface
...
// ---------------------------------------------------------------------------
FAxonSchemaDescriptor FAxonGASBulkFillAdapter::GasDescribe(const FString& TargetAsset)
{
	using namespace AxonGASBulkFillInternal;
	...
	if (!TargetAsset.IsEmpty())
	{
		...
		if (UDataTable* DT = Cast<UDataTable>(Asset))
		{
			...
			{
				FAxonSchemaDescriptor Out = BuildAttributeMetaDataDescriptor();
...
// ---------------------------------------------------------------------------
// Registration entry-points (called from FAxonGASModule::StartupModule
// / ShutdownModule).
//
// **H5 invariant**: Register() runs unconditionally — the body switches on
// WITH_GBA, not the registration. This guarantees `axon_discover("gas")`
// returns the same row in dev and release builds.
// ---------------------------------------------------------------------------
void FAxonGASBulkFillAdapter::Register()
{
	...
	FAxonBulkFillRegistry::Get().RegisterAdapter(
		TEXT("gas"),
		FAxonBulkFillRegistry::FBulkFillAdapter(&FAxonGASBulkFillAdapter::GasBulkFill),
		FAxonBulkFillRegistry::FDescribeAdapter(&FAxonGASBulkFillAdapter::GasDescribe));
}

void FAxonGASBulkFillAdapter::Unregister()
{
	FAxonBulkFillRegistry::Get().UnregisterAdapter(TEXT("gas"));
}
