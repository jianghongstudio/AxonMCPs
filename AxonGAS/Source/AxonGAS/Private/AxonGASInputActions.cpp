...
void FAxonGASInputActions::RegisterActions(FAxonToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("gas"), TEXT("setup_ability_input_binding"),
		...
		FAxonActionHandler::CreateStatic(&HandleSetupAbilityInputBinding),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("bind_ability_to_input"),
		...
		FAxonActionHandler::CreateStatic(&HandleBindAbilityToInput),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("batch_bind_abilities"),
		...
		FAxonActionHandler::CreateStatic(&HandleBatchBindAbilities),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("actor_path"), TEXT("Blueprint asset path of the actor"))
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("get_ability_input_bindings"),
		...
		FAxonActionHandler::CreateStatic(&HandleGetAbilityInputBindings),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("actor_path"), TEXT("Blueprint asset path of the actor"))
			.Build());
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("scaffold_input_binding_component"),
		...
		FAxonActionHandler::CreateStatic(&HandleScaffoldInputBindingComponent),
		FParamSchemaBuilder()
...
FAxonActionResult FAxonGASInputActions::HandleSetupAbilityInputBinding(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorPath;
	FAxonActionResult ErrorResult;
	if (!AxonGAS::RequireStringParam(Params, TEXT("actor_path"), ActorPath, ErrorResult))
	{
		return ErrorResult;
	}

	FString BindingMode;
	if (!AxonGAS::RequireStringParam(Params, TEXT("binding_mode"), BindingMode, ErrorResult))
	{
		return ErrorResult;
	}
	...
	FString AssetPath;
	FString LoadError;
	UBlueprint* BP = AxonGAS::LoadBlueprintFromParams(Params, AssetPath, LoadError);
	if (!BP)
	{
		// Try with actor_path since the helper checks asset_path
		FString Error2;
		UObject* Obj = AxonGAS::LoadAssetFromPath(ActorPath, Error2);
...
FAxonActionResult FAxonGASInputActions::HandleBindAbilityToInput(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorPath;
	FAxonActionResult ErrorResult;
	if (!AxonGAS::RequireStringParam(Params, TEXT("actor_path"), ActorPath, ErrorResult))
	...
	if (!AxonGAS::RequireStringParam(Params, TEXT("ability_class"), AbilityClass, ErrorResult))
	{
		return ErrorResult;
	}

	FString InputAction;
	if (!AxonGAS::RequireStringParam(Params, TEXT("input_action"), InputAction, ErrorResult))
	{
		return ErrorResult;
	}
	...
	UObject* AbilityObj = AxonGAS::LoadAssetFromPath(AbilityClass, AbilityError);
	...
	if (BindingMode == TEXT("enhanced_input"))
	{
		...
		FString IAError;
		UObject* IAObj = AxonGAS::LoadAssetFromPath(InputAction, IAError);
...
FAxonActionResult FAxonGASInputActions::HandleBatchBindAbilities(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorPath;
	...
	{
		return FAxonActionResult::Error(TEXT("Missing or empty required parameter: bindings (array)"));
	}
	...
	for (int32 i = 0; i < BindingsArray->Num(); ++i)
	{
		...
		FAxonActionResult SingleResult = HandleBindAbilityToInput(SingleParams);
...
FAxonActionResult FAxonGASInputActions::HandleGetAbilityInputBindings(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorPath;
	FAxonActionResult Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("actor_path"), ActorPath, Err)) return Err;
...
FAxonActionResult FAxonGASInputActions::HandleScaffoldInputBindingComponent(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorPath;
	FAxonActionResult Err;
	if (!AxonGAS::RequireStringParam(Params, TEXT("actor_path"), ActorPath, Err)) return Err;
	...
	{
		return FAxonActionResult::Error(TEXT("Missing required parameter: input_config (object)"));
	}
	...
	FString HeaderContent = FString::Printf(TEXT(
		"// Auto-generated by Axon GAS — AbilityInputBindingComponent\n"
