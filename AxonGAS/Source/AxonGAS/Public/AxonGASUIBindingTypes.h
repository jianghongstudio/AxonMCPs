// AxonGASUIBindingTypes.h
// USTRUCT + UENUM declarations for the Axon UI<->GAS attribute binding feature (Phase H1).
//
// These types are persisted on a Widget Blueprint via the editor-side
// UAxonGASUIBindingBlueprintExtension, and copied at compile time onto
// the runtime UAxonGASAttributeBindingClassExtension which lives on the
...
#include "AxonGASUIBindingTypes.generated.h"

/** How the binding resolves the AbilitySystemComponent owner at runtime. */
UENUM(BlueprintType)
enum class EAxonAttrBindOwner : uint8
{
...
UENUM(BlueprintType)
enum class EAxonAttrBindFormat : uint8
{
...
UENUM(BlueprintType)
enum class EAxonAttrBindUpdate : uint8
{
...
struct AXONGAS_API FAxonGASAttributeBindingSpec
{
    GENERATED_BODY()
    ...
    UPROPERTY(EditAnywhere, Category = "GAS Binding")
    EAxonAttrBindFormat Format = EAxonAttrBindFormat::Auto;
    ...
    /** Update strategy. */
    UPROPERTY(EditAnywhere, Category = "GAS Binding")
    EAxonAttrBindUpdate UpdatePolicy = EAxonAttrBindUpdate::OnChange;
    ...
    bool operator==(const FAxonGASAttributeBindingSpec& Other) const
    {
