// AxonGASAttributeBindingClassExtension.h
...
#include "AxonGASAttributeBindingClassExtension.generated.h"
...
UCLASS(MinimalAPI)
class UAxonGASAttributeBindingClassExtension : public UWidgetBlueprintGeneratedClassExtension
{
    GENERATED_BODY()

public:
    /** Persisted bindings, copied from the editor-side UAxonGASUIBindingBlueprintExtension at compile time. */
    UPROPERTY()
...
private:
    ...
    UAbilitySystemComponent* ResolveASC(UUserWidget* UW, const FAxonGASAttributeBindingSpec& Spec) const;
    ...
    void SubscribeRow(UUserWidget* UW, int32 RowIndex, const FAxonGASAttributeBindingSpec& Spec, FInstanceState& State);

    /** Apply pipeline — coerce float (or float pair) -> widget property. */
    void ApplyValue(UUserWidget* UW, const FAxonGASAttributeBindingSpec& Spec, FActiveSub& Sub) const;
